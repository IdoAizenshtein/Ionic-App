...
import UpdateTokenModal from '../SettingsScreen/components/BaseTokenTab/components/AddTokenModal/UpdateTokenModal'
  ...
  componentDidMount () {
    setTimeout(() => {
      if (ScrollCompWithHeader.listRef && this.props.isActive) {
        ScrollCompWithHeader.listRef.scrollTo({ animated: true, y: this.props.indexRowActive })
      }
    }, 500)
  }
  ...
  render () {
    ...
    >
      <View style={{
        ...
        borderBottomWidth: this.props['propsPage'] && this.props['propsPage'].length > 0 ? 1 : 0,
      }}>
        <View style={{
          height: 44,
          alignContent: 'center',
        ...
        {this.props['propsPage'] && this.props['propsPage'].length > 0 && (this.props.getMessageIds && this.props.getMessageIds.length > 0) && (
          <View style={{
            height: 30,
      ...
      {this.props['propsPage'] && !this.props['propsPage'].length && (
        <View
          ...
          <View style={commonStyles.horizontalCenterContainer}>
            <Image style={{ width: 115, height: 113.5, marginBottom: 40, marginTop: 90 }}
...
@connect(state => ({ currentCompanyId: state.currentCompanyId, isRtl: state.isRtl, accounts: getAccount(state) }))
@translate()
export default class MessagesScreen extends PureComponent {
  constructor (props) {
    super(props)

    const paramsLink = props.navigation.getParam('messageId')
  ...
  handleSetScrollPosition = (e) => this.setState({ currentScrollPosition: e.nativeEvent.contentOffset.y })
  ...
  componentDidMount () {
    ...
    return this.handlerMessages(this.props.navigation.state.params.messageId ? 'page' : 'popup')
  }
  ...
  handlerMessages = (type) => {
    this.setState({ refreshing: true, inProgress: true })
    this.getData(this.getMessages(type))
      .then((data) => {
        let messagesArr = []
        let indexRowActive = null
        if (data.length) {
          if (this.props.navigation.state.params.messageId) {
            indexRowActive = data.findIndex((item) => item.messageId === this.props.navigation.state.params.messageId)
            if (indexRowActive !== -1) {
          ...
          if (indNew.length) {
            messagesArr.push({ title: 'חדש', data: indNew })
          }
          const indNotNew = data.filter((item) => item.indNew === false)
          if (indNotNew.length) {
            messagesArr.push({ title: 'מוקדם יותר', data: indNotNew })
          }
        }
        // console.log(messagesArr)
        this.setState({ messagesArr, type, refreshing: false, inProgress: false, indexRowActive: indexRowActive })

        if (this.props.navigation.state.params.messageId) {
          delete this.props.navigation.state.params.messageId
      ...
      .catch(() => {
        this.setState({ messagesArr: [], type, refreshing: false, inProgress: false })
      })
  }
  ...
  updateMessages = (obj) => {
    this.getData(messagesUpdateApi.post({
      ...
      .then(() => {
        ...
        return this.props.dispatch(getMessagesCount(this.props.currentCompanyId))
      })
      .catch(() => {
  ...
  doAction = (data, act) => {
    ...
    switch (action) {
      ...
      case 'openPasswordPopup':
        ...
        break
      case 'taryaPopup':
        const openTarya = navigation.getParam('openTarya')
        if (openTarya !== undefined) {
          this.props.navigation.setParams({ openTarya: openTarya + 1 })
        } else {
          this.props.navigation.setParams({ openTarya: 0 })
        }
        break
      case 'SlikaTwoMonths':
      ...
      case 'updateNonCyclicTransaction':
        this.handlePopRowEditsModal(data.keyId, data.companyAccountId, action === 'updateTazrimTransaction')
        break
      ...
      case 'showCheqimLenicaion':
        goTo(navigation, CHECKS, {
          paramsLink: {
            queryStatus: action === 'showCheqimLemishmeret' ? 'mishmeret_babank' : 'lenicaion',
  ...
  handlePopRowEditsModal = (keyId, companyAccountId, isUpdateTazrimTransaction) => {
    const companyAccountIds = [companyAccountId]
    getAccountCflTransTypeApi.post({ body: { uuid: this.props.currentCompanyId } })
      .then(data => {
        ...
        cashFlowDetailsDataApi.post({
          body: {
            ...
            companyId: this.props.currentCompanyId,
            dateFrom: AppTimezone.moment(AppTimezone.moment().valueOf()).toISOString(),
            dateTill: AppTimezone.moment(AppTimezone.moment().add(30, 'days').valueOf()).toISOString(),
            expence: -1,
          },
        }).then((data) => {
          const rowInfo = data.cashFlowDetails.filter((it) => it.transId === keyId)
          if (rowInfo.length) {
            if (!isUpdateTazrimTransaction) {
              if (rowInfo[0].nigreret) {
                rowInfo[0].originalDate = AppTimezone.moment().add(30, 'days').valueOf()
              } else {
                rowInfo[0].transDate = AppTimezone.moment().add(30, 'days').valueOf()
              }
            }
            this.setState({
              rowInfo: rowInfo[0],
  ...
  getStatus = (token) => {
    if (this.props.currentCompanyId) {
      return getStatusTokenTypeApi.post({ body: { companyId: this.props.currentCompanyId, tokens: [token] } })
        .then(([newTokenStatus]) => {
          const statusCode = BankTokenService.getTokenStatusCode(newTokenStatus.tokenStatus)
          this.setState({ statusToken: statusCode })
          this.setState({ currentToken: newTokenStatus, updateTokenModalIsOpen: true })
        })
  ...
  get getMessageIds () {
    ...
    if (messagesArr && messagesArr.length) {
      const messagesArrFirst = messagesArr[0].data.filter((item) => item.indRead === false).map((it) => it.messageId)
      if (messagesArr.length > 1) {
        messageIds = messagesArrFirst.concat(messagesArr[1].data.filter((item) => item.indRead === false).map((it) => it.messageId))
      } else {
  ...
  updateAsHide = (id) => {
    ...
    const indexToDelete = messagesArr[0].data.findIndex(element => element.messageId === id)
    if (indexToDelete !== -1) {
      messagesArr[0].data.splice(indexToDelete, 1)
    }

    if (messagesArr.length > 1) {
      const indexToDelete = messagesArr[1].data.findIndex(element => element.messageId === id)
      if (indexToDelete !== -1) {
    ...
    return (
      <SafeAreaView style={[{
        ...
        <AnimatedSectionList
          ...
          renderScrollComponent={(props) => <ScrollCompWithHeader props={props} propsPage={messagesArr}
            type={type}
