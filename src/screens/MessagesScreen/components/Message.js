...
export default class Message extends PureComponent {
  constructor (props) {
    ...
    if (props.isActive) {
      setTimeout(() => {
        Animated.timing(this.state.animBg, { toValue: 1, duration: 6000 }).start()
      }, 10)
    }
  }
  ...
  get renderText () {
    ...
    if (!linkedText || (linkedText && data.linked_action && data.linked_action === 'getTransHistory' && IS_LIGHT.light)) {
      return (<Text style={{
        textAlign: 'right',
        color: '#022258',
        fontSize: sp(15),
      ...
      return (
        <Text style={{
          ...
          {data && data.linked_action && data.linked_action === 'getTransHistoryForLite' ? (
            <Text>
              {linkedText}
            </Text>
          ) : (
  ...
  componentWillReceiveProps ({ screenSwitchState }) {
    ...
    render () {
      ...
      const left = [
        {
          ...
          component: (
            <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
              {!data.indRead && (<Image style={{ width: 20, height: 20 }}
                source={require(`BiziboxUI/assets/read.png`)} />)}
              {data.indRead && (<Image style={{ width: 20, height: 20 }}
                source={require(`BiziboxUI/assets/unread.png`)} />)}
              <Text style={{
                paddingTop: 2,
                color: '#fff',
                fontSize: sp(14),
      ...
      return (
        <Fragment>
          <View style={{
            borderRightColor: data.indAlert === 'bizibox' ? (data.indRead ? '#ffffff' : '#edf4f8') : (data.indAlert === 'red' ? '#cd1010' : '#f0c100'),
            borderRightWidth: 5,
          }}>
            <Swipeout
              ...
              <Animated.View style={{
                ...
                <View style={{
                  ...
                  {data.iconName === 'system_alert' && (
                    <Image style={{ width: 22, height: 22, marginHorizontal: 2 }}
                ...
                <View style={{
                  ...
                  <View style={{
                    ...
                    <Text numberOfLines={1}
                      ...
                      {(AppTimezone.moment().diff(AppTimezone.moment(data.dateCreated), 'days') === -1)
                        ? (t('sumsTitles:today'))
                        : (
                          (AppTimezone.moment().diff(AppTimezone.moment(data.dateCreated), 'days') > 0)
                            ? t('bankAccount:lastUpdatedXDaysAgo', { days: AppTimezone.moment().diff(AppTimezone.moment(data.dateCreated), 'days') })
