...
export default class SettingsMenu extends PureComponent {
    ...
    render () {
      ...
      return (
        <View style={cs(currentTab, styles.menuContainer, styles.menuContainerCollapsed)}>
          <ScrollView
          ...
          >
            ...
            {menu.map(item => {
              return (
                <TouchableOpacity
                  key={item.name}
                  style={cs(currentTab === item.name, styles.menuRow, styles.menuRowActive)}
                  onPress={this.handleSetTab(item.name)}
                >
                  {!currentTab && <Text style={styles.menuText}>{item.title}</Text>}
                  <CustomIcon name={item.icon} size={20}
                    color={currentTab === item.name ? colors.white : colors.blue5} />
                </TouchableOpacity>
              )
            })}
          </ScrollView>
