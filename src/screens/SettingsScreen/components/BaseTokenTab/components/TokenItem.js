...
export default class TokenItem extends AnimatedRow {
  ...
  get accountsShow () {
    ...
    const filteredAccounts = isEmpty(items) ? [] : items.filter(a => a.token === token.token)
    const filteredDeletedAccounts = isEmpty(deletedItems)
      ? []
      : deletedItems.filter(a => a.token === token.token).map(a => ({ ...a, deleted: true }))

    return isShowRemovedItems ? [...filteredAccounts, ...filteredDeletedAccounts] : [...filteredAccounts]
  }
  ...
  handleToggle () {
    ...
    if (!isEmpty(accountsShow)) {
      ...
      Animated.timing(height, { toValue: finalValue, duration: 200 }).start()
    }
  }
  ...
  render () {
    ...
    return (
      <Animated.View style={[styles.tokenRowWrapper, { height }]}>
        <TouchableOpacity style={{ height: TOKEN_ROW_MIN_HEIGHT }} onPress={this.handleToggle}
          ...
          <View style={styles.tokenItemWrapper} onLayout={this.setMinHeight}>
            <TokenStatus token={token} openAddToken={openAddToken}
              onOpenTokenUpdateModal={onOpenTokenUpdateModal} currentCompany={currentCompany} />
            ...
            <View style={[styles.tokenItemLeftPart]}>
              ...
              <Text style={styles.tokenTitle}>{(token.tokenNickname || '')}</Text>
              <View style={commonStyles.spaceDivider} />
              <AccountIcon tokenType={tokenType} account={{ bankId: token.websiteTargetTypeId }} />
            </View>
          </View>
        </TouchableOpacity>
