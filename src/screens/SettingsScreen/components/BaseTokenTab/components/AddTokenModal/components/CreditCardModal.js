...
import accountModalStyles from 'src/components/AccountsModal/AccountsModalStyles'
import { colors } from 'src/styles/vars'
import { CREDIT_CARD_ICONS, CREDIT_CARDS_CREDENTIALS_SCHEME } from 'src/constants/bank'
...
export default class CreditCardModal extends PureComponent {
    ...
    render () {
      ...
      return (
        ...
        >
          <ScrollView
            style={[accountModalStyles.modalBodyRtl, accountModalStyles.modalBodyRtl]}
            contentContainerStyle={{ paddingVertical: 20 }}
          >
            {Object.keys(CREDIT_CARDS_CREDENTIALS_SCHEME).map(cardId => {
              const wrapperStyle = [accountModalStyles.item, accountModalStyles.itemRtl]
              ...
              return (
                <View
                  key={cardId}
                  style={[accountModalStyles.itemsWrapper, accountModalStyles.itemsWrapperRtl]}
                >
                  <TouchableOpacity
                    style={wrapperStyle}
                  ...
                  >
                    <View style={accountModalStyles.checkerWrapper}>
                      <Image source={CREDIT_CARD_ICONS[cardId].uri} style={{ width: 30, height: 19 }} />
                    ...
                    <Text style={accountModalStyles.itemText}>{t(`creditCardsName:${cardId}`)}</Text>
                  </TouchableOpacity>
                </View>
              )
            })}
