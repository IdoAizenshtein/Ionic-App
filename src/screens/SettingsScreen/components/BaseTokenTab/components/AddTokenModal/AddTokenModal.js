...
import { createTokenTypeApi, getStatusTokenTypeApi, updateTokenTypeApi } from 'src/api'
...
export default class AddTokenModal extends PureComponent {
    ...
    constructor (props) {
      ...
      this.state = ({
        ...
        accountId: isEmpty(props.accounts) ? null : props.accounts[0].companyAccountId,
        formData: {},
        formErrors: {},
    ...
    handleUpdateFormField = (name) => (value) => {
      let val = value || ''
      val = val.toString().replace(getEmoji(), '').replace(/([\u05D0-\u05FF/\s+]+)/g, '')
    ...
    handleChangeCreditCardId = (creditCardId) => {
      this.setState({ creditCardId, formData: {}, formErrors: {}, currentOtpField: null })
    };

    handleChangeAccountId = (accountId) => {
      this.setState({ accountId, formData: {}, formErrors: {}, currentOtpField: null })
    };

    handleChangeSlikaId = (slikaId) => {
      this.setState({ slikaId, formData: {}, formErrors: {}, currentOtpField: null })
    };

    handleToggleBankModal = () => this.setState({ bankModalIsOpen: !this.state.bankModalIsOpen });

    handleToggleCreditCardModal = () => this.setState({ creditCardModalIsOpen: !this.state.creditCardModalIsOpen });

    handleToggleAccountModal = () => this.setState({ accountModalIsOpen: !this.state.accountModalIsOpen });

    handleToggleSlikaModal = () => this.setState({ slikaModalIsOpen: !this.state.slikaModalIsOpen });
    ...
    handleCreateBankToken = () => {
      ...
      const schemeId = tokenType === 'CREDITCARD' ? creditCardId : tokenType === 'SLIKA' ? slikaId : bankId
      const scheme = ALL_BANK_CREDENTIALS_SCHEME[schemeId]
      if (!scheme || inProgress || !isEmpty(formErrors) || isEmpty(scheme.fields)) return
        ...
        if (companyId) {
          return getStatusTokenTypeApi.post({ body: { companyId, tokens: [newTokenId] } })
            .then(([newTokenStatus]) => {
              ...
              if (BankTokenService.isTokenStatusProgressing(newTokenStatus.tokenStatus) || statusCode === BANK_TOKEN_STATUS.ALMOST_DONE) {
                this.setState({
                  inProgress: false,
    ...
    renderHeaderForm = () => {
      ...
      const currentAccount = isEmpty(accounts) ? {} : accounts.find(a => a.companyAccountId === accountId)
      ...
      return (
        <Fragment>
          {tokenType === 'ACCOUNT' ? (
            ...
            <View style={styles.formRow}>
              <View style={styles.formRowLeftPart}>
                <TouchableOpacity style={styles.bankTypeBtn} onPress={this.handleToggleAccountModal}>
                  <Icon name='chevron-left' size={22} color={colors.blue8} />

                  <View style={[commonStyles.row, commonStyles.alignItemsCenter]}>
                    <Text style={styles.bankName}>{currentAccount.accountNickname}</Text>
          ...
          {tokenType === 'CREDITCARD' && (
            <View style={styles.formRow}>
              <View style={styles.formRowLeftPart}>
                <TouchableOpacity style={styles.bankTypeBtn} onPress={this.handleToggleCreditCardModal}>
                  <Icon name='chevron-left' size={22} color={colors.blue8} />

                  <View style={[commonStyles.row, commonStyles.alignItemsCenter]}>
                    <Text style={styles.bankName}>{t(`creditCardsName:${creditCardId}`)}</Text>
          ...
          {tokenType === 'SLIKA' && (
            <View style={styles.formRow}>
              <View style={styles.formRowLeftPart}>
                <TouchableOpacity style={styles.bankTypeBtn} onPress={this.handleToggleSlikaModal}>
                  <Icon name='chevron-left' size={22} color={colors.blue8} />

                  <View style={[commonStyles.row, commonStyles.alignItemsCenter]}>
                    <Text style={styles.bankName}>{t(`clearingAgenciesName:${slikaId}`)}</Text>
        ...
        >
            ...
            <LoaderWrapper isDefalse isLoading={inProgress}>
              ...
              <View style={{
                ...
                <Text style={{
                  ...
                  color: colors.blue8,
                  textAlign: 'center',
                }}>{tokenType === 'CREDITCARD' ? 'ניתן לשנות את חשבון הבנק אליו משויך כרטיס האשראי בכל עת דרך מסך הגדרות - כרטיסי אשראי' : tokenType === 'SLIKA' ? 'ניתן לשנות את חשבון הבנק אליו משויך הסולק בכל עת דרך מסך הגדרות - חשבונות סליקה' : ''}</Text>
              </View>
            </LoaderWrapper>
          </ScrollView>
          ...
          {accountModalIsOpen && (
            <AccountModal
              ...
              accounts={tokenType === 'ACCOUNT' ? accounts : accounts.filter((it) => it.currency === 'ILS')}
