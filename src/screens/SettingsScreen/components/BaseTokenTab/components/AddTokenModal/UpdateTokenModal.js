...
export default class UpdateTokenModal extends PureComponent {
    ...
    constructor (props) {
      ...
      const statusCode = (typeof props.token.tokenStatus === 'number') ? props.token.tokenStatus : BankTokenService.getTokenStatusCode(props.token.tokenStatus)
        ...
        return (
          <Fragment>
            ...
            {formData.password !== formData.newPassword && (
              <Text style={styles.errMessageText}>{t('settings:bankAccountsTab:passwordsAreNotBeSame')}</Text>
            )}
          </Fragment>
        )
      }
    ...
    handleUpdateBankToken = () => {
      ...
      const body = {
        ...
        companyId,
        tokenId: token.token,
        bankAuto: this.isUpdateOtpCode ? formData[`__otp__${currentOtpField}`] : null,
      ...
      return updateTokenTypeApi.post({ body })
        .then((time) => {
          ...
          this.props.navigation.setParams({ updateToken: token.token })
          if (companyId) {
            return getStatusTokenTypeApi.post({ body: { companyId, tokens: [token.token] } })
          } else {
            return false
          }
        })
    ...
    validateForm = () => {
      ...
      if (this.isUpdatePassword || this.isBlocked || this.isValid) {
        const passwordField = scheme.fields.find(f => f.controlType === BANK_CREDENTIALS_CONTROL_TYPES.PASSWORD)
        ...
        if (this.isBlocked || this.isValid) {
          const usernameField = scheme.fields.find(f => f.controlType === BANK_CREDENTIALS_CONTROL_TYPES.TEXT)
    ...
    render () {
      ...
      return (
        <Modal
          isOpen
          title={this.isBlocked ? (t('settings:bankAccountsTab:unblockAccountIsBlocked', { tokenName: token.tokenNickname }))
            : (this.isValid ? (
              (tokenType === 'ACCOUNT') ? t('settings:bankAccountsTab:updateAccount', { tokenName: token.tokenNickname }) : t('settings:bankAccountsTab:updateCard', { tokenName: token.tokenNickname })
            ) : (
              (tokenType === 'ACCOUNT') ? t('settings:bankAccountsTab:unblockAccount', { tokenName: token.tokenNickname }) : t('settings:bankAccountsTab:unblockCard', { tokenName: token.tokenNickname })
            ))
          }
          ...
          >
            ...
            <LoaderWrapper isDefalse isLoading={inProgress}>
              <Text style={styles.addAccountTitle}>
                {this.isValid ? 'הזינו את פרטי הכניסה העדכניים לאתר הבנק' : (this.isUpdatePassword
              ...
              <View style={styles.validationContainer}>
                ...
                {'message' in he.foreignCredentials[bankId] && (
                  <Text style={styles.infoMessageText}>{t(`foreignCredentials:${bankId}:message`)}</Text>
                )}
              </View>
            </LoaderWrapper>
          </ScrollView>
