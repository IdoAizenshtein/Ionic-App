...
export default class AccountRecoveryModal extends PureComponent {
  render () {
    ...
    return (
      <Modal
        isOpen
        title={tokenType === 'SLIKA' ? t('settings:bankAccountsTab:solekAccountRecovery') : t('settings:bankAccountsTab:bankAccountRecovery')}
      ...
      >
        ...
        >
          <Image style={styles.bucketIcon} source={require('BiziboxUI/assets/bucket.png')} />
          <Text style={styles.modalTitle1}>
            {
              (tokenType === 'ACCOUNT') ? t('settings:bankAccountsTab:recoverAccountName', { accountName: account.accountNickname })
                : tokenType === 'SLIKA' ? t('settings:bankAccountsTab:recoverSolekName', { accountName: account.solekDesc })
                  : t('settings:bankAccountsTab:recoverCardName', { accountName: account.creditCardNickname })
            }
          </Text>
          ...
          <TouchableOpacity style={styles.modalBtn} onPress={onSubmit}>
            <Text
              style={styles.modalBtnText}>{(tokenType === 'ACCOUNT' || tokenType === 'SLIKA') ? t('settings:bankAccountsTab:accountRecovery') : t('settings:bankAccountsTab:cardRecovery')}</Text>
          </TouchableOpacity>
