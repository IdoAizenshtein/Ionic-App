...
export default class TokenStatus extends PureComponent {
    ...
    constructor (props) {
      ...
      const tokenStatusKey = BankTokenService.getTokenStatusEnumKey(tokenStatus) ? BankTokenService.getTokenStatusEnumKey(tokenStatus) : ((tokenStatus === 'VALIDPOALIMBAASAKIM' || tokenStatus === 'SUSPENDED' || tokenStatus === 'AGREEMENT_REQUIRED' || tokenStatus === 'INVALIDPASSORDANDACCESS') ? tokenStatus : undefined)
      ...
      this.state = {
        ...
        isShouldUpdatePassword: BankTokenService.isShouldTokenUpdatePassword(tokenStatus),
        statusTitle: tokenStatusKey ? t(`settings:bankAccountsTab:status:${tokenStatusKey}`) : tokenStatus,
      }
    ...
    get icon () {
      ...
      if (hasProgressBar) {
        return (
          <Image
            style={[styles.imgIcon, { width: 11, height: 14 }]}
      ...
      switch (tokenStatusCode) {
        ...
        case BANK_TOKEN_STATUS.INVALID_PASSWORD_AND_ACCESS:
          return <CustomIcon name='exclamation-triangle' size={18} color={colors.red2} />
        case BANK_TOKEN_STATUS.TECHNICAL_PROBLEM:
          return (
            <Image
              style={[styles.imgIcon, { width: 15, height: 15 }]}
        ...
        case BANK_TOKEN_STATUS.BLOCKED:
          return (
            <Image
              style={[styles.imgIcon, { width: 13, height: 18 }]}
        ...
        case BANK_TOKEN_STATUS.DISCOD_REQUIRED:
          return (
            <Image
              style={[styles.imgIcon, { width: 11, height: 22 }]}
        ...
        case BANK_TOKEN_STATUS.PASSWORD_ABOUT_TO_EXPIRED:
          return (
            <Image
              style={[styles.imgIcon, { width: 14, height: 14 }]}
        ...
        case BANK_TOKEN_STATUS.AGREEMENT_REQUIRED:
          return (
            <Image
              style={[styles.imgIcon, { width: 16, height: 16 }]}
          ...
          return getStatusTokenTypeApi.post({
            ...
            .then(([newTokenStatus]) => {
              ...
              this.setState({
                ...
                percent: BankTokenService.getCompletedPercentage(tokenStatus),
                isShouldUpdatePassword: BankTokenService.isShouldTokenUpdatePassword(tokenStatus),
                statusTitle: tokenStatusKey ? this.props.t(`settings:bankAccountsTab:status:${tokenStatusKey}`) : tokenStatus,
              })
      ...
      return (
        <View style={styles.statusContainer}>
          <View style={styles.statusWrapper}>
            ...
            {!(token.tokenNickname === null && token.tokenStatus === null && token.tokenTargetType === null) && (
              <View>
            ...
            {(token.tokenNickname === null && token.tokenStatus === null && token.tokenTargetType === null) && (
              <View>
                <TouchableOpacity
                  hitSlop={{ top: 20, bottom: 20, left: 20, right: 20 }}
                  onPress={(token.tokenNickname === null && token.tokenStatus === null && token.tokenTargetType === null) ? openAddToken : onOpenTokenUpdateModal(token)}
                  activeOpacity={1}
                >
