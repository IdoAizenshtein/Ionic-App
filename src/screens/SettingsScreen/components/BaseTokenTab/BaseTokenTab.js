...
export default class BaseTokenTab extends PureComponent {
  ...
  getData = () => {
    ...
    return Promise.all([
      ...
      .then(([tokens, items]) => {
        ...
        if (tokens && !tokens.length && items && items.length) {
          ...
          for (const property in arrayGr) {
            ...
            tokensGrNew.push({
              token: objAcc.token,
              tokenStatus: null,
              tokenNickname: tokenType === 'SLIKA' ? t(`clearingAgenciesName:${objAcc.solekBankId}`) : (tokenType === 'CREDITCARD' ? t(`creditCardsName:${objAcc.creditCardTypeId}`) : t(`bankName:${objAcc.bankId}`)),
              isFromAccount: true,
              websiteTargetTypeId: tokenType === 'SLIKA' ? objAcc.solekBankId : (tokenType === 'CREDITCARD' ? objAcc.creditCardTypeId : objAcc.bankId),
          ...
          const allAccWithoutTknMatch = items.filter((acc) => tokens.every((tkn) => tkn.token !== acc.token))
        ...
        this.setState({ tokens, items, inProgress: !!(isShowRemovedAccounts) })
  ...
  getDeletedItems = () => {
    ...
    if (isShowRemovedAccounts) {
      getDeletedItems(tokens ? tokens.map(t => t.token) : [])
        .then((deletedItems) => {
          this.setState({ deletedItems, inProgress: false })
        })
  ...
  handleOpenAccountRecoverModal = (account) => {
    this.setState({ selectedAccountToAction: account, accountRecoveryModalIsOpen: true })
  }

  handleCloseAccountRecoverModal = () => {
    this.setState({ selectedAccountToAction: null, accountRecoveryModalIsOpen: false })
  }
  ...
  handleRecoverAccount = () => {
    ...
    if (inProgress || !selectedAccountToAction) return

    this.setState({ inProgress: true, selectedAccountToAction: null, accountRecoveryModalIsOpen: false })
  ...
  handleOpenAccountUpdateModal = (account) => {
    this.setState({ selectedAccountToAction: account, accountUpdateModalIsOpen: true })
  }

  handleCloseAccountUpdateModal = () => {
    this.setState({ selectedAccountToAction: null, accountUpdateModalIsOpen: false })
  }
  ...
  handleUpdateItem = ({ ...args }) => {
    ...
    this.setState({ inProgress: true, selectedAccountToAction: null, accountUpdateModalIsOpen: false })
    return onUpdateItem({ ...args, item: selectedAccountToAction }).finally(this.getData)
  }
  ...
  handleToggleAddAccountModal = () => {
    ...
    if (addAccountModalIsOpen) this.getData()
  }
  ...
  async componentWillReceiveProps (nextProps) {
    ...
    if (!isUpdate && tokenType === nextProps.tokenType) return
    this.setState({ ...this.getInitialState() }, this.getData)
  }

  async componentDidMount () {
    try {
      const isShowRemovedAccounts = await this.getDataStorage('isShowRemovedAccounts')
      this.setState({
        isShowRemovedAccounts: (isShowRemovedAccounts !== undefined && isShowRemovedAccounts !== null) ? isShowRemovedAccounts : true,
      }, this.getData)
    } catch (error) {
      console.log(error)
      this.setState({
      ...
      <Fragment>
        {!paramsLinkAddCard && (
          <View style={styles.switchWrapper}>
            <Text style={styles.switchText}>
              {tokenType === 'ACCOUNT' ? t('settings:bankAccountsTab:viewDeletedAccounts') : t('settings:bankAccountsTab:viewDeletedCards')}
            </Text>
        ...
        {!paramsLinkAddCard && (
          <View style={styles.addBtnWrapper}>
            <TouchableOpacity
              activeOpacity={(exampleCompany && tokenType !== 'ACCOUNT') ? 0.4 : 0.6}
              style={[styles.addNewBtn, {
                opacity: (exampleCompany && tokenType !== 'ACCOUNT') ? 0.4 : 1,
              }]}
              onPress={(exampleCompany && tokenType !== 'ACCOUNT') ? null : this.handleToggleAddAccountModal}>
              <CustomIcon
                name='plus'
                size={32}
                color={colors.white}
