      ...
      return (
        <View style={styles.accountDetailsWrapper}>
          <View style={styles.accountDetailsFirstRow}>
            {isDeleted
              ? (
                <TouchableOpacity onPress={this.handleOpenRecoveryModal}>
                  <Text style={styles.accountLinkText}>{t(`settings:bankAccountsTab:cardRecovery`)}</Text>
                </TouchableOpacity>
              )
              : (
            ...
            <Text
              style={cs(isDeleted, [styles.accountTitleText, commonStyles.semiBoldFont], styles.accountPaleText)}>
          ...
          <View style={[commonStyles.row, commonStyles.spaceBetween]}>
            <View>
              ...
              {isNull(creditCard.creditLimit)
                ...
                : (
                  <Text style={cs(isDeleted, tabStyles.creditLimitText, styles.accountPaleText)}>
                    <Text style={cs(isDeleted, commonStyles.lightFont, styles.accountPaleText)}>
            ...
            <View>
              <Text
                ...
                {account.accountNickname} {' | '} {`${creditCard.cycleDay} ${t('creditCards:perMonth')}`}
              </Text>
              ...
              <View style={{
                ...
                {(creditCard.isUpdate === false) && (
                  <View style={{
                    ...
                    <CustomIcon name='exclamation-triangle' size={16} color={colors.red2} />
                ...
                {!creditCard.deleted && (
                  <Text
                    ...
                    {(creditCard.isUpdate === false) && (
                      <Text style={[commonStyles.textRight, { color: colors.red6 }]}>
                        {(AppTimezone.moment().diff(AppTimezone.moment(creditCard.balanceLastUpdatedDate), 'days') > 0) ? AppTimezone.moment(creditCard.balanceLastUpdatedDate).format('DD/MM/YY') : (
                    ...
                    {creditCard.isUpdate && (
                      <Text
                        style={[commonStyles.boldFont, commonStyles.textRight, { color: colors.green4 }]}>
                        {isToday(creditCard.balanceLastUpdatedDate) ? t('calendar:today') : (isYesterday(creditCard.balanceLastUpdatedDate) ? t('calendar:yesterday') : t('settings:creditCardsTab:notUpdated'))}
                      </Text>
                    )}
                  </Text>
                )}
