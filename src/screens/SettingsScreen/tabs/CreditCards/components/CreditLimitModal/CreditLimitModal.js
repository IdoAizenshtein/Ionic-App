...
import { Dimensions, Keyboard, Modal, Text, TextInput, TouchableOpacity, View } from 'react-native'
...
export default class CreditLimitModal extends PureComponent {
  constructor (props) {
    ...
    this.state = { modalTop: -3000, value: props.total ? props.total.toString() : '' }
  }
  ...
  keyboardDidShow = (e) => {
    this.setState({ modalTop: winHeight - e.endCoordinates.height - CREDIT_LIMIT_MODAL_HEIGHT })
  }
        ...
        <View style={styles.creditLimitModalWrapper}>
          ...
          <View style={[styles.creditLimitModalInner, { top: modalTop }]}>
            <TextInput
              ...
              value={inputWorkaround.getWorkaroundChar() + `${getCurrencyChar(currency)} ${value}`}
              returnKeyType='done'
              multiline={false}
              autoCorrect={false}
              ...
              onEndEditing={(e) => {
                if (this.props.allowPoint) {
                  this.setState({ value: e.nativeEvent.text.toString().replace(/[^\d.]/g, '') })
                } else {
                  this.setState({ value: e.nativeEvent.text.toString().replace(/[^\d]/g, '') })
                }
              }}
            ...
            <TouchableOpacity style={styles.saveBtnWrapper} onPress={this.handleSubmit}>
              <Text style={styles.saveBtnText}>{inProgress ? t(`common:loading`) : t('common:update')}</Text>
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
