...
export default class CreditCardDetailsWrapper extends PureComponent {
  get creditCards () {
    ...
    const filteredCards = isEmpty(items) ? [] : items.filter(c => c.token === token.token)
    const filteredDeletedCards = isEmpty(deletedItems)
      ? []
      : deletedItems.filter(c => c.token === token.token).map(c => ({ ...c, deleted: true }))

    return isShowRemovedItems ? [...filteredCards, ...filteredDeletedCards] : [...filteredCards]
  }

    getAccount = (card) => {
      const account = this.props.accounts.find(a => a.companyAccountId === card.companyAccountId)
      return account || {}
    };
    ...
    render () {
      ...
      if (isEmpty(creditCards)) {
        return (
          <Text style={[commonStyles.textCenter, { marginVertical: 10 }]}>Credit cards not found</Text>
        )
      }
