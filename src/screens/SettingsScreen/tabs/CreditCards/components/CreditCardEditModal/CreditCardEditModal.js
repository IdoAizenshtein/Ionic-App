...
export default class CreditCardEditModal extends PureComponent {
  constructor (props) {
    ...
    handleApplyChanges = () => {
      ...
      return (name.length || (creditLimit && creditLimit !== this.props.item.creditLimit)) ? onUpdate({
        name,
        companyAccountId,
        creditLimit: ((creditLimit !== this.props.item.creditLimit) ? creditLimit : this.props.item.creditLimit),
      }) : null
    };
