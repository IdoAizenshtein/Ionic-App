...
import tabStyles from '../../../../../components/BaseTokenTab/BaseTokenTabStyles'
...
export default class CreditCardDetails extends PureComponent {
  constructor (props) {
    ...
    this.state = {
      ...
      dataList: types,
      account: types.find((it) => it.companyAccountId === this.props.creditCard.companyAccountId),
    }
  }
    ...
    closeModalChangeAccounts = () => {
      this.setState({
        modalChangeAccounts: false,
        modalAlertChangeAccounts: this.state.account.companyAccountId !== this.props.creditCard.companyAccountId,
      })
    };
    ...
    render () {
      const { t, onRemove, name, onChangeName, isRtl, creditCard, creditLimit, onChangeCreditLimit } = this.props
          ...
          >
            <View style={styles.detailsRow}>
              ...
              <View style={styles.detailsRightPart}>
                <Text
                  style={styles.detailsRightText}>{t('settings:creditCardsTab:editCreditCardName')}</Text>
              </View>
            </View>
            ...
            <View style={styles.detailsRow}>
              ...
              <View style={styles.detailsRightPart}>
                <Text style={styles.detailsRightText}>{'מסגרת אשראי'}</Text>
              </View>
            </View>

            {(creditCard.bankLoaded === false) && (
              <View style={styles.detailsRow}>
                <TouchableOpacity style={[styles.detailsLeftPart, {
                  flexDirection: 'row',
                  ...
                  <Text style={styles.detailsRightText}>{account.accountNickname}</Text>
                  <View style={{ paddingHorizontal: 2 }}><AccountIcon account={account} /></View>
                </TouchableOpacity>
                <View style={styles.detailsRightPart}>
                  <Text style={styles.detailsRightText}>{t('settings:slikaTab:account')}</Text>
                </View>
              </View>
            )}
            ...
            <View style={styles.detailsLastRow}>
              <TouchableOpacity onPress={onRemove}>
                <Text style={tabStyles.modalLinkText}>
                  {t('settings:creditCardsTab:deleteCard')}
                </Text>
              </TouchableOpacity>
            </View>
          </ScrollView>
                      ...
                      {dataList.map((item, i) => (
                        <TouchableOpacity
                          ...
                          style={{
                            ...
                            backgroundColor: (item.companyAccountId === account.companyAccountId) ? '#f5f5f5' : '#ffffff',
                          }}>
                          ...
                          <View style={{
                            ...
                            <Text
                              style={{ color: colors.red }}>{` - ${t('bankAccount:notUpdated')}`}</Text>}
                            <Text
            ...
            <SafeAreaView style={{
              ...
              <View style={{
                ...
                <View style={{
                  ...
                  >
                    <Image
                      style={[styles.imgIcon, { width: 70, height: 51, marginBottom: 20 }]}
                      source={require('BiziboxUI/assets/swichIcon.png')}
                    />

                    <Text style={styles.confirmRemoveTitle1}>
                      {t('settings:creditCardsTab:changeCardAcc', { name: account.accountNickname.replace(/"/g, '') })}
                    </Text>

                    <Text
                      style={styles.confirmRemoveDesc1}>{t('settings:creditCardsTab:changeCardAccDesc')}</Text>
                    <TouchableOpacity style={[tabStyles.modalBtn, {
                      marginTop: 20,
                    ...
                    <TouchableOpacity onPress={this.cancelChangeAcc}>
                      <Text style={tabStyles.modalLinkText}>{t('common:cancel')}</Text>
                    </TouchableOpacity>
