        ...
        <View style={styles.accountDetailsWrapper}>
          <View style={styles.accountDetailsFirstRow}>
            {isDeleted
              ? (
                <TouchableOpacity onPress={this.handleOpenAccountRecoveryModal}>
                  <Text
                    style={styles.accountLinkText}>{t(`settings:bankAccountsTab:accountRecovery`)}</Text>
                </TouchableOpacity>
              )
              : (
            ...
            <View style={{
              ...
              <Text style={[cs(isDeleted, styles.accountTitleText, styles.accountPaleText)]}>
          ...
          <View style={[commonStyles.row, commonStyles.spaceBetween]}>
            <View>
              <Text
                style={cs(isDeleted, [styles.accountRegularText, commonStyles.textCenter], styles.accountPaleText)}>
                {t('settings:bankAccountsTab:lastUpdate')}
              </Text>
              {isToday(account.balanceLastUpdatedDate)
                ? (
                  <Text style={cs(isDeleted, styles.accountTodayText, styles.accountPaleText)}>
                    {t('calendar:today')}
                  </Text>
                )
                : isYesterday(account.balanceLastUpdatedDate)
                  ? (
                    <Text
                      style={[cs(isDeleted, styles.accountTodayText, styles.accountPaleText), { color: colors.red2 }]}>
                      {t('calendar:yesterday')}
                    </Text>
                  )
                  : (
                    <Text style={cs(isDeleted, styles.accountDateText, styles.accountPaleText)}>
                      {AppTimezone.moment(account.balanceLastUpdatedDate).format(DEFAULT_DATE_FORMAT)}
                    </Text>
                  )}
            </View>
            ...
            <View>
              <Text
                style={cs(isDeleted, [styles.accountRegularText, commonStyles.textRight], styles.accountPaleText)}>
                {t('settings:bankAccountsTab:branchNumber', { number: account.bankSnifId })}
              </Text>

              <View style={[commonStyles.row, commonStyles.spaceBetween, commonStyles.alignItemsCenter]}>
                {(!isEmpty(account.privsList) && !account.privsList.includes('Denied Access')) ? (
                  <TouchableOpacity onPress={this.handleGoToUsers}>
                    <Text style={styles.accountLinkText}>
                      {(account.privsList.length > 1)
                        ? t('settings:bankAccountsTab:usersCount', { count: account.privsList.length })
                        : account.privsList[0]}
                    </Text>
                  </TouchableOpacity>
                ) : null}
                ...
                {(!isDeleted && account.creditCardNum !== 0) ? (
                  <TouchableOpacity onPress={this.handleGoToCards}>
                    <Text style={[styles.accountLinkText, {
                      ...
                      {account.creditCardNum >= 2 && t('settings:bankAccountsTab:creditCardsNum', { creditCardsNum: account.creditCardNum })}
                      {account.creditCardNum === 1 && cards && ('כרטיס ')}
                      {account.creditCardNum === 1 && cards && cards.find((card) => card.companyAccountId === account.companyAccountId) && (
                        cards.find((card) => card.companyAccountId === account.companyAccountId).creditCardNo
                      )}
                    </Text>
                  </TouchableOpacity>
                ) : null}
