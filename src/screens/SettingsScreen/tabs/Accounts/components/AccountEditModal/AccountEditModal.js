...
export default class AccountEditModal extends PureComponent {
  constructor (props) {
    ...
    handleToggleConfirmRemoveModal = () => {
      this.setState({ confirmRemoveModalIsOpen: !this.state.confirmRemoveModalIsOpen })
    };

    handleTogglePrimaryAccount = () => this.setState({ isPrimaryAccount: !this.state.isPrimaryAccount });
    ...
    handleApplyChanges = () => {
      ...
      return accountNickname.length ? onUpdate({ accountNickname, isPrimary: isPrimaryAccount }) : null
    };
