...
import { combineStyles as cs, getBankTransIcon, getEmoji, sp } from '../../../../utils/func'
...
export default class PaymentsTab extends PureComponent {
  ...
  get companiesLite () {
    ...
    if (companies && companies.length && billingAccounts && billingAccounts.length) {
      const companiesLite = companies.filter(c => c.biziboxType === 'regular')
      if (companiesLite.length) {
        const idAllCompanies = billingAccounts[idx].companies.map((id) => id.companyId)
        return idAllCompanies.every((id) => companiesLite.filter((ids) => ids.companyId === id).length)
      }
    }
    return false
  }
  ...
  _onRefresh = (isRefresh) => {
    this.setState({ refreshing: (!(isRefresh !== undefined && isRefresh === false)) })
    this.getUserBillingAccounts()
  }
  ...
  handleUpdateFieldValidAsyncPop = (e) => {
    ...
    if (val && re.test(val) && val.length > 0) {
      ...
      this.setState({ 'mailValid': false, mailIsHebrew: isHebrew })
    }
  }
  ...
  handleUpdateFieldValidPop = name => val => {
    ...
    this.setState({ [name]: mailValid, mailIsHebrew: isHebrew })
  }
  ...
  handleUpdateFieldValid = (name, isKey) => val => {
    ...
    if (name === 'contactMailValid') {
      const re = /\S+@\S+\.\S+/
      const isHebrew = !!(value && value.length > 0 && /[\u0590-\u05FF]/.test(value))
      const contactMailValid = !!(value && value.length > 0 && re.test(value))
      this.setState({ [name]: contactMailValid, mailIsHebrew: isHebrew })
    } else if (name === 'contactPhoneValid') {
      this.setState({
        billingAccountPhoneSide: 'right',
        [name]: value && (value.length === 10 || value.length === 11) && new RegExp('(050|052|053|054|055|057|058|02|03|04|08|09|072|073|076|077|078)-?\\d{7,7}').test(value),
      })
    } else if (name === 'companyNameValid') {
      this.setState({ [name]: value && (value.length !== 0 && value.length < 40) })
      ...
      if (c.value) {
        const digits = Array.from(String(c.value).replace(/\D/g, '').padStart(9, '0'))
          .map(ch => +ch)
      ...
      this.setState({ [name]: value && (value.length === 9 && result), companyIdSide: 'right' })
    } else {
      this.setState({ [name]: value && (value.length !== 0 && value.length < 30) })
    }
    if (!isKey) {
  ...
  handleUpdateFieldValidAsync = (e) => {
    ...
    const { getBillingAccountDetails } = this.state
    const re = /\S+@\S+\.\S+/
    let val = (IS_IOS ? e.nativeEvent.text : getBillingAccountDetails['billingAccountEmail']) || ''
    if (val && re.test(val) && val.length > 0) {
      setTimeout(() => this.handleUpdateCompany(), 150)
    } else {
      const isHebrew = (val && /[\u0590-\u05FF]/.test(val))
      this.setState({ 'contactMailValid': false, mailIsHebrew: isHebrew })
    }
  }
  sendInvoiceMail = () => {
    ...
    return sendInvoiceMailApi.post({
      ...
      .then(() => {
        this.setState({ inProgress: false, emailAddressParamas: {}, openModalEmail: false })
      })
      .catch(() => {
  ...
  handleUpdateCompany = (isRefresh) => {
    ...
    if (inProgress || !(
      ...
      getBillingAccountDetails.billingAccountHp && String(getBillingAccountDetails.billingAccountHp).length > 0 && getBillingAccountDetails.billingAccountEmail && getBillingAccountDetails.billingAccountEmail.length > 0 && getBillingAccountDetails.billingAccountCompanyName && getBillingAccountDetails.billingAccountCompanyName.length > 0
    )) {
      return
    }
  ...
  filterCity = () => {
    ...
    if (this.state.currentCity && this.state.currentCity.length > 0) {
      citiesArr = citiesArr.filter((item) => item.cityName.includes(this.state.currentCity))
    }
    citiesArr = citiesArr.reduce((memo, trans, currentIndex) => {
      const title = trans.cityName.substring(0, 1)
      const oldIndex = memo.findIndex(item => item.title === title)
  ...
  setCity = (city) => () => {
    ...
    this.setState({ getBillingAccountDetails, modalCitiesShow: false })
  ...
  endBiziboxService = () => {
    ...
    const param = {
      'companyIds': billingAccounts[idx].companies.map((id) => id.companyId),
      'reason': listReasons.filter((it) => it.checked).map(item => item.text).join(),
      'text': text,
    }
      ...
      <SafeAreaView style={[{
        ...
        {showListOfCompanies && (
          <View style={{
            ...
            {billingAccounts && billingAccounts.length > 0 && billingAccounts[idx].companies.length > 1 && (
              <ScrollView
                  ...
                  <View>
                    <View style={[commonStyles.row, {
                      ...
                      <TextInput
                        ...
                        onEndEditing={(e) => {
                          this.setState({
                            text: e.nativeEvent.text.toString().replace(getEmoji(), ''),
                          })
                        }}
                        value={text}
                      />
            ...
            <Fragment>
                ...
                <View style={{
                  ...
                  {billingAccounts && billingAccounts.length > 0 && (
                    <View style={{
                      ...
                      {billingAccounts[idx].billingAccountId === '00000000-0000-0000-0000-000000000000' && (
                        <Image
                          resizeMode='contain'
                          style={{
                            width: 24,
                      ...
                      {billingAccounts[idx].billingAccountId !== '00000000-0000-0000-0000-000000000000' && billingAccounts[idx].paymentTypeId === 3 && (
                        <Fragment>
                          {(billingAccounts[idx].extspMutag24 === 0) && (<Image
                            ...
                            source={require('BiziboxUI/assets/creditIc.png')} />)}
                          ...
                          {(billingAccounts[idx].extspMutag24 === 1) && (<Image
                            ...
                            source={require('BiziboxUI/assets/billing1.png')} />)}
                          ...
                          {(billingAccounts[idx].extspMutag24 === 2) && (<Image
                            ...
                            source={require('BiziboxUI/assets/billing2.png')} />)}
                          ...
                          {(billingAccounts[idx].extspMutag24 === 4) && (<Image
                            ...
                            source={require('BiziboxUI/assets/billing4.png')} />)}
                          ...
                          {(billingAccounts[idx].extspMutag24 === 5) && (<Image
                            ...
                            source={require('BiziboxUI/assets/billing5.png')} />)}
                          ...
                          {(billingAccounts[idx].extspMutag24 === 6) && (<Image
                            ...
                            source={require('BiziboxUI/assets/billing6.png')} />)}
                          ...
                          {(billingAccounts[idx].extspMutag24 === 7) && (<Image
                            ...
                            source={require('BiziboxUI/assets/billing7.png')} />)}
                          ...
                          {(billingAccounts[idx].extspMutag24 === 8) && (<Image
                            ...
                            source={require('BiziboxUI/assets/billing8.png')} />)}
                        </Fragment>
                      )}
                      {billingAccounts[idx].billingAccountId !== '00000000-0000-0000-0000-000000000000' && billingAccounts[idx].paymentTypeId !== 3 && (
                        <CustomIcon
                          style={{ marginHorizontal: 10 }}
                          name={getBankTransIcon(this.getIconName(billingAccounts[idx].paymentTypeId))}
                          size={20}
                          color={'#022258'} />
                      )}
                    </View>)}
                  <View>
                    {billingAccounts && billingAccounts.length > 0 && billingAccounts[idx].billingAccountId === '00000000-0000-0000-0000-000000000000' && (
                      <Text style={{
                        textAlign: 'center',
                        color: '#022258',
                        fontSize: sp(18),
                    ...
                    {billingAccounts && billingAccounts.length > 0 && billingAccounts[idx].billingAccountId !== '00000000-0000-0000-0000-000000000000' && (
                      <Text style={{
                        ...
                        lineHeight: 20,
                        fontFamily: fonts.semiBold,
                      }}>{billingAccounts[idx].paymentTypeId === 3 ? (t(`billingCreditCard:${billingAccounts[idx].extspMutag24}`) + ' ' + billingAccounts[idx].extspCardnumber5) : t(`billingPayment:${billingAccounts[idx].paymentTypeId}`)}</Text>
                    )}
                    ...
                    {billingAccounts && billingAccounts.length > 0 && billingAccounts[idx].companies.length > 1 && (
                      <TouchableOpacity
                        activeOpacity={(billingAccounts[idx].companies.length === 2) ? 1 : 0.2}
                        onPress={billingAccounts[idx].companies.length === 2 ? null : this.openShowListOfCompanies}>
                        <Text style={{
                          textAlign: 'center',
                          color: '#2aa1d9',
                          fontSize: sp(16),
                          lineHeight: 16,
                          fontFamily: fonts.semiBold,
                        }}>{billingAccounts[idx].companies.length === 2 ? `ועוד חברה אחת ` : `ועוד ${billingAccounts[idx].companies.length} חברות`}</Text>
                      </TouchableOpacity>
                    )}
                  </View>
              ...
              <View style={{
                ...
                <Text style={{
                  ...
                  fontSize: sp(18),
                  fontFamily: fonts.regular,
                }}>{getBillingAccountDetails.nextPaymentDate ? AppTimezone.moment(getBillingAccountDetails.nextPaymentDate).format('DD/MM/YY') : ''}</Text>
                <Text style={{
                  flex: 0.9,
                  textAlign: 'left',
                  color: '#022258',
              ...
              {getBillingAccountDetails && getBillingAccountDetails.billingAccountId === '00000000-0000-0000-0000-000000000000' && (
                <View style={{
                  paddingHorizontal: 20,
                }}>
                  ...
                  <TouchableOpacity
                    activeOpacity={(getBillingAccountDetails.billingAccountCompanyName !== '' && getBillingAccountDetails.billingAccountCompanyName !== null &&
                      getBillingAccountDetails.billingAccountHp !== '' && getBillingAccountDetails.billingAccountHp !== null &&
                      getBillingAccountDetails.billingAccountEmail !== '' && getBillingAccountDetails.billingAccountEmail !== null) ? 0.2 : 1}
                    onPress={(getBillingAccountDetails.billingAccountCompanyName !== '' && getBillingAccountDetails.billingAccountCompanyName !== null &&
                      getBillingAccountDetails.billingAccountHp !== '' && getBillingAccountDetails.billingAccountHp !== null &&
                      getBillingAccountDetails.billingAccountEmail !== '' && getBillingAccountDetails.billingAccountEmail !== null) ? this.getCardcomClient : null}
                    style={{
                      opacity: (getBillingAccountDetails.billingAccountCompanyName !== '' && getBillingAccountDetails.billingAccountCompanyName !== null &&
                        getBillingAccountDetails.billingAccountHp !== '' && getBillingAccountDetails.billingAccountHp !== null &&
                        getBillingAccountDetails.billingAccountEmail !== '' && getBillingAccountDetails.billingAccountEmail !== null) ? 1 : 0.3,
                      flexDirection: 'row',
                      alignItems: 'center',
                      alignSelf: 'center',
                      alignContent: 'center',
              ...
              {getBillingAccountDetails && getBillingAccountDetails.billingAccountId !== '00000000-0000-0000-0000-000000000000' && (
                <View style={{
                    ...
                    <Fragment>
                      <View style={{
                        ...
                        <Text style={{
                          ...
                          fontSize: sp(15),
                          fontFamily: fonts.regular,
                        }}>{t(`billingPayment:${getBillingAccountDetails.paymentTypeId}`)}</Text>
                      </View>
                      <View style={{
                        borderBottomColor: '#eaeaea',
                        borderBottomWidth: 1,
                      ...
                      <View style={{
                        ...
                        <Text style={{
                          ...
                          fontSize: sp(15),
                          fontFamily: fonts.regular,
                        }}>{t(`billingCreditCard:${getBillingAccountDetails.extspMutag24}`)}</Text>
                      </View>
                      <View style={{
                        borderBottomColor: '#eaeaea',
                        borderBottomWidth: 1,
                      ...
                      <View style={{
                        ...
                        {getBillingAccountDetails.cardvaliditymonth && getBillingAccountDetails.cardvalidityyear && (
                          <Text style={{
                            color: '#d0cece',
                            fontSize: sp(15),
                            fontFamily: fonts.regular,
                  ...
                  {getBillingAccountDetails.paymentTypeId !== 3 && getBillingAccountDetails.operationresponse === 0 && (
                    <Fragment>
                      <View style={{
                        flexDirection: 'row-reverse',
                        justifyContent: 'space-between',
              ...
              <View style={{
                ...
                <View
                  ...
                  <View style={[{
                    ...
                    <TextInput
                      ...
                      onChangeText={this.handleUpdateField('billingAccountCompanyName')}
                  ...
                  <View style={[{
                    ...
                    <TextInput
                      ...
                      style={[{
                        ...
                        height: 42,
                        fontSize: sp(15),
                        textAlign: (!getBillingAccountDetails.billingAccountHp || (getBillingAccountDetails.billingAccountHp && String(getBillingAccountDetails.billingAccountHp).length === 0)) ? 'right' : companyIdSide,
                        width: '100%',
                      }, commonStyles.regularFont]}
                      onEndEditing={(e) => {
                        let currentCompanyState = Object.assign({}, this.state.getBillingAccountDetails)
                        currentCompanyState['billingAccountHp'] = e.nativeEvent.text.toString().replace(/[^\d]/g, '')
                        this.setState({ getBillingAccountDetails: currentCompanyState })
                      ...
                      onSubmitEditing={this.handleUpdateCompany}
                      value={getBillingAccountDetails.billingAccountHp ? String(getBillingAccountDetails.billingAccountHp) : null}
                    />
                  </View>
                  ...
                  <View style={{
                    ...
                    <TouchableOpacity
                      ...
                      <Text
                        ...
                        ellipsizeMode='tail'
                      >
                        {cities && cities.find((item) => item.cityId === getBillingAccountDetails.billingAccountCityId) !== undefined ? cities.find((item) => item.cityId === getBillingAccountDetails.billingAccountCityId).cityName : ''}
                      </Text>
                    </TouchableOpacity>
                  </View>
                </View>
                ...
                <View
                  ...
                  <View style={[{
                    ...
                    <TextInput
                      ...
                      onSubmitEditing={this.handleUpdateCompany}
                      onChangeText={this.handleUpdateField('billingAccountAddress')}
                ...
                <View
                  ...
                  <View style={[{
                    ...
                    <TextInput
                      ...
                      onBlur={this.handleUpdateCompany}
                      onSubmitEditing={this.handleUpdateCompany}
                      onChangeText={this.handleUpdateField('billingAccountName')}
                  ...
                  <View style={[{
                    ...
                    <TextInput
                      ...
                      style={[{
                        ...
                        height: 42,
                        fontSize: sp(15),
                        textAlign: (!getBillingAccountDetails.billingAccountEmail || (getBillingAccountDetails.billingAccountEmail && getBillingAccountDetails.billingAccountEmail.length === 0)) ? 'right' : emailSide,
                        width: '100%',
                      }, commonStyles.regularFont]}
                      autoCorrect={false}
                      autoCapitalize='none'
                      ...
                      onSubmitEditing={this.handleUpdateCompany}
                      onChangeText={this.handleUpdateField('billingAccountEmail')}
                  ...
                  <View style={[{
                    ...
                    <TextInput
                      ...
                      style={[{
                        ...
                        height: 42,
                        fontSize: sp(15),
                        textAlign: (!getBillingAccountDetails.billingAccountPhone || (getBillingAccountDetails.billingAccountPhone && getBillingAccountDetails.billingAccountPhone.length === 0)) ? 'right' : billingAccountPhoneSide,
                        width: '100%',
                      }, commonStyles.regularFont]}
                      onEndEditing={(e) => {
                        let currentCompanyState = Object.assign({}, this.state.getBillingAccountDetails)
                        currentCompanyState['billingAccountPhone'] = e.nativeEvent.text.toString().replace(/[^\d-]/g, '')
                        this.setState({ getBillingAccountDetails: currentCompanyState })
                      ...
                      onChangeText={this.handleUpdateField('billingAccountPhone')}
                      onSubmitEditing={this.handleUpdateCompany}
                ...
                {getBillingAccountDetails && (getBillingAccountDetails.billingAccountId === '00000000-0000-0000-0000-000000000000' || getBillingAccountDetails.leloMaam === true) && (
                  <View style={{
                    height: 30,
                    alignSelf: 'flex-end',
                    alignItems: 'flex-end',
                    ...
                    <CheckBox
                      disabled={getBillingAccountDetails.billingAccountId !== '00000000-0000-0000-0000-000000000000'}
                      containerStyle={{
                        backgroundColor: 'transparent',
                        left: 0,
                        margin: 0,
                ...
                {companiesLite && (!getBillingAccountDetails || (getBillingAccountDetails.billingAccountId && getBillingAccountDetails.billingAccountId !== '00000000-0000-0000-0000-000000000000' && getBillingAccountDetails.leloMaam !== true)) && (
                  <View style={{
                    height: 30,
                    alignSelf: 'flex-start',
                    alignItems: 'flex-start',
            ...
            <View style={{
              ...
              <View style={{
                ...
                <View style={cs(
                  ...
                  <View>
                    <TouchableOpacity onPress={this.closeModalChangeBillingAccounts}>
                      <View style={{
                        marginRight: 'auto',
                      }}>
                    ...
                    {billingAccounts.map((item, i) => (
                      <TouchableOpacity
                        ...
                        <View style={{
                          ...
                          <View>
                            {item.billingAccountId === '00000000-0000-0000-0000-000000000000' && (
                              <Image
                                resizeMode='contain'
                                style={{
                                  width: 24,
                            ...
                            {item.billingAccountId !== '00000000-0000-0000-0000-000000000000' && item.paymentTypeId === 3 && (
                              <Fragment>
                            ...
                            {item.billingAccountId !== '00000000-0000-0000-0000-000000000000' && item.paymentTypeId !== 3 && (
                              <CustomIcon
                                style={{ marginHorizontal: 10 }}
                                name={getBankTransIcon(this.getIconName(item.paymentTypeId))}
                                size={20}
                                color={'#022258'} />
                            )}
                          </View>
                          <View>
                            ...
                            {item.billingAccountId !== '00000000-0000-0000-0000-000000000000' && (
                              <Text style={{
                                ...
                                fontSize: sp(14),
                                fontFamily: fonts.semiBold,
                              }}>{item.paymentTypeId === 3 ? (t(`billingCreditCard:${item.extspMutag24}`) + ' ' + item.extspCardnumber5) : t(`billingPayment:${item.paymentTypeId}`)}</Text>
                            )}
                            ...
                            <View style={{
                              ...
                              {item.companies.length > 1 && (
                                <Text style={{
                                  ...
                                  fontSize: sp(14),
                                  fontFamily: fonts.semiBold,
                                }}>{' '}{item.companies.length === 2 ? `ועוד חברה אחת ` : `ועוד ${item.companies.length} חברות`}</Text>
                              )}
                            </View>
                          </View>
                        </View>
                      ...
                      style={[
                        {
                          marginBottom: 5,
                          width: 260,
                          borderBottomColor: (!mailValid) ? colors.red2 : '#022258',
                          borderBottomWidth: 2,
                          fontFamily: fonts.semiBold,
                          fontWeight: 'normal',
                          color: '#022258',
                          textAlign: (!emailAddressParamas.emailAddress || (emailAddressParamas.emailAddress && emailAddressParamas.emailAddress.length === 0)) ? 'right' : 'left',
                          height: 30,
                          fontSize: sp(15),
                          lineHeight: 20,
                          backgroundColor: 'transparent',
                      ...
                      <View
                        ...
                        <View style={{
                          ...
                          <View style={{
                            ...
                            <TouchableOpacity onPress={this.downloadInvoice(item)}
                              style={{
                                flexDirection: 'row-reverse',
                                justifyContent: 'flex-start',
                                alignItems: 'flex-start',
                              ...
                              <Text style={{
                                ...
                                fontSize: sp(16),
                                fontFamily: fonts.bold,
                              }}>{AppTimezone.moment(item.paymentDate).format('DD/MM/YY')}</Text>
                              <Text style={{
                                textAlign: 'right',
                                color: '#2aa1d9',
                                fontSize: sp(16),
                        ...
                        <View style={{
                          ...
                          <TouchableOpacity onPress={this.openSendEmail(item)}>
                            <Image
                              style={[styles.imgIcon, { width: 18.5, height: 13.5 }]}
