...
export default class SlikaDetailsWrapper extends PureComponent {
  get slikaCards () {
    ...
    getAccounts = (slika) => {
      const accounts = this.props.accounts.find(a => a.companyAccountId === slika.companyAccountId)
      return accounts || {}
    };
    ...
    render () {
      ...
      if (isEmpty(slikaCards)) {
        return (
          <Text style={[commonStyles.textCenter, { marginVertical: 10 }]}>Slika not found</Text>
        )
      }
