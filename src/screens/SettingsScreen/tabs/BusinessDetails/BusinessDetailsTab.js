...
import { getCitiesApi, getUserSettingsApi, isEmailExists, updateCompanyApi, updateLeadInfo } from '../../../../api'
...
export default class BusinessDetailsTab extends PureComponent {
  ...
  handleUpdateField = name => val => {
    ...
    if (name === 'contactMail' || name === 'contactPhone' || name === 'companyName' || name === 'mainContactName') {
  ...
  handleUpdateFieldValidAsync = (e) => {
    ...
    if (val && re.test(val) && val.length > 0) {
      ...
      this.setState({ 'contactMailValid': false, mailIsHebrew: isHebrew, emailExists: false })
    }
  }
  ...
  handleUpdateCompany = () => {
    ...
    if (inProgress || !(
      contactMailValid && contactPhoneValid && companyNameValid && mainContactNameValid && !emailExists &&
      currentCompany.contactMail.length > 0 && currentCompany.contactPhone.length > 0 && currentCompany.companyName.length > 0 && currentCompany.mainContactName.length > 0
    )) {
      return
    }
    Keyboard.dismiss()
    ...
    updateCompanyApi.post({
      ...
      .then((list) => {
        return this.props.dispatch(selectCompany(currentCompany.companyId))
          .then(() => {
            const newCompany = list.filter((item) => item.companyId === currentCompany.companyId)
  ...
  setCity = (city) => () => {
    ...
    this.setState({ currentCompany: currentCompanyState, modalCitiesShow: false })
  ...
  handleUpdateFieldValidAdd = name => val => {
    ...
    if (name === 'companyNameValidAdd') {
      ...
      if (value && value.length === 9 && result) {
        const { currentCompany } = this.state
        new Api({ endpoint: 'companies/hp-exists', secure: currentCompany.contactMail === null }).post({
          body: {
            companyHp: value,
  ...
  handleAddCompany = () => {
    ...
    updateLeadInfo.post({
      ...
      return dispatch(signupCreate({
        ...
        .then((list) => {
          const companyIdNew = list.find((com) => saveCompanies.every(save => save.companyId !== com.companyId)).companyId
          return dispatch(selectCompany(companyIdNew))
            .then(() => {
              const newCompany = list.filter((item) => item.companyId === companyIdNew)
              this.props.handleSetCompany(newCompany[0])
              setTimeout(() => {
                ...
                <KeyboardAwareScrollView>
                  ...
                  <View
                    ...
                    <View style={[{
                      ...
                      <TextInput
                        maxLength={40}
                        onEndEditing={this.handleUpdateFieldValidAdd('companyNameValidAdd')}
                        onBlur={this.handleUpdateFieldValidAdd('companyNameValidAdd')}
                        autoCorrect={false}
                        autoCapitalize='sentences'
                        returnKeyType='done'
                        keyboardType='default'
                  ...
                  <View
                    ...
                    <View style={[{
                      ...
                      <TextInput
                        ...
                        onBlur={this.handleUpdateFieldValidAdd('companyHpValid')}
                  ...
                  <View
                    ...
                    <View style={[{
                      ...
                      <TextInput
                        ...
                        onSubmitEditing={this.handleAddCompany}
                        onChangeText={this.handleUpdateFieldAdd('businessCategory')}
                  ...
                  <TouchableOpacity
                    ...
                    <Text
                      style={[styles.btnMatch, { fontFamily: fonts.semiBold }]}>{'קישור חשבון בנק'}</Text>
                  </TouchableOpacity>
        ...
        <KeyboardAwareScrollView style={{
            ...
            <View style={{
              ...
              <TouchableOpacity
                ...
                <Text
                  ...
                  ellipsizeMode='tail'
                >
                  {cities && cities.find((item) => item.cityId === currentCompany['cityId']) !== undefined ? cities.find((item) => item.cityId === currentCompany['cityId']).cityName : ''}
                </Text>
              </TouchableOpacity>
            </View>
          </View>
          ...
          <View
            ...
            <View style={[{
              ...
              <TextInput
                editable
                onEndEditing={this.handleUpdateFieldValid('mainContactNameValid')}
          ...
          <View
            style={[cs(isRtl, commonStyles.row, [commonStyles.rowReverse]), {
              height: 42,
              marginBottom: (emailExists || mailIsHebrew || !contactMailValid) ? 0 : 8,
            }]}>
            <View style={{ flex: 2.5 }}>
              <Text style={{
                color: '#0f3860',
            ...
            <View style={[{
              ...
              <TextInput
                ...
                style={[{
                  ...
                  height: 42,
                  fontSize: sp(15),
                  textAlign: (!currentCompany['contactMail'] || (currentCompany['contactMail'] && currentCompany['contactMail'].length === 0)) ? 'right' : 'left',
                  width: '100%',
                }, commonStyles.regularFont]}
            ...
            <View style={[{
              ...
              <TextInput
                ...
                style={[{
                  ...
                  height: 42,
                  fontSize: sp(15),
                  textAlign: (!currentCompany['contactPhone'] || (currentCompany['contactPhone'] && currentCompany['contactPhone'].length === 0)) ? 'right' : 'left',
                  width: '100%',
                }, commonStyles.regularFont]}
                onEndEditing={(e) => {
                  let currentCompanyState = Object.assign({}, this.state.currentCompany)
                  currentCompanyState['contactPhone'] = e.nativeEvent.text.toString().replace(/[^\d-]/g, '')
                  this.setState({ currentCompany: currentCompanyState })
