...
export default class HelpScreen extends PureComponent {
  ...
  getQuestionsAnswers = () => {
    ...
    const currentCompany = companies && companies.length && companies.find(c => c.companyId === currentCompanyId)
    getQuestionsAnswersApi.post({
      ...
      .then(data => {
        if (!this.state.screen) {
          this.setState({ questionsAnswers: data, questionsAnswersFilter: data, inProgress: false })
        } else {
  ...
  _renderHeader = (section, _, isActive) => {
    return (
      <View style={[styles.header, (isActive ? styles.active : styles.inactiveHedaer), {
        ...
        <Text style={styles.headerText}>{section.question ? section.question.replace(/\n/ig, '') : ''}</Text>
      </View>
    )
  }
  ...
  _onRefresh = () => {
    ...
    this.getQuestionsAnswers()
    setTimeout(() => {
      this.setState({ refreshing: false, inProgress: false })
    }, 1000)
  }

  handleSearch = value => {
    this.setState({ valSearch: value, inProgress: true })
    setTimeout(() => {
      this.handleSendSearch()
    }, 50)
  }
    ...
    if (!screen) {
      if (valSearch && valSearch.length) {
        const filterSearch = questionsAnswers.filter((item) => {
          return (item.question.split(' ').some((key) => valSearch.includes(key)) || item.keywords.some((key) => valSearch.includes(key.keyword)))
        })
        this.setState({
  ...
  handleUpdate = () => {
    ...
    openTicketApi.post({
      ...
      .then(() => {
        this.setState({ inProgress: false, successSend: true })
      })
      .catch(() => {
  ...
  getTerms = () => {
    ...
    getTermsApi.post({
      ...
      .then(data => {
        if (!this.state.screen) {
          this.setState({ terms: data, termsFilter: data, inProgressTerms: false })
        } else {
  ...
  closeModalTerms = () => {
    this.setState({ openModalTerms: false, terms: [], termsFilter: [] })
  }

  handleSearchTerms = value => {
    this.setState({ valTermsSearch: value, inProgressTerms: true })
    setTimeout(() => {
      this.handleSendSearchTerms()
    }, 50)
  }
    ...
    if (!screen) {
      if (valTermsSearch && valTermsSearch.length) {
        const filterSearch = terms.filter((item) => {
          return (item.subject.split(' ').some((key) => valTermsSearch.includes(key)) || item.keywords.some((key) => valTermsSearch.includes(key.keyword)))
        })
        this.setState({
      ...
      if (valTermsSearch && valTermsSearch.length) {
        const filterSearch = terms.filter((item) => {
          return item.screens.some((key) => key.screenName === screen) &&
            (
              item.subject.split(' ').some((key) => valTermsSearch.includes(key)) ||
      ...
      <View style={[{
        ...
        <SafeAreaView style={{
          ...
          <KeyboardAwareScrollView
            ...
            {(screen) && (
              <View style={{
                ...
                {screen === 'fixedMovements' && (
                  <Image
                    style={[{ alignSelf: 'center', resizeMode: 'contain', width: 25 }]}
                    source={require('BiziboxUI/assets/fixedMovements.png')}
                  />
                )}
                {screen === 'overview' && (
                ...
                {screen === 'budget' && (
                  <CustomIcon
                    ...
                    size={25}
                    style={{
                      alignSelf: 'center', resizeMode: 'contain', width: 25,
                    }}
                    color={'#022258'}
                  />
                )}
            ...
            {(!inProgress && questionsAnswersFilter.length === 0 && valSearch.length > 0) && (
              <View>
                <Image
                  style={[{ width: 42, height: 47.5, marginBottom: 5, alignSelf: 'center' }]}
            ...
            {!screen && (questionsAnswersFilter.length === 0 || valSearch.length === 0) && (
              <View>
                <Carousel
                  slideHeight={230}
                  ref={(c) => {
                    ...
                    <View
                      ...
                      <View style={[{
                        ...
                        <TextInput
                          ...
                          onEndEditing={(e) => {
                            this.setState({
                              taskTitle: e.nativeEvent.text.toString().replace(getEmoji(), ''),
                            })
                        ...
                        <TextInput
                          ...
                          style={[
                            styles.input,
                            {
                              ...
                              fontFamily: fonts.regular,
                              fontWeight: 'normal',
                              textAlign: (!closeMailToSend || (closeMailToSend && closeMailToSend.length === 0)) ? 'right' : 'left',
                              backgroundColor: 'transparent',
                            },
                          ]}
                          onSubmitEditing={this.handleUpdate}
                          onChangeText={this.handleUpdateFields('closeMailToSend')}
                    ...
                    <View
                      ...
                      <View style={[{
                        ...
                        <TextInput
                          ...
                          onChangeText={this.handleUpdateFields('taskOpenerName')}
                          onSubmitEditing={this.handleUpdate}
                          onEndEditing={(e) => {
                            this.setState({
                              taskOpenerName: e.nativeEvent.text.toString().replace(getEmoji(), ''),
                            })
                            this.handleUpdateFieldValid('taskOpenerNameValid')(e)
                          }}
                    ...
                    <View
                      ...
                      <View style={[{
                        ...
                        <TextInput
                          ...
                          underlineColorAndroid='transparent'
                          style={[{
                            textAlign: (!userCellPhone || (userCellPhone && userCellPhone.length === 0)) ? 'right' : 'left',
                            color: '#0f3860',
                            height: 42,
                            fontSize: sp(15),
                            width: '100%',
                          ...
                          onEndEditing={(e) => {
                            this.setState({
                              userCellPhone: e.nativeEvent.text.toString().replace(/[^\d-]/g, ''),
                            })
                            this.handleUpdateFieldValid('cellValid')(e)
                          }}
                          onBlur={this.handleUpdateFieldValid('cellValid')}
                          onChangeText={this.handleUpdateFields('userCellPhone')}
                          onSubmitEditing={this.handleUpdate}
                          value={userCellPhone}
                        />
                      </View>
                    ...
                    <View>
                      ...
                      <View style={[commonStyles.row, {
                        ...
                        <TextInput
                          ...
                          onEndEditing={(e) => {
                            this.setState({
                              taskDesc: e.nativeEvent.text.toString().replace(getEmoji(), ''),
                            })
          ...
          <Modal
            ...
            <SafeAreaView style={{
              ...
              <KeyboardAwareScrollView
                ...
                {(termsFilter && termsFilter.length > 0 && termsFilter.map(s => {
                ...
                {(!inProgressTerms && termsFilter.length === 0 && valTermsSearch.length > 0) && (
                  <View style={{
                    marginTop: 20,
                  }}>
                    <Image
