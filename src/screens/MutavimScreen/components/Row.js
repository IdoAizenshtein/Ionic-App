...
export default class Row extends PureComponent {
  sectionString (highlight, origStr, caseSensitive) {
    ...
    render () {
      ...
      const accountMutavName = item.accountMutavName ? item.accountMutavName.trim() : item.accountMutavName
      ...
      const numberStyle = cs(item.averageThreeMonths.toString().includes('-'), [styles.dataValue, { color: colors.green4 }], { color: colors.red2 })
        ...
        <TouchableOpacity style={styles.dataRowAnimatedWrapper} onPress={onToggle}>
          <View style={wrapperStyles}>
            <View style={[cs(isRtl, styles.dataValueDescWrapperLevel2, commonStyles.rowReverse)]}>
              <Text
                style={[cs(isOpen, [styles.dataValue, commonStyles.regularFont, { flex: 0 }], commonStyles.boldFont)]}
            ...
            <Text style={[styles.dataValueWrapper, styles.dataValueWrapperLevel2]}>
              <Text style={numberStyle}>{item.hova ? '-' : ''}{this.renderTextYellow(total[0], true)}</Text>
