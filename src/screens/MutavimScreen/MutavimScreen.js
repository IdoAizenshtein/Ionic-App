...
class ScrollCompWithHeader extends PureComponent {
  ...
  componentWillReceiveProps (props) {
    if (props.refreshScroll && props.refreshScroll !== this.props.refreshScroll) {
      setTimeout(() => {
        ...
        <View
          ...
          <View style={commonStyles.horizontalCenterContainer}>
            ...
            <Button
              ...
              title={selectedAccounts.length > 0 ? t('common:changeFilter') : t('bankAccount:selectAccount')}
            />
          </View>
        </View>
      )}
...
export default class MutavimScreen extends PureComponent {
  ...
  handleCloseCategoriesModal = () => {
    this.setState({ categoriesModalIsOpen: false, idxCategory: 0 })
  }
      ...
      getAccountCflTransTypeApi.post({ body: { uuid: currentCompanyId } })
        .then(data => {
          ...
          if (selectedAccountIds) {
            ...
            this.setState({
              navigationState: this.props.navigation.state.params,
              isReady: true,
              queryStatus: {
                expense: null,
                query: this.props.navigation.state.params.filterText ? this.props.navigation.state.params.filterText : null,
              },
              selectedAccountIds,
              ...
              setTimeout(() => {
                ...
                if (this.props.navigation.state.params.filterText) {
                  delete this.props.navigation.state.params.filterText
                }
                return this.companyMutavDetails()
              }, 200)
            })
    ...
    companyMutavDetailsApi.post({
      ...
      .then((mutavDetailsArr) => {
        ...
        if (mutavDetailsArr && mutavDetailsArr.length) {
          categoriesMatch = Array.from(new Set(mutavDetailsArr.map(s => s.transTypeId))).map(id => {
          ...
          mutavDetailsArr.forEach((trns1, idx1) => {
            let transType = this.state.categories.find(ctt => ctt.transTypeId === trns1.transTypeId)
  ...
  getDetails = () => {
    ...
    return mutavHistoryApi.post({
      ...
      .then(detailsHistory => {
        this.setState({ detailsHistory, inProgressDetails: false })
      })
  ...
  filtersAll = (changeTypes, status) => {
    ...
    setTimeout(() => {
              ...
              mutavDetails = mutavDetails.filter((item, idx) => {
                ...
                if (queryStatus.query !== null && queryStatus.query !== '') {
                  if (queryStatus.expense !== null) {
                    return (
                      (item.averageThreeMonths.toString().includes('-') === queryStatus.expense) &&
                      (categoriesMatchPressFilter.length ? categoriesMatchPress : !categoriesMatchPress) &&
                      (
                        (item.accountMutavName && item.accountMutavName.toString().toLowerCase().includes(queryStatus.query.toLowerCase())) ||
                        (item.averageThreeMonths && item.averageThreeMonths.toString().includes(queryStatus.query))
                      )
                    )
                  } else {
                    return (
      ...
      if (this.state.bankTransIdOpened) {
        const lastRowOpened = mutavDetails.find((item) => item.biziboxMutavId === this.state.bankTransIdOpened)
        if (lastRowOpened) {
  ...
  renderItem = ({ item, index }) => {
    ...
    return (
      <Row
        queryStatus={queryStatus}
        key={index}
        isOpen={(queryStatus && queryStatus.query !== null && queryStatus.query !== '') ? true : (item.biziboxMutavId) === currentOpenItemIndex}
        isRtl={isRtl}
        item={item}
  ...
  handleUpdateTransText = () => {
    ...
    if (inProgress || (this.state[`${type}Valid`] !== true && copyMainDesc.length !== 0) || mailIsHebrew) {
      return
    }
    ...
    this.setState({ dataRow: valuesSave, editDescModal: false, inProgress: true })
    const obj = {
  ...
  handleUpdateFieldValid = (name, isNotBlur, isRun) => val => {
    if (isRun === undefined || isRun) {
      ...
      if (name === 'contactMailValid') {
        ...
        this.setState({
          [name]: (!value || (value && value.length === 0)) ? true : contactMailValid,
          mailIsHebrew: isHebrew,
        })
      } else if (name === 'contactPhoneValid') {
        this.setState({
          billingAccountPhoneSide: 'right',
          [name]: (!value || (value && value.length === 0)) ? true : !!(value && (value.length === 10 || value.length === 11) && new RegExp('(050|052|053|054|055|057|058|02|03|04|08|09|072|073|076|077|078)-?\\d{7,7}').test(value)),
        })
        ...
        if (c.value) {
          const digits = Array.from(String(c.value).replace(/\D/g, '').padStart(9, '0')).map(ch => +ch)
        ...
        this.setState({
          [name]: (!value || (value && value.length === 0)) ? true : !!(value && (value.length === 9 && result)),
          companyIdSide: 'right',
        })
  ...
  handleUpdateFieldValidAsync = (e) => {
    ...
    if (!(val && re.test(val) && val.length > 0)) {
      ...
      this.setState({ 'contactMailValid': (!val || (val && val.length === 0)), mailIsHebrew: isHebrew })
    }
  }
  ...
  render () {
    ...
    const total = dataRow ? getFormattedValueArray(dataRow.averageThreeMonths) : null
    const numberStyle = dataRow ? cs(dataRow.averageThreeMonths.toString().includes('-'), [{
      fontFamily: fonts.semiBold,
    }, { color: colors.green4 }], { color: colors.red2 }) : {}
    const account = dataRow ? accounts.find(a => a.bankAccountId === dataRow.accountId) : null
      ...
      <Fragment>
        ...
        <Fragment>
          ...
          <AnimatedFlatList
            ...
            renderScrollComponent={(props) => <ScrollCompWithHeader
              ...
              props={props}
              notHasData={((!mutavDetailsArr) || (mutavDetailsArr && mutavDetailsArr.length === 0)) && !this.isLoader}
              ...
              <View style={{
                ...
                <View style={{
                  ...
                  <View style={cs(
                    ...
                    <View style={{
                      alignSelf: 'center',
                      opacity: (this.state[`${type}Valid`] === true || copyMainDesc.length === 0) ? 1 : 0.3,
                    }}>
                      <TouchableOpacity
                        activeOpacity={(this.state[`${type}Valid`] === true || copyMainDesc.length === 0) ? 0.2 : 1}
                        onPress={(this.state[`${type}Valid`] === true || copyMainDesc.length === 0) ? this.handleUpdateTransText : null}>
                        <Text style={{
                          fontSize: sp(16),
                          color: '#022258',
                          fontFamily: fonts.semiBold,
                    ...
                    <View style={[commonStyles.row, {
                      ...
                      {type === 'accountMutavDetails' && (
                        <TextInput
                          ...
                          onEndEditing={this.handleUpdateFieldValid('accountMutavDetailsValid', false, !IS_IOS)}
                          onBlur={this.handleUpdateFieldValid('accountMutavDetailsValid', false, IS_IOS)}
                          onChangeText={this.handleUpdateField('accountMutavDetails')}
                      ...
                      {type === 'contactName' && (
                        <TextInput
                          ...
                          onEndEditing={this.handleUpdateFieldValid('contactNameValid', false, !IS_IOS)}
                          onBlur={this.handleUpdateFieldValid('contactNameValid', false, IS_IOS)}
                      ...
                      {type === 'contactMail' && (
                        <TextInput
                          ...
                          numberOfLines={12}
                          style={[{
                            textAlign: (!copyMainDesc || (copyMainDesc && copyMainDesc.length === 0)) ? 'right' : emailSide,
                            color: '#022258',
                            fontSize: sp(20),
                            width: '100%',
                            height: 300,
                      ...
                      {type === 'contactPhone' && (
                        <TextInput
                          ...
                          underlineColorAndroid='transparent'
                          style={[{
                            textAlign: (!copyMainDesc || (copyMainDesc && copyMainDesc.length === 0)) ? 'right' : billingAccountPhoneSide,
                            color: '#022258',
                            fontSize: sp(20),
                            width: '100%',
                            height: 300,
                          ...
                          onEndEditing={(e) => {
                            this.setState({ copyMainDesc: e.nativeEvent.text.toString().replace(/[^\d-]/g, '') })
                      ...
                      {type === 'accountMutavHp' && (
                        <TextInput
                          ...
                          underlineColorAndroid='transparent'
                          style={[{
                            textAlign: (!copyMainDesc || (copyMainDesc && String(copyMainDesc).length === 0)) ? 'right' : companyIdSide,
                            color: '#022258',
                            fontSize: sp(20),
                            width: '100%',
                            height: 300,
                          ...
                          onEndEditing={(e) => {
                            this.setState({ copyMainDesc: e.nativeEvent.text.toString().replace(/[^\d]/g, '') })
                            this.handleUpdateFieldValid('accountMutavHpValid')(e)
                          }}
                          onFocus={this.onFocusInput('companyIdSide')}
                          onBlur={this.handleUpdateFieldValid('accountMutavHpValid')}
        ...
        <View style={[styles.panelContainer]} pointerEvents={'box-none'}>
          ...
          <Interactable.View
            ...
            snapPoints={[{ y: 20 }, { y: Screen.height - 360 }, { y: Screen.height + 30 }]}
            boundaries={{ top: -360 }}
            ...
            <View style={styles.panel}>
              ...
              <View style={{ left: 0, right: 0, height: Screen.height - 70 }}>
                {!categoriesMatchModal && dataRow && (
                  <View>
                    <View style={{
                      ...
                      <View>
                        <Text style={styles.panelTitle}>{dataRow.accountMutavName}</Text>
                      </View>
                ...
                <Animated.ScrollView
                  ...
                  {categoriesMatchModal && !dataRow && categoriesMatchCopy.length > 0 && (
                    <View style={{ paddingHorizontal: 10 }}>
                      {categoriesMatchModal && !dataRow && categoriesMatchCopy.length > 0 && this.divideList(categoriesMatchCopy).map((gr, i) => {
                        return (
                          <View key={i.toString()}
                            style={{
                              height: 77,
                    ...
                    <View>
                      <View style={{
                        ...
                        <TouchableOpacity
                          ...
                          onPress={!disabledEdit ? this.handleOpenCategoriesModal : null}>
                          <View style={{
                            flexDirection: 'row-reverse',
                            alignItems: 'center',
                            alignContent: 'center',
                          ...
                          <View style={{
                            ...
                            {inProgressDetails
                              ...
                              : (<Graph dataGraph={detailsHistory}
                                isOpen={detailsHistory && !inProgressDetails} />)}
                          </View>
                        </View>
                        <View style={{
                          flex: 1,
                      ...
                      <View style={{ paddingHorizontal: 10 }}>
                        <TouchableOpacity
                          ...
                          onPress={this.handleToggleEditDesc('accountMutavDetails')}>
                          <View style={{
                            flexDirection: 'row-reverse',
                            alignItems: 'center',
                            alignContent: 'center',
                            ...
                            <Text style={[{
                              ...
                              textAlign: 'right',
                              fontFamily: fonts.regular,
                              color: dataRow.accountMutavDetails ? '#022258' : '#d0cece',
                            }]}>{dataRow.accountMutavDetails ? dataRow.accountMutavDetails : 'הערה'}</Text>
                          </View>
                        </TouchableOpacity>
                        <View style={{
                          height: 1,
                          ...
                          <View style={{
                            ...
                            <View style={{
                              ...
                              <Image style={{
                                ...
                                width: 19,
                                height: 19,
                              }} source={require('BiziboxUI/assets/accountMutavHp.png')} />
                            </View>
                            <View style={commonStyles.spaceDividerDouble} />
                            <Text style={[{
                              fontSize: sp(16),
                              textAlign: 'right',
                              fontFamily: fonts.regular,
                              color: dataRow.accountMutavHp ? '#022258' : '#d0cece',
                            }]}>{dataRow.accountMutavHp ? dataRow.accountMutavHp : 'ע.מ/ח.פ'}</Text>
                          </View>
                        </TouchableOpacity>
                        <View style={{
                          height: 1,
                        ...
                        <TouchableOpacity
                          ...
                          <View style={{
                            ...
                            <Text style={[{
                              ...
                              textAlign: 'right',
                              fontFamily: fonts.regular,
                              color: dataRow.contactName ? '#022258' : '#d0cece',
                            }]}>{dataRow.contactName ? dataRow.contactName : 'שם איש קשר'}</Text>
                          </View>
                        </TouchableOpacity>
                        <View style={{
                          height: 1,
                          ...
                          <View style={{
                            ...
                            <View style={{
                              ...
                              <Image style={{
                                ...
                                width: 19,
                                height: 19,
                              }} source={require('BiziboxUI/assets/contactMail.png')} />
                            </View>
                            <View style={commonStyles.spaceDividerDouble} />
                            <Text style={[{
                              fontSize: sp(16),
                              textAlign: 'right',
                              fontFamily: fonts.regular,
                              color: dataRow.contactMail ? '#022258' : '#d0cece',
                            }]}>{dataRow.contactMail ? dataRow.contactMail : 'מייל'}</Text>
                          </View>
                        </TouchableOpacity>
                        <View style={{
                          height: 1,
                          ...
                          <View style={{
                            ...
                            <View style={{
                              ...
                              <Image style={{
                                ...
                                width: 19,
                                height: 19,
                              }} source={require('BiziboxUI/assets/contactPhone.png')} />
                            </View>
                            <View style={commonStyles.spaceDividerDouble} />
                            <Text style={[{
                              fontSize: sp(16),
                              textAlign: 'right',
                              fontFamily: fonts.regular,
                              color: dataRow.contactPhone ? '#022258' : '#d0cece',
                            }]}>{dataRow.contactPhone ? dataRow.contactPhone : 'טלפון'}</Text>
                          </View>
                        </TouchableOpacity>
                        <View style={{
                          height: 1,
