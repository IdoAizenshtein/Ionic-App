...
import { cashFlowAggregatedCalculateNigrarot, getCashFlowDetailsDataBySections } from 'src/redux/selectors/cashFlow'
...
import { getDaysBetweenTwoDates, getListOfDatesInterval } from '../../utils/date'
...
export default class CashFlowScreen extends PureComponent {
  constructor (props) {
    super(props)
    const moveToAggregate = props.navigation.getParam('moveToAggregate')
    const messageLink = props.navigation.getParam('messageLink')
    this.state = {
  ...
  get selectedHarigaDatetAccounts () {
    ...
    return this.screenSwitchState
      ? ((cashFlowAggregatedDataState && cashFlowAggregatedDataState.length) ? cashFlowAggregatedDataState.filter(d => d.harigaDate !== null) : []) : ((cashFlowDetailsData && cashFlowDetailsData.harigaArray && cashFlowDetailsData.harigaArray.length) ? cashFlowDetailsData.harigaArray.filter(d => d.harigaDate !== null) : [])
  }

  get selectedHarigaDatetAccountsAll () {
    const { accounts } = this.props
    const arrUuid = this.screenSwitchState ? this.selectedHarigaDatetAccounts.map(a => a.accountUuid) : this.selectedHarigaDatetAccounts.map(a => a.companyAccountId)
  ...
  selectDefaultAccounts = () => {
    ...
    const findIdsToSelect = (currency) => {
      ...
      if (hasUpdatedAccount) {
        ...
        this.setState({
          ...
          accountBalanceChartDataState = accountBalanceChartDataState.map((id) => {
            let creditLimit
            if (id === null) {
              creditLimit = this.props.accounts.filter(a => this.state.selectedAccountIds.includes(a.companyAccountId)).reduce((memo, account) => {
                return memo + account.creditLimit
              }, 0)
            } else {
              creditLimit = this.props.accounts.filter(a => this.state.selectedAccountIds.includes(a.companyAccountId)).find(a => a.companyAccountId === id).creditLimit
            }
      ...
      this.setState({
        dateFromTimestamp: this.selectedAccounts.length > 0 ? AppTimezone.moment(this.selectedAccounts[0].balanceLastUpdatedDate).valueOf() : AppTimezone.moment().valueOf(),
        dateTillTimestamp: this.selectedAccounts.length > 0 ? AppTimezone.moment(this.selectedAccounts[0].balanceLastUpdatedDate).add(30, 'day').valueOf() : AppTimezone.moment().add(1, 'month').valueOf(),
      })
  ...
  getScreenData = (isLoader = true, isSwitchAccount = false, isReload = false) => {
    const { currentSelectedAccountId, categoriesArr } = this.state
    const objParamas = this.props.navigation.getParam('objParamas')
    const messageLink = this.props.navigation.getParam('messageLink')
    ...
    const getScreenDataReq = this.state.screenMode === SCREEN_MODES.details ? this.getDetailsData() : this.getAggregateData()
    ...
    return Promise.all([getScreenDataReq, this.getChartData()])
      .then((data) => {
        ...
        if (this.state.screenMode !== SCREEN_MODES.details) {
          cashFlowAggregatedDataState = data[0].filter(d => (this.selectedAccounts.length > 1 ? (d.accountUuid === null) : (d.accountUuid !== null)))
          if (cashFlowAggregatedDataState.length) {
            ...
            const past = cashFlowAggregatedDataState[0].accountTransactions.filter((item) => item.transDate !== null && item.transDate < balanceLastUpdatedDate)
            if (past.length) {
              cashFlowAggregatedDataStateArr.push({ title: 'תנועות עבר', data: past })
            }
            const nigreret = cashFlowAggregatedDataState[0].accountTransactions.filter((item) => item.transDate === null)
            if (nigreret.length) {
              cashFlowAggregatedDataStateArr.push({ title: 'תנועות נגררות', data: nigreret })
            }
            const future = cashFlowAggregatedDataState[0].accountTransactions.filter((item) => item.transDate !== null && item.transDate >= balanceLastUpdatedDate)
            if (future.length) {
              cashFlowAggregatedDataStateArr.push({ title: 'תנועות עתידיות (צפויות)', data: future })
            }
          }
        }
        this.headerChartX = new Animated.ValueXY()
        ...
        if (this.state.screenMode === SCREEN_MODES.details) {
          if (data && data[0] && data[0].cashFlowDetails) {
            data[0].cashFlowDetails.forEach((trns, idx) => {
              trns.data.forEach((trns1, idx1) => {
                trns1.transType = this.state.categories.find(ctt => ctt.transTypeId === trns1.transTypeId)
          ...
          if (this.selectedAccounts.length > 0) {
            const balanceLastUpdatedDate = AppTimezone.moment(this.selectedAccounts[0].balanceLastUpdatedDate).format('DD/MM/YY')
            if (balanceLastUpdatedDate) {
              if (data[0].cashFlowDetails) {
                const balanceLastUpdatedDateIdx = data[0].cashFlowDetails.findIndex((item) => item.title === balanceLastUpdatedDate)
                const balanceLastUpdatedDateArr = data[0].cashFlowDetails.filter((item) => item.title === balanceLastUpdatedDate)
                if (balanceLastUpdatedDateArr.length > 0) {
                  const nigreretFalse = balanceLastUpdatedDateArr[0].data.filter((item) => item.nigreret === false)
                  const nigreretTrue = balanceLastUpdatedDateArr[0].data.filter((item) => item.nigreret === true)
                  if (nigreretTrue.length > 0 && nigreretFalse.length > 0) {
                    ...
                    if (nigreretFalse.length > 0) {
                      data[0].cashFlowDetails.splice(balanceLastUpdatedDateIdx + 1, 0, {
                        data: nigreretFalse,
                        first: false,
          ...
          if (messageLink) {
            delete this.props.navigation.state.params.messageLink
          }
      ...
      .catch((err) => {
        setTimeout(() => {
          ...
          this.headerChartX = new Animated.ValueXY()
          if (err.path && (err.path.includes('ballance-chart') || err.path.includes('balance-chart'))) {
            // this.setState({
            //   isLayoutComplete: true,
            //   cashFlowAggregatedDataStateArr: null,
            ...
            let cashFlowAggregatedDataState = null
            let cashFlowAggregatedDataStateArr = null
            if (this.state.screenMode !== SCREEN_MODES.details && cashFlowAggregatedData) {
              cashFlowAggregatedDataState = cashFlowAggregatedData.filter(d => d.accountUuid !== null)
            ...
            if (this.state.screenMode === SCREEN_MODES.details) {
              if (cashFlowDetailsData && cashFlowDetailsData.cashFlowDetails) {
                cashFlowDetailsData.cashFlowDetails.forEach((trns, idx) => {
                  trns.data.forEach((trns1, idx1) => {
                    cashFlowDetailsData.cashFlowDetails[idx].data[idx1].transType = this.state.categories.find(ctt => ctt.transTypeId === trns1.transTypeId)
                    if (cashFlowDetailsData.cashFlowDetails[idx].data[idx1].transType === undefined) {
              ...
              if (this.selectedAccounts.length > 0) {
                const balanceLastUpdatedDate = AppTimezone.moment(this.selectedAccounts[0].balanceLastUpdatedDate).format('DD/MM/YY')
                if (balanceLastUpdatedDate && cashFlowDetailsData.cashFlowDetails) {
                  const balanceLastUpdatedDateIdx = cashFlowDetailsData.cashFlowDetails.findIndex((item) => item.title === balanceLastUpdatedDate)
                  const balanceLastUpdatedDateArr = cashFlowDetailsData.cashFlowDetails.filter((item) => item.title === balanceLastUpdatedDate)
                  if (balanceLastUpdatedDateArr.length > 0) {
                    const nigreretFalse = balanceLastUpdatedDateArr[0].data.filter((item) => item.nigreret === false)
                    const nigreretTrue = balanceLastUpdatedDateArr[0].data.filter((item) => item.nigreret === true)
                    if (nigreretTrue.length > 0 && nigreretFalse.length > 0) {
                      ...
                      if (nigreretFalse.length > 0) {
                        cashFlowDetailsData.cashFlowDetails.splice(balanceLastUpdatedDateIdx + 1, 0, {
                          data: nigreretFalse,
                          first: false,
                          title: balanceLastUpdatedDateArr[0].title,
                        })
  ...
  filtersAll = (changeTypes) => {
    ...
    setTimeout(() => {
      ...
      if (cashFlowDetailsData && cashFlowDetailsData.cashFlowDetails && cashFlowDetailsData.cashFlowDetails.length) {
                  ...
                  section.data = section.data.filter((item, index) => {
                    ...
                    if (queryStatus.query !== null && queryStatus.query !== '') {
                      return (
                        ...
                        (
                          (item.transName && item.transName.toString().toLowerCase().includes(queryStatus.query.toLowerCase())) ||
        ...
        if (dataGraph.length) {
          ...
          if (listDays) {
            listDays.forEach((it) => {
              ...
              const hova = dataGraph.filter((item) => item.expence === true && it === AppTimezone.moment(item.transDate).format('DD/MM/YY'))
              const zchut = dataGraph.filter((item) => item.expence === false && it === AppTimezone.moment(item.transDate).format('DD/MM/YY'))
      ...
      if (this.state.bankTransIdOpened) {
        const lastRowOpened = bankTrans.find((item) => item.data.find((row) => row.transId === this.state.bankTransIdOpened))
        if (lastRowOpened) {
          const lastRow = lastRowOpened.data.find((item) => item.transId === this.state.bankTransIdOpened)
          if (lastRow) {
  ...
  handleChangeScreenMode = (state, isOpen, closeState) => {
    ...
    if (!isOpen) {
      this.setState({
        dateFromTimestamp: (screenMode === SCREEN_MODES.details) ? AppTimezone.moment().valueOf() : this.selectedAccounts.length > 0 ? AppTimezone.moment(this.selectedAccounts[0].balanceLastUpdatedDate).valueOf() : AppTimezone.moment().valueOf(),
        dateTillTimestamp: (screenMode === SCREEN_MODES.details) ? AppTimezone.moment().add(1, 'month').valueOf() : this.selectedAccounts.length > 0 ? AppTimezone.moment(this.selectedAccounts[0].balanceLastUpdatedDate).add(30, 'day').valueOf() : AppTimezone.moment().add(1, 'month').valueOf(),
      })
    }
    setTimeout(() => {
      return this.getScreenData()
        .then(() => this.setState({ inProgress: false, changeScreenModeInProgress: false }))
  ...
  selectNextAccount = () => {
    ...
    if (!currentSelectedAccountId) {
      return this.setState({ currentSelectedAccountId: selectedAccountIds[0] }, this.getScreenData)
    }

    const index = this.nextAccountIndex
    this.setState({
      currentSelectedAccountId: index === 0 ? null : selectedAccountIds[index - 1],
    }, this.getScreenData)
  }
  ...
  selectPreviousAccount = () => {
    ...
    if (!currentSelectedAccountId) {
      return this.setState({ currentSelectedAccountId: last(selectedAccountIds) }, this.getScreenData)
    }
  ...
  componentDidMount () {
    ...
    if (currentCompanyId) {
      getAccountCflTransTypeApi.post({ body: { uuid: currentCompanyId } })
        .then(data => {
          this.setState({ categories: data.filter(item => item.shonaScreen), categoriesArr: data })
          const selectedAccountIds = this.props.navigation.getParam('selectedAccountIds')
          const messageLink = this.props.navigation.getParam('messageLink')
          if (selectedAccountIds) {
            ...
            this.handleSelectAccounts(currency, selectedAccountIds, () => {
              ...
              setTimeout(() => {
                const balanceLastUpdatedDate = this.selectedAccounts.length > 0 ? this.selectedAccounts[0].balanceLastUpdatedDate : AppTimezone.moment().valueOf()
                this.setState({
                  dateFromTimestamp: messageLink ? AppTimezone.moment().valueOf() : AppTimezone.moment(balanceLastUpdatedDate).valueOf(),
                  dateTillTimestamp: messageLink ? AppTimezone.moment().valueOf() : AppTimezone.moment(balanceLastUpdatedDate).add(30, 'day').valueOf(),
                })

                if (this.props.navigation.state.params.selectedAccountIds) {
                  delete this.props.navigation.state.params.selectedAccountIds
                }
                return this.getScreenData()
              }, 200)
            })
  ...
  addMovement = (type) => () => {
    if (this.props.navigation.state.params.objParamas) {
      delete this.props.navigation.state.params.objParamas
    }

    this.setState({ openModalCreatePayments: true, dataTypePayments: type })
  }
  ...
  openBottomSheet = (dataRow, notChange) => {
    ...
    if ((dataRow.targetType === 'CYCLIC_TRANS' && dataRow.unionId !== null) || dataRow.pictureLink) {
      setTimeout(() => {
        this.getDetails()
      }, 50)
    }
  ...
  handleOpenCategoriesInsideModal = (category, idxCategory, bankTrans) => () => {
    this.setState({
  ...
  handleSelectCategory = (category) => {
    ...
    if (categoryItem) {
      if (!categoryItem || categoryItem.transTypeId === category.transTypeId) return
      ...
      if (!dataRow || dataRow.transType.transTypeId === category.transTypeId) return
  ...
  handleUpdateBankTrans = (newBankTrans) => {
    if (this.state.categoryItem) {
      ...
      if (dataOfRow.transType.transTypeId !== newBankTrans.transType.transTypeId) {
  ...
  handleUpdateTrans = (dataOfRow) => {
    ...
    } else if (dataOfRow['targetType'] === 'LOAN_TAZRIM' || dataOfRow['targetType'] === 'CCARD_TAZRIM' || dataOfRow['targetType'] === 'SOLEK_TAZRIM') {
      ...
      new Api({ endpoint: `cyclic-trans/cfl/single` }).post({
        ...
        .then(data => {
          ...
          if (data && Array.isArray(data.transes) && data.transes.length > 0) {
            ...
            dataRow = Object.assign(JSON.parse(JSON.stringify(dataOfRow)), data.transes[0])
            ...
            if (targetType === 'CYCLIC_TRANS') {
              ...
              if (
                !(mutavArray && Array.isArray(mutavArray) && mutavArray.length === 1 && mutavArray[0].biziboxMutavId !== null) &&
                !(mutavArray && Array.isArray(mutavArray) && mutavArray.length > 1)
              ) {
              ...
              if (targetType === 'CYCLIC_TRANS' && ['MONTH', 'WEEK'].includes(dataRow.transFrequencyName) && dataRow.transDate) {
                if (dataRow.transFrequencyName === 'WEEK') {
                  dataRow.frequencyDay = (AppTimezone.moment(dataRow.transDate).format('dddd')).toUpperCase()
                }
                if (dataRow.transFrequencyName === 'MONTH') {
                  dataRow.frequencyDay = AppTimezone.moment(dataRow.transDate).format('D')
                }
              }
          ...
          new Api({ endpoint: `cyclic-trans/cfl/${dataOfRow['targetType']}/update` }).post({
            body: {
  ...
  handleUpdateTransText = () => {
    ...
    if (dataOfRow.transName !== this.state.copyMainDesc) {
      ...
      this.setState({ dataRow: dataOfRow, editDescModal: false })
      ...
      setTimeout(() => {
        if (dataOfRow['targetType'] === 'BANK_TRANS') {
          ...
          const balanceLastUpdatedDateAcc = this.selectedAccounts.find((acc) => acc.companyAccountId === dataOfRow.companyAccountId)
          if (balanceLastUpdatedDateAcc && dataOfRow.transDate >= balanceLastUpdatedDateAcc.balanceLastUpdatedDate) {
            new Api({ endpoint: `payments/cfl/${dataOfRow['targetType']}/update` }).post({
              body: {
                'asmachta': dataOfRow.asmachta,
                'bank': dataOfRow.bank,
  ...
  getDetails = () => {
    ...
    if (dataRow.pictureLink) {
    ...
    } else if (dataRow.targetType === 'CYCLIC_TRANS' && dataRow.unionId !== null) {
      return cashFlowUnionBankDetApi.post({
        ...
        .then(res => {
          ...
          this.setState({ details: mutavArray, inProgressDetails: false })
        })
        .catch(() => this.setState({ inProgressDetails: false }))
    }
  }
  ...
  render () {
    ...
    const ratio = (win.height - Header.HEIGHT) / win.width

    // //console.log(cashFlowDetailsData,
    //   cashFlowAggregatedData)
    const alertState = this.headerAlertState
    const hasData = this.screenSwitchState
      ? ((cashFlowAggregatedData && !isEmpty(cashFlowAggregatedData.accountTransactions)))
      : ((cashFlowDetailsData && cashFlowDetailsData.cashFlowDetails && cashFlowDetailsData.cashFlowDetails.length > 0))
    ...
    if (modalGetUnionDet && modalGetUnionDet.length) {
      totalModal = getFormattedValueArray(modalGetUnionDet.reduce((memo, val) => {
    ...
    if (dataRow) {
      ...
      if (dataRow && dataRow.paymentDesc === 'Checks' && dataRow.pictureLink && dataRow.pictureLink !== '00000000-0000-0000-0000-000000000000' && dataRow.pictureLink !== null) {
        editCategory = false
      }
    }
        ...
        <View style={[styles.fill, {
          ...
          <Fragment>
            <DataList
              ...
              dataAlertHeader={this.screenSwitchState ? cashFlowAggregatedData : cashFlowDetailsData}
            ...
            {alertDetailsIsOpen && (
              <AccountAlertDetails
                ...
                accounts={this.selectedDeviantAccounts && this.selectedDeviantAccounts.length ? this.selectedDeviantAccounts : this.selectedHarigaDatetAccountsAll}
                onSelectAccount={this.handleForceSelectAccount}
              />
            )}
            ...
            {calendarModalIsOpen && (
              <CashFlowCalendarModal
                ...
                balanceLastUpdatedDate={this.selectedAccounts.length > 0 ? this.selectedAccounts[0].balanceLastUpdatedDate : AppTimezone.moment().valueOf()}
                dateFromTimestamp={dateFromTimestamp}
                dateTillTimestamp={dateTillTimestamp}
                      ...
                      <View style={{ flex: 1, marginLeft: 10 }}>
                        ...
                        {monthYear && monthYear.filter(item => item !== '').length === 1 && (
                          <XAxis
                            ...
                            formatLabel={(value) => Number(datesGraphList[value].split('/')[0])}
                          />
                        )}
                        <XAxis
                          data={monthYear}
                          ...
                          formatLabel={(value) => {
                            ...
                            if (val.toString().length === 0) {
                              ...
                              if (IS_IOS) {
                                return (LocaleConfig.locales['he'].monthNamesShort[Number(vals[0]) - 1] + ' ' + vals[1])
                              } else {
                                const mon = Number(vals[0]) - 1
                                const monHeb = LocaleConfig.locales['he'].monthNamesShort[mon]
                                return (vals[1] + ' ' + Array.from(monHeb).reverse().join(''))
                              }
                            }
                          }}
                        />
                ...
                <View style={{
                  ...
                  <View style={{
                    ...
                    <ScrollView>
                      ...
                      {modalGetUnionDet && modalGetUnionDet.length === undefined && (
                        <View style={[styles.sliderItemGradientNotCard, { width: '100%' }]}>
                          <Loader
                            isDefault
                      ...
                      {modalGetUnionDet && modalGetUnionDet.length > 0 && modalGetUnionDet.map((c, i) => {
                        const numberStyle = cs(c.expence, [styles.dataValue, { color: colors.green4 }], { color: colors.red2 })
                        const total = getFormattedValueArray(c.expence ? -Math.abs(c.total) : c.total)
                          ...
                          <View key={i.toString()}>
                            <View
                              ...
                              <View style={{
                                ...
                                {dataRow && dataRow.canChangeZefi && (
                                  <TouchableOpacity
                                    onPress={this.handlePopRowEditsFromModal(c)}>
                                    <View style={{
                                      marginRight: 'auto',
                                    }}>
                      ...
                      <View style={{
                        ...
                        <View style={{
                          ...
                          <Text
                            ...
                            <Text style={styles.fractionalPart}>.{totalModal[1]}</Text>
                          </Text>
                        </View>
                        <View style={{
                          flex: 20,
                      ...
                      <View
                        ...
                        <View style={{
                          ...
                          <TouchableOpacity
                            ...
                            onPress={this.handleTransCategory(updateTypePopup.updateType === 'future+past' ? 'future' : 'future+past')}>
                            <View style={{
                              marginRight: 'auto',
                              flex: 1,
                              flexDirection: 'row',
          ...
          <View style={[styles.panelContainer]} pointerEvents={'box-none'}>
            ...
            <Interactable.View
              ...
              <View style={styles.panel}>
                ...
                {dataRow && dataRow.canChangeZefi && !(dataRow.sourceProgramName === 'BANK' && AppTimezone.moment(dataRow.transDate).isBefore(AppTimezone.moment().subtract(1, 'days').startOf('day'))) && (
                  <TouchableOpacity
                      ...
                      <View style={{
                        ...
                        <TouchableOpacity
                          ...
                          {!dataRow.canChangeZefi && (
                            <Text style={styles.panelTitle}>{' - סכום סופי'}</Text>
                          )}
                        </TouchableOpacity>
                    ...
                    {dataRow && (
                      <View>
                        <View style={{
                          ...
                          {(dataRow.nigreret) && (
                            <View
                              ...
                              <View style={{
                                ...
                                <Text
                                  style={[styles.dataRowLevel3Text, { color: colors.red2 }]}>
                                  {'נגרר'} {getDaysBetweenTwoDates(new Date(), new Date(dataRow.originalDate))} {'ימים'}
                                </Text>
                              </View>
                            </View>
                          )}
                          ...
                          <TouchableOpacity
                            activeOpacity={(dataRow.unionId && dataRow.targetType === 'SOLEK_TAZRIM') ? 0.2 : 1}
                            onPress={dataRow.unionId && dataRow.targetType === 'SOLEK_TAZRIM' ? this.getUnionDet : null}
                            style={{
                              flexDirection: 'row-reverse',
                              marginVertical: 15,
                              alignItems: 'center',
                            ...
                            <View style={{
                              ...
                              <View style={commonStyles.spaceDividerDouble} />
                              <Text
                                style={styles.dataRowLevel3Text}>{this.props.searchkey && this.props.searchkey.length > 0 && this.props.searchkey.find((it) => it.paymentDescription === dataRow.paymentDesc) ? this.props.searchkey.find((it) => it.paymentDescription === dataRow.paymentDesc).name : ''}</Text>
                            </View>

                            {dataRow.unionId && dataRow.targetType === 'SOLEK_TAZRIM' && (
                          ...
                          {!editCategory ? (
                            <View
                              ...
                              <View style={{
                                ...
                                <CustomIcon
                                  name={getTransCategoryIcon(dataRow.transType.iconType)}
                                  size={16}
                                  color={'#022258'}
                                />
                                <View style={commonStyles.spaceDividerDouble} />
                                <Text
                                  style={styles.dataRowLevel3Text}>{(!textCategory) ? dataRow.transType.transTypeName : textCategory}</Text>
                              </View>
                            </View>
                          ) : (
                            <TouchableOpacity
                              ...
                              onPress={(!textCategory) ? this.handleOpenCategoriesModal : null}>
                        ...
                        {((dataRow.targetType === 'CYCLIC_TRANS' && dataRow.unionId !== null) || dataRow.pictureLink) &&
                        (<View style={{
                          marginTop: 18,
                          alignSelf: 'center',
                          flex: 1,
