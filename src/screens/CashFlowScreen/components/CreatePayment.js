...
import { createAccountCflTransTypeApi, existingCheckApi, getMutavApi, removeAccountCflTransTypeApi } from '../../../api'
import { connect } from 'react-redux'
import { setPopAddCashShow, setPopAddCheckShow } from '../../../redux/actions/user'
...
export default class CreatePayment extends PureComponent {
  ...
  update = () => {
    ...
    this.setState({
      ...
      targetTypeValid: valuesSave.targetType === null || valuesSave.targetType === '',
  ...
  handleSelectCategory = (category) => {
    ...
    this.setState({ obj: valuesSave, categoriesModalIsOpen: false, currentEditBankTrans: null })
  }
  ...
  handleCloseCheckListModal = () => {
    ...
    if (this.state.typeEditModal === 'targetType') {
      ...
      this.setState({
        targetTypeValid: this.state.obj.targetType === null || this.state.obj.targetType === '',
      })
    }
  ...
  editInput = (param) => () => {
    if (param === 'accounts') {
      ...
      const selected = paymentList.find((item) => item.id === this.state.obj.targetType)
      if (selected) {
  ...
  setMutav = (mutav) => () => {
    if (this.state.obj.biziboxMutavId && this.state.obj.biziboxMutavId === mutav.biziboxMutavId) {
      ...
      this.setState({
        obj: currentObjState, biziboxMutavValid: false,
      }, () => {
        this.existingCheck(this.state.obj.transes[this.state.idxObj].asmachta)
      })
    } else {
      ...
      currentObjState.transes[this.state.idxObj].paymentDesc = (currentObjState.receiptTypeId !== 44 ? 'העברה מ' : 'העברה ל') + '' + mutav.accountMutavName
      currentObjState.transes[this.state.idxObj].transTypeId = (this.props.categories.find(ctt => ctt.transTypeId === mutav.transTypeId)) ? this.props.categories.find(ctt => ctt.transTypeId === mutav.transTypeId) : {
        companyId: '00000000-0000-0000-0000-000000000000',
        createDefaultSupplier: true,
        iconType: 'No category',
        shonaScreen: true,
          ...
          <Modal
            ...
            {addMutavModal && (
              <AddMutav
                ...
                companyId={currentCompanyId}
                companyAccountId={obj.companyAccountId ? obj.companyAccountId : accounts.find(a => a.companyAccountId === selectedAccountIds[0]).companyAccountId}
                ...
                <View style={{
                  ...
                  <KeyboardAwareScrollView>
                    ...
                    {this.state.typeEditModal === 'dueDate' && (<View style={{
                      ...
                      <Calendar
                        minDate={this.today}
                        current={AppTimezone.moment(this.state.obj.transes[this.state.idxObj].dueDate).format('YYYY-MM-DD')}
                        markedDates={{
                ...
                <View style={cs(
                  ...
                  <View style={{ alignItems: 'center' }}>
                    <Text style={{ fontSize: sp(20), color: '#ffffff', fontFamily: fonts.semiBold }}>
                      {'הוספת'} {dataTypePayments === 44 ? 'הוצאה' : 'הכנסה'} {'צפוייה'}
                    </Text>
                  </View>
                  <View>
                    <TouchableOpacity
                    ...
                    <View style={[{
                      ...
                      <TouchableOpacity
                        ...
                        <Text style={[{
                          ...
                          {(this.state.obj.targetType) ? (this.props.searchkey && this.props.searchkey.length > 0 && this.props.searchkey.find((it) => it.paymentDescription === this.state.obj.targetType) ? this.props.searchkey.find((it) => it.paymentDescription === this.state.obj.targetType).name : '') : 'בחר'}
                        </Text>
                      </TouchableOpacity>
                    </View>
                  </View>
                  ...
                  <View
                    ...
                    <View style={{
                      ...
                      <TextInput
                        ...
                        onEndEditing={(e) => {
                          let valuesSave = Object.assign({}, this.state.obj)
                          valuesSave.transes[this.state.idxObj].asmachta = e.nativeEvent.text.toString().replace(/[^\d]/g, '')
                          this.setState({ obj: valuesSave })
                          this.existingCheck(e.nativeEvent.text.toString().replace(/[^\d]/g, ''))
                        }}
                        onChangeText={(asmachta) => {
                          let valuesSave = Object.assign({}, this.state.obj)
                          valuesSave.transes[this.state.idxObj].asmachta = asmachta.toString().replace(/[^\d]/g, '')
                          this.setState({ obj: valuesSave })
                          this.existingCheck(asmachta.toString().replace(/[^\d]/g, ''))
                        }}
                        value={inputWorkaround.getWorkaroundChar() + String(this.state.obj.transes[this.state.idxObj].asmachta)}
                        underlineColorAndroid='transparent'
                      />
                    </View>
                  </View>
