...
export default class BankTransSlider extends PureComponent {
    ...
    renderItem = ({ item, index }) => {
      ...
      return (
        ...
        >
          {item
            ? cashFlowDetailsDataItem.pictureLink ? this.blockWithImg(item) : this.simpleBlock(item)
            : <Text>Not found</Text>}
        </LinearGradient>
      )
    };
          ...
          <View style={rowStyles}>
            ...
            <View style={styles.sliderRowTextGroup}>
              <Text style={styles.sliderRowTitle}>
                {t(`bankAccount:${cashFlowDetailsData.hova ? 'nameOfBeneficiary' : 'nameOfTransferor'}`)}
              </Text>
    ...
    render () {
      ...
      const wrapperStyles = cs(cashFlowDetailsDataItem.pictureLink, styles.sliderItemWrapper, styles.sliderItemWrapperHasImg)
        ...
        >
          {inProgress ? (
            <LinearGradient
              locations={[0, 0.5, 1]}
              colors={[colors.blue10, colors.blue11, colors.blue12]}
              style={[styles.sliderItemGradient, { width: slideWidth, marginHorizontal: IS_IOS ? 0 : 24 }]}
            >
              <Loader
