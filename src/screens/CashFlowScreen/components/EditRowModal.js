      ...
      if (this.state.tab === 1) {
        ...
        if (!this.isEquivalent(dataOfRow, this.state.saveOriginObj)) {
          ...
          new Api({ endpoint: `payments/cfl/${dataOfRow['targetType']}/update` }).post({
            ...
            this.setState({ editRowModalIsOpen: false, saveOriginObj: dataOfRow })
        ...
        let frequencyDay = (this.state.dataOfRowGroup.frequencyDay) ? this.state.dataOfRowGroup.frequencyDay : null
        if (dataOfRowGroup.targetType === 'CYCLIC_TRANS' && ['MONTH', 'WEEK'].includes(dataOfRowGroup.transFrequencyName) && dataOfRowGroup.transDate) {
          if (dataOfRowGroup.transFrequencyName === 'WEEK') {
            frequencyDay = (AppTimezone.moment(dataOfRowGroup.transDate).format('dddd')).toUpperCase()
          }
          if (dataOfRowGroup.transFrequencyName === 'MONTH') {
            frequencyDay = AppTimezone.moment(dataOfRowGroup.transDate).format('D')
          }
        }
        if (dataOfRowGroup.transFrequencyName === 'WEEK' && dataOfRowGroup.transDate) {
          frequencyDay = (AppTimezone.moment(dataOfRowGroup.transDate).format('dddd')).toUpperCase()
        }
        if (dataOfRowGroup.transFrequencyName === 'MONTH' && dataOfRowGroup.transDate) {
          frequencyDay = AppTimezone.moment(dataOfRowGroup.transDate).format('D')
        }
        const params = {
        ...
        new Api({ endpoint: `cyclic-trans/cfl/${this.state.dataOfRowGroup.targetType}/update` }).post({
          body: params,
        })
          .then(data => {
            updateRow(true)
              ...
              <View style={{
                  ...
                  <View style={cs(
                    ...
                    <View style={{ alignItems: 'center' }}>
                      <Text style={{ fontSize: sp(20), color: '#ffffff', fontFamily: fonts.semiBold }}>
                        {'עריכת'} {dataOfRow.expence ? 'הוצאה' : 'הכנסה'} {'קבועה'}
                      </Text>
                ...
                (<View style={{
                  ...
                  <View style={cs(
                    ...
                    commonStyles.rowReverse,
                  )}><View><TouchableOpacity underlayColor={'#08d3b8'} style={cs((this.state.tab === 2), [
                      {
                        alignItems: 'center',
                        height: 37,
                        paddingHorizontal: 8,
                      ...
                      <View
                        ...
                        <View style={{
                          ...
                          <TouchableOpacity
                            ...
                            onPress={() => {
                              if (!this.state.disabledGrTab1.transTypeName) {
                                const types = this.props.getAccountCflTransType.map((item) => {
                                  return {
                                    text: item.transTypeName,
                                    id: item.transTypeId,
                                    selected: (item.transTypeId === dataOfRow.transTypeId),
                                  }
                                })
                                this.setState({
                          ...
                          >
                            ...
                            >
                              {this.getAccountCflTransType(dataOfRow.transTypeId).transTypeName}
                            </Text>
                          </TouchableOpacity>
                        </View>
                      </View>
                        ...
                        <View style={{
                          ...
                          {this.state.editSum && (
                            <TextInput
                              ...
                              value={dataOfRow.total ? String(dataOfRow.total) : null}
                              underlineColorAndroid='transparent'
                            />
                          )}
                    ...
                    <KeyboardAwareScrollView>
                      ...
                      <View
                        ...
                        >
                              ...
                              if (!this.state.disabledGrTab2.transTypeName) {
                                const types = this.props.getAccountCflTransType.map((item) => {
                                  return {
                                    text: item.transTypeName,
                                    id: item.transTypeId,
                                    selected: (item.transTypeId === this.state.dataOfRowGroup.transTypeId),
                                  }
                                })
                                this.setState({
                                  typeEditModal: 'transTypeId',
                          ...
                          >
                            ...
                            >
                              {this.getAccountCflTransType(this.state.dataOfRowGroup.transTypeId).transTypeName}
                            </Text>
                          </TouchableOpacity>
                        </View>
                      </View>
                      ...
                      {(this.state.dataOfRowGroup.targetType === 'CYCLIC_TRANS') && (<View
                        style={[cs(!isRtl, commonStyles.row, [commonStyles.rowReverse]), {
                          height: 42,
                          marginBottom: 8,
                        }, cs(this.state.disabledGrTab2.total, { opacity: 1 }, { opacity: 0.3 })]}>
                        <View style={{ flex: 1.76, alignItems: 'flex-end' }}>
                          <Text style={{
                            color: '#0f3860',
                            fontSize: sp(13),
                        ...
                        <View style={{
                          ...
                          {this.state.editSum && (
                            <TextInput
                              ...
                              multiline={false}
                              autoFocus
                              editable={this.state.editSum && !this.state.disabledGrTab2.total}
                              keyboardType={(IS_IOS) ? 'decimal-pad' : 'numeric'}
                              style={[{
                                direction: 'ltr',
                                textAlign: 'right',
                              ...
                              value={this.state.dataOfRowGroup.total ? String(this.state.dataOfRowGroup.total) : null}
                              underlineColorAndroid='transparent'
                            />
                          )}
                            ...
                            <TouchableOpacity
                              ...
                              <Text
                                ...
                                <Text style={[numberStyle, {
                                  ...
                                  lineHeight: 42,
                                  color: '#0f3860',
                                }, commonStyles.regularFont]}>{getFormattedValueArray(this.state.dataOfRowGroup.total)[0]}</Text>
                                <Text style={[styles.fractionalPart, {
                                  fontSize: sp(15),
                                  lineHeight: 42,
                                  color: '#0f3860',
                                }, commonStyles.regularFont]}>.{getFormattedValueArray(this.state.dataOfRowGroup.total)[1]}</Text>
                              </Text>
                            </TouchableOpacity>
                          )}
                        </View>
                      ...
                      {(this.state.dataOfRowGroup.targetType !== 'CCARD_TAZRIM' && this.state.dataOfRowGroup.targetType !== 'SOLEK_TAZRIM') && (
                        <View
                          style={[cs(!isRtl, commonStyles.row, [commonStyles.rowReverse]), {
                            height: 42,
                            marginBottom: 8,
                      ...
                      {(this.state.dataOfRowGroup.targetType === 'DIRECTD') && (<View
                        ...
                        <View style={{
                          ...
                          <Text
                            ...
                            ellipsizeMode='tail'
                          >
                            {AppTimezone.moment(this.state.dataOfRowGroup.transDate).format('DD/MM/YY')}
                          </Text>
                        </View>
                      </View>)}
                      {(this.state.dataOfRowGroup.targetType === 'DIRECTD') && (<View
                      ...
                      {(this.state.dataOfRowGroup.targetType === 'CASH' || this.state.dataOfRowGroup.targetType === 'CCARD_TAZRIM' || this.state.dataOfRowGroup.targetType === 'SOLEK_TAZRIM' || this.state.dataOfRowGroup.targetType === 'DIRECTD' || this.state.dataOfRowGroup.targetType === 'CYCLIC_TRANS') && (
                        <View
                          style={[cs(!isRtl, commonStyles.row, [commonStyles.rowReverse]), {
                            height: 42,
                            marginBottom: 8,
                          ...
                          <View style={{
                            ...
                            <TouchableOpacity
                              ...
                              <Text style={[{
                                ...
                                {(this.state.autoUpdateTypeName.find((item) => item.id === this.state.dataOfRowGroup.autoUpdateTypeName)) ? this.state.autoUpdateTypeName.find((item) => item.id === this.state.dataOfRowGroup.autoUpdateTypeName).text : ''}
                              </Text>
                            </TouchableOpacity>
                          </View>
                        </View>)}
                        ...
                        <View style={{
                          ...
                          <TouchableOpacity
                            ...
                            <Text style={[{
                              ...
                              fontSize: sp(15),
                              lineHeight: 42,
                            }, commonStyles.regularFont]}> {this.state.endDate.find((item) => item.id === this.state.dataOfRowGroup.endDate).text} {(this.state.dataOfRowGroup.endDate === 'on') ? AppTimezone.moment(this.state.dataOfRowGroup.expirationDate).format('DD/MM/YY') : (this.state.dataOfRowGroup.endDate === 'times') ? this.state.dataOfRowGroup.timesValue + ' פעמים' : ''}</Text>
                          </TouchableOpacity>
                        </View>
                      </View>)}
                      ...
                      {(this.state.dataOfRowGroup.targetType === 'LOAN_TAZRIM') && (<View
                        ...
                        <View style={{
                          ...
                          <Text
                            ...
                            ellipsizeMode='tail'
                          >
                            {AppTimezone.moment(this.state.dataOfRowGroup.transDate).format('DD')}
                          </Text>
                        </View>
                      </View>)}
                      {(this.state.dataOfRowGroup.targetType === 'LOAN_TAZRIM') && (<View
