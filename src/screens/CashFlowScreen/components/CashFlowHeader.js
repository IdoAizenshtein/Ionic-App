...
import styles, { HEADER_DATA_ROW_HEIGHT } from '../../BankAccountsScreen/BankAccountsStyles'
...
@translate()
export default class CashFlowHeader extends PureComponent {
  constructor (props) {
    super(props)
    this.state = {
  ...
  get accountTitle () {
    ...
    const title = selectedAccounts.every(a => a._isUpdated) && selectedAccounts.length === accountGroups[selectedGroup].filter(a => a._isUpdated).length
  ...
  containerTouched = () => {
    if (CashFlowHeader.listRef && CashFlowHeader.listRef.blur) {
      CashFlowHeader.listRef.blur()
    }
    return false
  }
  handleSetRef = (ref) => {
    CashFlowHeader.listRef = ref
  }
  ...
  render () {
    ...
    if (AppTimezone.moment(dateFromTimestamp).endOf('month').valueOf() === dateTillTimestamp) {
      textDates = t('cashFlow:tillEndOfThisMonth')
    } else if (AppTimezone.moment(dateFromTimestamp).add(2, 'months').date(0).valueOf() === dateTillTimestamp) {
      textDates = t('cashFlow:tillEndOfNextMonth')
    } else if (AppTimezone.moment(dateFromTimestamp).add(60, 'day').valueOf() === dateTillTimestamp) {
      textDates = t('cashFlow:nextSixtyDays')
    } else if (AppTimezone.moment(dateFromTimestamp).add(30, 'day').valueOf() === dateTillTimestamp) {
      textDates = t('cashFlow:nextThirtyDays')
    }
      ...
      >
        ...
        >
          <View style={[styles.bankAccountHeaderBgWrapper, {
            ...
            <Text style={styles.bankAccountTitle}>{t('mainMenu:cashFlow')}</Text>
