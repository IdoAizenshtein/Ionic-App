...
import { combineStyles as cs, getBankTransIcon, getFormattedValueArray, sp } from '../../../utils/func'
...
import ControlledEditableTextInput from '../../../components/FormInput/ControlledEditableTextInput'
import CategoriesModal from 'src/components/CategoriesModal/CategoriesModal'
import { accountCflDataUpdateApi, createAccountCflTransTypeApi, removeAccountCflTransTypeApi } from '../../../api'
...
export default class BankTransRow extends PureComponent {
  constructor (props) {
    ...
    this.state = {
      currentEditBankTrans: JSON.parse(JSON.stringify(this.props.cashFlowDetailsDataItem)).transType,
      categoriesModalIsOpen: false,
      isEditing: false,
      mainDesc: JSON.parse(JSON.stringify(this.props.cashFlowDetailsDataItem)).transName,
    }
  }

  componentWillReceiveProps (props) {
    if (props.cashFlowDetailsDataItem.transType.transTypeId !== this.props.cashFlowDetailsDataItem.transType.transTypeId) {
      this.setState({
        currentEditBankTrans: JSON.parse(JSON.stringify(props.cashFlowDetailsDataItem)).transType,
      })
    }
  }
  ...
  sectionString (highlight, origStr, caseSensitive) {
    ...
    onStartEdit = () => {
      ...
      if (!this.state.mainDesc) return this.handleChangeMainDesc(dataOfRow.transName)
    ...
    render () {
      ...
      const total = getFormattedValueArray(cashFlowDetailsDataItem.expence ? -Math.abs(cashFlowDetailsDataItem.total) : cashFlowDetailsDataItem.total)
      const numberStyle = cs(cashFlowDetailsDataItem.expence, [styles.dataValue, { color: colors.green4 }], { color: colors.red2 })
            ...
            <TouchableOpacity onPress={this.handleToggle}>
              <View style={wrapperStyles} onLayout={onSetMinHeight}>
                ...
                <View style={[cs(isRtl, styles.dataValueDescWrapperLevel2, commonStyles.rowReverse), {
                  ...
                  <View style={{
                    flexDirection: 'row',
                    alignItems: (cashFlowDetailsDataItem.nigreret) ? 'flex-start' : 'center',
                  }}>
                    {cashFlowDetailsDataItem.targetType !== 'BANK_TRANS' && (
                      <Text
                        style={[cs(isOpen, [styles.dataValue, commonStyles.regularFont], commonStyles.boldFont)]}
                        numberOfLines={1}
                        ellipsizeMode='tail'>
                    ...
                    {cashFlowDetailsDataItem.targetType === 'BANK_TRANS' && (
                      <ControlledEditableTextInput
                        ...
                        value={(isEditing || !mainDesc.length) ? mainDesc : this.renderTextYellow(mainDesc)}
                  ...
                  {(cashFlowDetailsDataItem.nigreret || nigreret) && (
                    <View style={{
                      ...
                      <Text
                        ...
                        numberOfLines={1}
                        ellipsizeMode='tail'>
                        {'נגרר'} {getDaysBetweenTwoDates(new Date(), new Date(cashFlowDetailsDataItem.originalDate))} {'ימים'}
                      </Text>
                    </View>
                  )}
                </View>
                <View style={{
                  ...
                  <Text
                    ...
                    numberOfLines={1}
                    ellipsizeMode='tail'>
                    <Text style={numberStyle}>{this.renderTextYellow(total[0], true)}</Text>
