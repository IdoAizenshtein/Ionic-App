...
export default class DataRow extends AnimatedRow {
    getExpandedData = () => {
      ...
      OneAggregateDataPerDay(data.transDate, (data.hova !== 0) ? 1 : 0, (data.transDate === null))
        .then((cashFlowDetailsData) => {
          let expandedData
          if (data.transDate === null) {
            expandedData = cashFlowDetailsData.cashFlowDetails.filter((item) => item.nigreret === true)
          } else if (data.isTodaySummary) {
            expandedData = cashFlowDetailsData.cashFlowDetails.filter((item) => item.nigreret === false)
          } else {
            expandedData = cashFlowDetailsData.cashFlowDetails
          }
          expandedData.forEach((trns, idx) => {
            trns.transType = categories.find(ctt => ctt.transTypeId === trns.transTypeId)
            if (trns.transType === undefined) {
              trns.transType = categories[0]
            }
          })
          this.setState({ inProgress: false, expandedData })
        })
        ...
        <Animated.View style={[styles.dataRowAnimatedWrapper, { height }]}>
          <TouchableOpacity onPress={this.handleToggle}>
            <View style={wrapperStyles} onLayout={this.setMinHeight}>
              <View style={[{ flex: 1 }, commonStyles.column]}>
                ...
                {data.transDate === null && (
                  <Text style={cs(isOpen, styles.transData, colorGray6)}>
                    {(accountUuid === null) ? t('calendar:today') : accountUuid} {'(נגררות)'}
                  </Text>
                )}
                {data.transDate !== null && (
                  <Text style={cs(isOpen, styles.transData, colorGray6)}>
                    {(data.isTodaySummary) ? t('calendar:today') : dateToFromNowDaily(data.transDate, t)} {data.transDate === null ? ('(נגררות)') : ''}
                  </Text>
                )}
              </View>

              <Text style={styles.dataValueWrapper}>
                <Text style={cs(isOpen, [styles.dataValue, styles.hovaValue], colorWhite)}>{hova[0]}</Text>
              ...
              {data.transDate !== null && (<Text style={[styles.dataValueWrapper, { maxWidth: 90 }]}>
          ...
          <View onLayout={this.setMaxHeight}>
            {inProgress
              ...
              : <BankTrans isRtl={isRtl}
                ...
                categories={categories} accounts={accounts} removeItem={removeItem}
