...
class ScrollCompWithHeader extends PureComponent {
    ...
    get accountsBalance () {
      if (this.props.propsPage.selectedAccounts && this.props.propsPage.selectedAccounts.length) {
        const total = this.props.propsPage.selectedAccounts.reduce((memo, account) => {
    ...
    render () {
      ...
      const notUpdate = selectedNotUpdatedAccounts && selectedNotUpdatedAccounts.length === 1
      const notUpDateTime = (notUpdate) ? AppTimezone.moment().diff(selectedNotUpdatedAccounts[0].balanceLastUpdatedDate, 'days') : 0

      const accountsBalance = (getCurrencyChar(selectedGroup) + ' ' + this.accountsBalance)

      const totalBalanceTitle = (selectedAccounts.length === 1) ? (selectedAccounts[0].isUpdate ? t('bankAccount:totalBalance') : (selectedAccounts[0].isShowItrot ? (`יתרה ל-${AppTimezone.moment(selectedAccounts[0].balanceLastUpdatedDate).format('DD/MM/YY')}`) : t('bankAccount:totalBalance'))) : t('sumsTitles:accountBalance')
      const totalBalanceText = (selectedAccounts.length === 1) ? (selectedAccounts[0].isUpdate ? (accountsBalance) : (selectedAccounts[0].isShowItrot ? (accountsBalance) : '-')) : (accountsBalance)

      const tomorrowItra = dataAlertHeader ? ((selectedAccounts.length === 1 && !screenSwitchState && dataAlertHeader && !selectedAccounts[0].isUpdate && selectedAccounts[0].isShowItrot) ? dataAlertHeader.tomorrowItraNoUpdate : dataAlertHeader.tomorrowItra) : ''
            ...
            <View style={{
              ...
              {hasData && (
                <View style={styles.totalBalanceWrapper}>
                  <TouchableOpacity
                    style={cs(isRtl, [commonStyles.alignItemsCenter], commonStyles.rowReverse)}
                    onPress={this.handleToggleAlerts}
                  >
                    {toggleAlerts
                      ? (
                        <View style={{ alignItems: 'center', flex: 1 }}>
                          <Text style={styles.totalBalanceTitle}>{totalBalanceTitle}</Text>
                        ...
                        >
                          <View>
                            <View style={{
                              ...
                              <Text
                                style={[styles.totalBalanceTitle, { textAlign: 'right' }]}>{'סכום תנועות נגררות'}</Text>

                              {((notUpdate && notUpDateTime >= 7) || (dataAlertHeader && dataAlertHeader[hovaNigrarot] === 0 && dataAlertHeader[zhutNigrarot] === 0)) && (
                                <Text style={[styles.totalBalanceText, {
                                  textAlign: 'right',
                                  fontSize: sp(22),
                                  paddingBottom: 1,
                              ...
                              {((!notUpdate) || (notUpdate && notUpDateTime < 7)) && dataAlertHeader && (dataAlertHeader[hovaNigrarot] !== 0 || dataAlertHeader[zhutNigrarot] !== 0) && (
                                <Text style={[styles.totalBalanceText, {
                                  textAlign: 'right',
                                ...
                                >
                                  ...
                                  {/* - */}
                                  {/* </Text>} */}
                                  {dataAlertHeader && (dataAlertHeader[hovaNigrarot] !== 0) && (dataAlertHeader[zhutNigrarot] !== 0) && (
                                    <Text>{'('}</Text>
                                  )}
                                  {dataAlertHeader && (dataAlertHeader[hovaNigrarot] !== 0) &&
                                  <Text style={{ color: '#fe3461' }}>
                                    ...
                                    {numberFormat.format(Math.round(dataAlertHeader[hovaNigrarot]))}
                                  </Text>}
                                  ...
                                  <Text style={{ color: '#53e69d' }}>
                                    {getCurrencyChar(selectedGroup)}
                                    {numberFormat.format(Math.round(dataAlertHeader[zhutNigrarot]))}
                                  </Text>}
                          ...
                          <View>
                            <View style={{
                              ...
                              <Text
                                style={[styles.totalBalanceTitle, { textAlign: 'right' }]}>{'יתרה צפויה למחר'}</Text>

                              {!notUpdate && screenSwitchState && dataAlertHeader && dataAlertHeader.accountTransactions && (
                                <Text style={[styles.totalBalanceText, {
                                  textAlign: 'right',
                                  fontSize: sp(22),
                                }]}>
                                  {getCurrencyChar(selectedGroup)} {numberFormat.format(Math.round((dataAlertHeader.accountTransactions[1].zhut - dataAlertHeader.accountTransactions[1].hova) + this.accountsBalanceNumber))}
                                </Text>
                              )}
                              {!screenSwitchState && dataAlertHeader && (
                                <Text style={[styles.totalBalanceText, {
                                  ...
                                  {getCurrencyChar(selectedGroup)} {numberFormat.format(Math.round((tomorrowItra)))}
                                </Text>
                              )}
                              {/* {notUpdate && ( */}
                              {/* <Text style={[styles.totalBalanceText, { */}
            ...
            <View>
              <View style={[styles.tableHeadWrapper]}>
                <View style={tableHeaderWrapperStyles}>
                  ...
                  <Text style={[styles.tableHeadText]}>{'יתרה (כולל נגררות)'}</Text>
                </View>
              </View>
            </View>
          )}
...
export default class DataList extends PureComponent {
  constructor (props) {
    ...
    renderAggregatedScrollItem = ({ item }) => {
      ...
      const accountUuid = accounts.find(a => a.companyAccountId === selectedAccountIds[0])

      return (
        <DataRow
          openBottomSheet={this.openBottomSheet}
          companyId={companyId}
          showBtnMatch={!(selectedAccountIds.length === 1 && AppTimezone.moment().diff(accountUuid.balanceLastUpdatedDate, 'days') > 7)}
          accountUuid={(selectedAccountIds.length === 1) ? ((accountUuid.isUpdate) ? null : AppTimezone.moment(accountUuid.balanceLastUpdatedDate).format('DD/MM/YY')) : null}
          goToMatch={goToMatch}
          reload={reload}
    ...
    renderDetailsDataScrollItem = ({ item, section, index }) => {
      ...
      return (
        <Row
          queryStatus={queryStatus}
          key={index}
          isOpen={(queryStatus && queryStatus.query !== null && queryStatus.query !== '') ? true : (item.transId + '_' + item.nigreret ? item.originalDate : item.transDate) === currentOpenItemIndex}
          isRtl={isRtl}
          cashFlowDetailsDataItem={item}
    ...
    openBottomSheet = (item) => () => {
      ...
      if (!screenSwitchState) {
        this.setState({ currentOpenItemIndex: (item.transId + '_' + item.nigreret ? item.originalDate : item.transDate) })
      }
      openBottomSheet(item)
    };
    ...
    handleScrollTo = (index) => {
      ...
      if (this.listRef && this.listRef.props.scrollToPosition) {
        const offset = parseInt(index * (DATA_ROW_HEIGHT + 1) + this.props.headerMaxHeight - 20)
        this.listRef.props.scrollToPosition(0, offset)
      }
    };

    componentWillReceiveProps ({ screenSwitchState, cashFlowDetailsData, currentOpenItemIndex }) {
    ...
    handleScrollEnd = (e) => this.props.onSetScrollPosition(e.nativeEvent.contentOffset.y);
      ...
      return screenSwitchState
        ? (
          <AnimatedSectionList
            ...
            sections={(cashFlowAggregatedDataStateArr && cashFlowAggregatedDataStateArr.length > 0 && !inProgress) ? cashFlowAggregatedDataStateArr : []}
