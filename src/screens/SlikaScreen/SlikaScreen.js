...
import { getAccountSortByDate, getAccountsWithSlika } from '../../redux/selectors/account'
import { getSlikaDetails, getSlikaWithIds } from '../../redux/selectors/slika'
import { getSlikaSummaryApi, slikaCflAggregateDataApi, slikaCflDetailsApi } from '../../api'
...
export default class SlikaScreen extends PureComponent {
  constructor (props) {
    ...
    this.state = ({
      dateFromTimestamp: AppTimezone.moment().subtract(8, 'month').startOf('month').valueOf(),
  ...
  get aggregatedFutureCredits () {
    ...
    getAggregatedDataBody = (cardIds) => {
      const currentCards = this.props.cards.filter(c => cardIds.some(id => c.solekId === id))
      ...
      if (this.props.navigation.state.params.paramTo) {
        const params = {
          companyAccountIds: [],
          solekNums,
        }
        if (this.props.navigation.state.params.paramTo === 'SlikaTwoMonths') {
          params.dateFrom = AppTimezone.moment().subtract(2, 'month').startOf('month').toISOString()
          params.dateTill = AppTimezone.moment().subtract(1, 'month').endOf('month').toISOString()
        } else if (this.props.navigation.state.params.paramTo === 'SlikaThirteenMonths') {
          params.dateFrom = AppTimezone.moment().subtract(13, 'month').startOf('month').toISOString()
          params.dateTill = AppTimezone.moment().subtract(1, 'month').endOf('month').toISOString()
        }
        delete this.props.navigation.state.params.paramTo
    ...
    getCurrentData = (cardsData, currentCardSlideId) => {
      ...
      if (!data) return null
      const newData = data.data.filter(d => d.soleksTotals && d.soleksTotals.length)
    ...
    getLast12MonthData = (data = []) => {
      ...
      return Object.values(data.reduce((memo, item) => {
        ...
        memo[date] = {
          ...
          soleksTotals: [...memo[date].soleksTotals, ...item.soleksTotals].reduce((memo2, card) => {
            const account = accounts.find(a => a.companyAccountId === card.companyAccountIds[0])
        ...
        .map(({ title, month, data, sumMonthlyTotal }) => {
          return {
            ...
            data: data.reduce((memo, soleksTotals) => {
              const oldIndex = memo.findIndex(c => c.companyAccountId === soleksTotals.companyAccountIds[0])
              const account = accounts.find(a => a.companyAccountId === soleksTotals.companyAccountIds[0])
              ...
              if (oldIndex > -1) {
                memo[oldIndex] = {
                  ...
                  monthlyTotal: soleksTotals.monthlyTotal + memo[oldIndex].monthlyTotal,
                  notFinal: memo[oldIndex].notFinal || soleksTotals.notFinal,
                  solekBankId: cards.find((item) => item.solekNum === soleksTotals.solekNum).solekBankId,
                }
              } else {
                soleksTotals.solekBankId = cards.find((item) => item.solekNum === soleksTotals.solekNum).solekBankId
                memo.push({
                  ...soleksTotals,
                  currency: account ? account.currency : null,
                  account: account,
    ...
    getDataForScrollItem = (item, selectedCards) => {
      ...
      const account = currentCard
        ? accountsWithSlika.find(a => a.companyAccountId === currentCard.companyAccountId)
        : null
    ...
    getDataForCarouselItem = (item, cardsData) => {
      return {
        account: this.props.accountsWithSlika.find(a => a.companyAccountId === item.companyAccountId),
