...
import styles from '../../CreditCardsScreen/components/CreditCardHeaderStyles'
...
@translate()
export default class SlikaHeader extends PureComponent {
  get cardTitle () {
    const { cards, selectedCards, t, isRtl, onOpenCardsModal, filterStatus } = this.props
    ...
    if (filterStatus.noCards) {
      return (
        <TouchableOpacity
          style={wrapperStyle}
          onPress={onOpenCardsModal}
        >
          <CustomIcon name='enter-card-alt' size={16} color={colors.blue15} />
          <View style={commonStyles.spaceDividerDouble} />
          <Text style={styles.cardTitleText}>{t('slika:noSolekChosen')}</Text>
        </TouchableOpacity>
      )
    }
    ...
    if (!selectedCards.length) {
      return (
        <TouchableOpacity
          ...
          onPress={onOpenCardsModal}
        >
          <CustomIcon name='enter-card-alt' size={22} color={colors.blue15} />
          <View style={commonStyles.spaceDividerDouble} />
          <Text style={styles.cardTitleText}>{t('slika:noSolekChosen')}</Text>
        </TouchableOpacity>
      )
    ...
    if (selectedCards.length === 1) {
      return (
        ...
        >
          ...
          <View style={commonStyles.spaceDividerDouble} />
          <Text
            style={styles.cardTitleText}>{card.solekNum} {t(`slika:cardNameByType:${card.solekBankId}`)}</Text>
        </TouchableOpacity>
      )
    }
