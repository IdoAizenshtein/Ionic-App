...
import { combineStyles as cs, formatNumberToUnits, getCurrencyChar, sp } from 'src/utils/func'
...
export default class SlikaSliderItem extends PureComponent {
  ...
  get theme () {
    ...
    const defaultTheme = { ...CARD_THEMES[80], imgUri: null }
    ...
    return (
      ...
      >
        ...
        <View style={styles.itemHeadWrapper}>
          <View style={[commonStyles.justifyCenter, isAggregatedView ? styles.itemHeadPart : { flex: 1 }, {}]}>
            <View style={{
              ...
              {isAggregatedView ? (
                <Fragment>
                  <Text
                    style={[styles.cartBalanceText, { marginBottom: 0 }]}>{t('slika:allSelectedCards')}</Text>
                  <View style={[styles.cardIconWrapper, styles.cardCountWrapper]}>
                    <Text>
                      <Text style={styles.cartBalanceText}>{t('slika:cardsCount')}{' '}</Text>
        ...
        <View style={{
          ...
          <View style={{
            ...
            <Text
              ...
              {isAggregatedView ? (
                `${getCurrencyChar(CURRENCIES.ILS)} ${futureCredits && numberFormat.format(futureCredits.toString().split('.')[0])}`
              ) : card.futureCredits ? (
                `${getCurrencyChar(CURRENCIES.ILS)} ${card && numberFormat.format(card.futureCredits.toString().split('.')[0])}`
              ) : '-'
              }
            </Text>
          </View>
          ...
          <View>
            ...
            <Text
              ...
              {isAggregatedView ? (
                `${getCurrencyChar(CURRENCIES.ILS)} ${futureCharges && numberFormat.format(futureCharges.toString().split('.')[0])}`
              ) : card.futureCharges ? (
                `${getCurrencyChar(CURRENCIES.ILS)} ${card && numberFormat.format(card.futureCharges.toString().split('.')[0])}`
              ) : '-'
              }
            </Text>
          </View>
          ...
          <View>
            ...
            <Text
              ...
              {isAggregatedView ? (
                `${getCurrencyChar(CURRENCIES.ILS)} ${futureBalance && numberFormat.format(futureBalance.toString().split('.')[0])}`
              ) : card.futureBallance ? (
                `${getCurrencyChar(CURRENCIES.ILS)} ${card && numberFormat.format(card.futureBallance.toString().split('.')[0])}`
              ) : '-'
              }
            </Text>
          </View>
