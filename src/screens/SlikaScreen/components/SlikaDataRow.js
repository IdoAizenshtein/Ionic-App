...
export default class SlikaDataRow extends AnimatedControlledRow {
  ...
  getExpandedData () {
    ...
    if (!isEmptyData) {
      ...
      onGetDetails({ companyAccounts, solekNums }, data.month)
  ...
  render () {
    ...
    const dataWrapperStyle = cs(isOpen, styles.dataRowWrapper, styles.dataRowActive)
    const isEmptyData = data.sumMonthlyTotal === null
          ...
          <TouchableOpacity onPress={onItemToggle}>
            <View style={dataWrapperStyle} onLayout={this.setMinHeight}>
              ...
              <Text style={cs(isOpen, styles.dataRowDateText, commonStyles.textWhite)}>
                ...
                {data.soleksTotals.some(c => c.notFinal) && `(${t('creditCards:notFinal')})`}
              </Text>
            </View>
          </TouchableOpacity>
