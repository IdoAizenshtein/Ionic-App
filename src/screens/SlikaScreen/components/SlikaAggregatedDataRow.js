...
export default class SlikaAggregatedDataRow extends AnimatedRow {
  ...
  getExpandedData () {
    ...
    onGetDetails({ companyAccounts: data.companyAccountIds, solekNums: data.solekNum }, section.month)
      .then((data) => this.setState({ inProgress: false, expandedData: data ? data.cycleDetails : null }))
  ...
  render () {
    ...
    const dataWrapperStyle = cs(isOpen, [styles.dataRowWrapper, styles.dataRowAggregatedWrapper], styles.dataRowActive)
        ...
        <Animated.View style={[styles.dataRowAnimatedWrapper, { height }]}>
          <TouchableOpacity onPress={this.handleToggle}>
            <View style={dataWrapperStyle} onLayout={this.setMinHeight}>
              <Text style={[styles.dataRowValueText, { flex: 0.95, maxWidth: 95 }]}>
              ...
              <Text style={cs(isOpen, styles.dataRowDateText, commonStyles.textWhite)}>
                {t(`clearingAgenciesName:${data.solekBankId}`)} {data.solekNum}{' '}{data.notFinal && `(${t('creditCards:notFinal')})`}
              </Text>
            </View>
          </TouchableOpacity>
