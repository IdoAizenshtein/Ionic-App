...
export default class SlikaAlert extends PureComponent {
          ...
          <TouchableOpacity
            ...
            <View
              ...
              <Text style={styles.alertAdditionalText}
                numberOfLines={1}
                ellipsizeMode='tail'>
                {(AppTimezone.moment().diff(AppTimezone.moment(selectedCards[0].ballanceLastUpdatedDate), 'days') > 0) ? t(
                  'bankAccount:lastUpdatedXDaysAgo',
                  { days: AppTimezone.moment().diff(AppTimezone.moment(selectedCards[0].ballanceLastUpdatedDate), 'days') },
                ) : (
    ...
    get allStatus () {
      ...
      return (
        <View style={styles.alertOuter}>
          <TouchableOpacity
            onPress={hasBtn ? this.handleOpenUpdateTokenModal(selectedNotUpdatedCards[0]) : this.handleToggleDetails}>
                    ...
                    {isOneOfOne && (
                      <Text style={styles.alertAdditionalText}
                        ...
                        {t(
                          ...
                          { days: AppTimezone.moment().diff(AppTimezone.moment(selectedNotUpdatedCards[0].ballanceLastUpdatedDate), 'days') },
                        )}
                      </Text>
                    )}
                    {isOneOfMultiple && (
              ...
              {(isMoreThenOneOfMultiple) && (
                <View style={styles.alertTextWrapper}>
                  <Text
                    numberOfLines={1}
                    ellipsizeMode='tail'
                    style={[styles.alertText, commonStyles.textCenter, styles.underLine, commonStyles.boldFont]}>
                    {t('slika:xTicketsAreOutOfDate', { count: selectedNotUpdatedCards.length })}
                  </Text>
                </View>
              )}
              {(hasBtn) && (
    ...
    componentWillReceiveProps (nextProps) {
      ...
      if (selectedAccountsNextProps.length === 1 && selectedAccountsNextProps[0].isUpdate === false && selectedAccountsNextProps[0].alertStatus === null) {
