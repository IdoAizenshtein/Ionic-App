...
@translate()
export default class CreditCardsModal extends PureComponent {
  get groups () {
    const {
      isRtl,
      accounts,
    ...
    return accounts.map((account, i) => {
      const filteredCards = cards.filter(c => c.companyAccountId === account.companyAccountId)
      ...
      return (
        <Fragment key={account.companyAccountId}>
          ...
          {i + 1 < accounts.length &&
          <Divider
            style={cs(isRtl, accountModalStyles.accountGroupsDivider, accountModalStyles.accountGroupsDividerRtl)} />}
        </Fragment>
      )
    })
  }
  ...
  render () {
    ...
    return (
      ...
      >
        <ScrollView
          style={cs(isRtl, accountModalStyles.modalBody, accountModalStyles.modalBodyRtl)}
          contentContainerStyle={{ paddingVertical: 20 }}
        >
          <TouchableOpacity
            style={cs(isRtl, cs(isAllSelected, [styles.item, accountModalStyles.group], accountModalStyles.itemChecked), styles.itemRtl)}
            onPress={onSelectAll}
