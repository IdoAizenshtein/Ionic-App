...
import { combineStyles as cs, getEmoji, goTo, isRtl as checkRtl, sp } from 'src/utils/func'
...
export default class SignupScreen extends PureComponent {
  ...
  handleUpdateField = name => val => {
    ...
    if (name === 'password') {
      this.setState({ [name]: value.toString().replace(/([\u05D0-\u05FF/\s+]+)/g, '') })
  ...
  handleUpdateFieldValid = name => val => {
    ...
    if (name === 'passwordValid') {
      this.setState({ [name]: !(!value || value.length < 8 || value.length >= 12 || value.replace(/[^\d]/g, '').length === 0 || value.replace(/[^A-Za-z]/g, '').length === 0) })
      ...
      this.setState({ [name]: value && (value.length === 10 || value.length === 11) && new RegExp('(050|052|053|054|055|057|058|02|03|04|08|09|072|073|076|077|078)-?\\d{7,7}').test(value) })
      ...
      if (value && value.length === 9 && result) {
        ...
        new Api({ endpoint: 'companies/hp-exists', secure: mail === null }).post({
          body: {
            companyHp: value,
            username: mail,
          },
  ...
  handleUpdateFieldValidAsync = (e) => {
    ...
    if (val && re.test(val) && val.length > 0) {
      ...
      this.setState({ 'mailValid': false, mailIsHebrew: isHebrew, emailExists: false })
    }
  }

  handleUpdateLeadInfo = () => {
    const { firstName, lastName, cell, mail, firstNameValid, lastNameValid, cellValid, mailValid, passwordValid, agreementConfirm, emailExists, inProgress, password, business } = this.state

    if (inProgress || !(
      firstNameValid && lastNameValid && cellValid && mailValid && passwordValid && agreementConfirm && !emailExists &&
      firstName.length > 0 && lastName.length > 0 && password.length > 0 && mail.length > 0 && cell.length > 0
    )) {
      return
    }
    Keyboard.dismiss()
    ...
    return updateLeadInfo.post({
      ...
      .then((data) => {
        this.setState({ inProgress: false, screen: SCREEN.BusinessData })
      })
      .catch(() => {
  ...
  handleSignupCreate = () => {
    ...
    return dispatch(signupCreate({
      ...
      .then(() => {
        companiesApi.get()
          .then((data) => {
            const companyId = Array.isArray(data) && data.length > 0 ? data[0].companyId : null
            if (companyId) {
              this.setState({ inProgress: false, screen: SCREEN.AccountData, companyId })
            }
          })
          .catch(() => {
  ...
  componentDidMount () {
    const { isRtl, dispatch } = this.props
  ...
  handleGoToPersonalData = () => this.setState({ inProgress: false, screen: SCREEN.PersonalData })

  handleToggleOtpFormModal = () => this.setState({ otpFormModalIsOpen: !this.state.otpFormModalIsOpen })
  ...
  handleChangeBankId = (bankId) => this.setState({ bankId, formData: {}, formErrors: {}, currentOtpField: null })

  handleToggleBankModal = () => this.setState({ bankModalIsOpen: !this.state.bankModalIsOpen })

  handleSetOtpField = (currentOtpField) => () => this.setState({ currentOtpField: currentOtpField })
  ...
  handleCreateBankToken = () => {
    ...
    setTimeout(() => {
      ...
      if (!scheme || inProgress || !isEmpty(formErrors)) return this.setState({ inProgress: false })

      this.setState({ inProgress: true })

      const keyCode = scheme.otp
        ? `__otp__${currentOtpField}`
        : scheme.fields[1].controlType === BANK_CREDENTIALS_CONTROL_TYPES.PASSWORD
      ...
      if (!newTokenId) {
        const body = {
          ...
          bankPass: formData[scheme.fields.find(f => f.controlType === BANK_CREDENTIALS_CONTROL_TYPES.PASSWORD).key],
        ...
        return createTokenTypeApi.post({ body })
          .then((token) => {
            this.props.navigation.setParams({ updateToken: token })
            this.setState({ newTokenId: token, is409Error: false })
            if (companyId) {
          ...
          .catch((err) => {
            if (err.status === 409) {
              this.setState({ inProgress: false, is409Error: true })
            } else {
        ...
        return updateTokenTypeApi.post({ body })
          .then(() => {
            this.props.navigation.setParams({ updateToken: newTokenId })
            if (companyId) {
              return getStatusTokenTypeApi.post({ body: { companyId, tokens: [newTokenId] } })
            } else {
              return false
            }
          })
        ...
        return getStatusTokenTypeApi.post({ body: { companyId, tokens: [newTokenId] } })
          .then(([newTokenStatus]) => {
            ...
            if (statusCode === BANK_TOKEN_STATUS.INVALID_PASSWORD) {
              this.setState({
                inProgress: false,
                invalidPasswordModalIsOpen: newTokenStatus.screenPasswordUpdateCount < 3,
                lockedModalIsOpen: newTokenStatus.screenPasswordUpdateCount >= 3,
              })
              this.stopPullingTokenStatus()
            }

            if ([BANK_TOKEN_STATUS.VALID, BANK_TOKEN_STATUS.PASSWORD_ABOUT_TO_EXPIRED].includes(statusCode)) {
              this.setState({ inProgress: false, successModalIsOpen: true })
              this.stopPullingTokenStatus()
            }
          })
      }
    ...
    return (
      <Fragment>
        ...
        {scheme.fields.map(f => {
          ...
          let placeHolder = t(`foreignCredentials:${schemeId}:${f.key}:placeHolder`)
            ...
            <View style={styles.formRow} key={f.key}>
              <View style={styles.formRowLeftPart}>
                <RoundedTextInput
                  ...
                  onEndEditing={(e) => {
                    this.handleUpdateFormField(f.key)(e.nativeEvent.text.toString().replace(/([\u05D0-\u05FF/\s+]+)/g, ''))
                  }}
                  isInvalid={formErrors[f.key]}
                  isPassword={f.controlType === BANK_CREDENTIALS_CONTROL_TYPES.PASSWORD}
                  onChangeText={this.handleUpdateFormField(f.key)}
                  onSubmitEditing={this.validateForm}
                  onBlur={this.validateForm}
                  value={formData[f.key]}
                  keyboardType={f.validation.numericality ? 'numeric' : 'default'}
                />
              </View>
        ...
        {(scheme.otp && scheme.otpTypes) ? (
          <TouchableOpacity style={styles.formRow} onPress={this.handleToggleOtpFormModal}>
            <View
              style={cs(Object.keys(formErrors).some(k => k.includes('__otp__')), styles.bankTypeBtn, [textInputStyles.invalid, {
                borderTopWidth: 1,
                borderBottomWidth: 1,
                borderRightWidth: 1,
              }])}>
              ...
              {currentOtpField && (
                <Text style={styles.bankName}>{formData[`__otp__${currentOtpField}`]}</Text>
              )}
            </View>
    ...
    return (
      <Fragment>
        <View style={styles.formRow}>
          <View style={styles.formRowLeftPart}>
            <TouchableOpacity style={styles.bankTypeBtn} onPress={this.handleToggleBankModal}>
              <Icon name='chevron-left' size={22} color={colors.blue8} />
            ...
            return (
              <View style={styles.formRow} key={f.key}>
                <ExtendableTextInput
                  ...
                  placeholder={t(`foreignCredentials:${bankId}:otpTypes:${f.key}:rule`)}
                  placeholderTextColor={colors.gray34}
                  label={t(`foreignCredentials:${bankId}:otpTypes:${f.key}:label`)}
                  description={t(`foreignCredentials:${bankId}:otpTypes:${f.key}:description`)}
                  tooltip={t(`foreignCredentials:${bankId}:otpTypes:${f.key}:info`)}
                />
              </View>
            )
          })}
  ...
  validateForm = () => {
    ...
    const formErrors = scheme.fields.reduce((memo, field) => {
      if (!field.validation || (field.key === 'userCode' && scheme.otp)) return memo
    ...
    if (scheme.otp) {
      if (currentOtpField) {
        ...
        const error = validate.single(formData[`__otp__${currentOtpField}`], otpField.validation)
  ...
  render () {
    const {
      inProgress, screen, agreementConfirm, firstName, lastName, password, mail, cell, isReady, secureTextEntry, emailExists,
      firstNameValid,
      lastNameValid,
      cellValid,
      mailValid,
        ...
        <SafeAreaView style={{
          ...
          <KeyboardAwareScrollView
            ...
            <View style={[styles.titleWrapper, {
              ...
              />
              <Text
                style={styles.titleText}>{screen === SCREEN.Preview ? 'בחרו את המוצר שאליו תרצו להירשם' : (
                  business === false
                    ? 'הרשמה ל-bizibox'
              ...
              <View style={{
                ...
                <TouchableHighlight
                  ...
                  style={{
                    ...
                    alignContent: 'center',
                    flexDirection: 'column',
                    backgroundColor: business === false ? '#0bc8b0' : 'transparent',
                  }}>
                  <View>
                    <View style={{
                ...
                <TouchableHighlight
                  ...
                  style={{
                    ...
                    alignContent: 'center',
                    flexDirection: 'column',
                    backgroundColor: business === true ? '#0bc8b0' : 'transparent',
                  }}>
                  <View>
                    <View style={{
                      flexDirection: 'row-reverse',
              ...
              <View style={{
                ...
                <View style={{
                  ...
                  <View style={{
                    ...
                    alignItems: 'center',
                    alignContent: 'center',
                    opacity: (screen !== SCREEN.PersonalData && screen !== SCREEN.BusinessData) ? 1 : 0.5,
                  }}>
                    <Image
                      resizeMode='contain'
                      style={{
                ...
                <View style={{
                  ...
                  <View style={{
                    ...
                    <Text style={{
                      fontFamily: screen !== SCREEN.PersonalData ? fonts.semiBold : fonts.regular,
                      color: '#0f3860',
                      textAlign: 'center',
                      fontSize: sp(14.5),
                  ...
                  <View style={{
                    ...
                    <Text style={{
                      fontFamily: (screen !== SCREEN.PersonalData && screen !== SCREEN.BusinessData) ? fonts.semiBold : fonts.regular,
                      color: '#0f3860',
                      textAlign: 'center',
                      fontSize: sp(14.5),
              ...
              {screen === SCREEN.PersonalData && (
                <Fragment>
                  ...
                  <View style={{
                    ...
                    <TextInput
                      maxLength={30}
                      onEndEditing={this.handleUpdateFieldValid('firstNameValid')}
                      ...
                      style={[
                        styles.input,
                        {
                          borderBottomColor: !firstNameValid ? colors.red2 : colors.blue3,
                          borderBottomWidth: 2,
                          fontFamily: fonts.semiBold,
                          fontWeight: 'normal',
                          color: colors.blue29,
                  ...
                  <View style={{
                    ...
                    <TextInput
                      maxLength={30}
                      onEndEditing={this.handleUpdateFieldValid('lastNameValid')}
                      ...
                      style={[
                        styles.input,
                        {
                          borderBottomColor: !lastNameValid ? colors.red2 : colors.blue3,
                          borderBottomWidth: 2,
                          fontFamily: fonts.semiBold,
                          fontWeight: 'normal',
                          color: colors.blue29,
                  ...
                  <View style={{
                    ...
                    <TextInput
                      ...
                      style={[
                        styles.input,
                        {
                          borderBottomColor: !cellValid ? colors.red2 : colors.blue3,
                          borderBottomWidth: 2,
                          fontFamily: fonts.semiBold,
                          fontWeight: 'normal',
                          color: colors.blue29,
                          textAlign: (!cell || (cell && cell.length === 0)) ? 'right' : 'left',
                          height: 50,
                          fontSize: sp(17),
                          backgroundColor: 'transparent',
                        },
                      ...
                      onEndEditing={(e) => {
                        this.setState({
                          cell: e.nativeEvent.text.toString().replace(/[^\d-]/g, ''),
                        })
                  ...
                  <View style={{
                    ...
                    <TextInput
                      ...
                      style={[
                        styles.input,
                        {
                          borderBottomColor: (emailExists || !mailValid) ? colors.red2 : colors.blue3,
                          borderBottomWidth: 2,
                          fontFamily: fonts.semiBold,
                          fontWeight: 'normal',
                          color: colors.blue29,
                          textAlign: (!mail || (mail && mail.length === 0)) ? 'right' : 'left',
                          height: 50,
                          fontSize: sp(17),
                          backgroundColor: 'transparent',
                        },
                  ...
                  <View style={{
                    ...
                    <TextInput
                      ...
                      style={[
                        styles.input,
                        {
                          borderBottomColor: !passwordValid ? colors.red2 : colors.blue3,
                          borderBottomWidth: 2,
                          fontFamily: fonts.semiBold,
                          fontWeight: 'normal',
                          color: colors.blue29,
                      ...
                      onEndEditing={(e) => {
                        this.setState({
                          password: e.nativeEvent.text.toString().replace(getEmoji(), '').replace(/\s+/g, ''),
                        })
                    ...
                    <TouchableOpacity style={{
                      ...
                      {this.state.secureTextEntry
                        ...
                        : <Icons name='eye-off-outline' size={19} color={colors.blue29} />}
                    </TouchableOpacity>
                  </View>
                  <View style={[{
                    width: '100%',
                    ...
                    <View style={{
                      ...
                      <Icon
                        name={(password.length >= 8 && password.length < 12) ? 'check' : 'block-helper'}
                        type='material-community'
                        size={(password.length >= 8 && password.length < 12) ? 16 : 12}
                        color={'#022258'}
                      />
                    ...
                    <View style={{
                      ...
                      <Icon
                        name={(password.replace(/[^\d]/g, '').length > 0) ? 'check' : 'block-helper'}
                        type='material-community'
                    ...
                    <View style={{
                      ...
                      <Icon
                        name={(password.replace(/[^A-Za-z]/g, '').length > 0) ? 'check' : 'block-helper'}
                        type='material-community'
                        size={(password.replace(/[^A-Za-z]/g, '').length > 0) ? 16 : 12}
                        color={'#022258'}
                      />
                      <View style={commonStyles.spaceDivider} />
                      <Text style={{
                    ...
                    <View style={{
                      ...
                      <View>
                        <CheckBox
                          ...
                          textStyle={{
                            fontSize: sp(13),
                            color: !agreementConfirm ? colors.red2 : colors.blue30,
                            fontWeight: 'normal',
                            textAlign: 'right',
                            fontFamily: fonts.regular,
                            right: -2,
                          ...
                          iconRight
                          checkedIcon={<Image
                            style={{ width: 19.5, height: 19.5, marginTop: 0 }}
                            source={require('BiziboxUI/assets/boxChecked.png')} />}
                          uncheckedIcon={<Image
                            style={{ width: 19.5, height: 19.5, marginTop: 0 }}
                            source={require('BiziboxUI/assets/boxUnChecked.png')} />}
                          checked={agreementConfirm}
                          onPress={() => this.setState({ agreementConfirm: !agreementConfirm })}
                        />
                      </View>
                      <Text
                  ...
                  <Button
                    disabled={!(
                      firstNameValid && lastNameValid && cellValid && mailValid && passwordValid && agreementConfirm && !emailExists &&
                      (firstName && firstName.length > 0) && (lastName && lastName.length > 0) && (password && password.length > 0) && (mail && mail.length > 0) && (cell && cell.length > 0)
                    )}
                    disabledTitleStyle={{
                      color: '#ffffff',
                      opacity: 0.8,
                ...
                <Fragment>
                  ...
                  <View style={{
                    ...
                    <TextInput
                      ...
                      style={[
                        styles.input,
                        {
                          borderBottomColor: !companyNameValid ? colors.red2 : colors.blue3,
                          borderBottomWidth: 2,
                          fontFamily: fonts.semiBold,
                          fontWeight: 'normal',
                          color: colors.blue29,
                  ...
                  <View style={{
                    ...
                    <TextInput
                      ...
                      style={[
                        styles.input,
                        {
                          borderBottomColor: (!companyHpValid || idExists) ? colors.red2 : colors.blue3,
                          borderBottomWidth: 2,
                          fontFamily: fonts.semiBold,
                          fontWeight: 'normal',
                          color: colors.blue29,
                          textAlign: (!companyHp || (companyHp && companyHp.length === 0)) ? 'right' : 'left',
                          height: 50,
                          fontSize: sp(17),
                          backgroundColor: 'transparent',
                        },
                      ...
                      onEndEditing={(e) => {
                        this.setState({
                          companyHp: e.nativeEvent.text.toString().replace(/[^\d]/g, ''),
                        })
                  ...
                  <Button
                    disabled={!(
                      ...
                      (companyHp && companyHp.length > 0) && (companyName && companyName.length > 0)
                    )}
                    disabledTitleStyle={{
                      color: '#ffffff',
                      opacity: 0.8,
                ...
                <Fragment>
                  ...
                  <View style={styles.validationContainer}>
                    {Object.keys(formErrors).map(key => {
                      if (key.includes('__otp__')) {
                        ...
                        return (
                          <Text style={styles.errMessageText} key={key}>
                            {t(`foreignCredentials:${bankId}:otpTypes:${currentOtpField}:label`)}: {t(`foreignCredentials:${bankId}:otpTypes:${currentOtpField}:rule`)}
                          </Text>
                        )
                      }
                      ...
                      return (
                        <Text style={styles.errMessageText} key={key}>
                          {t(`foreignCredentials:${schemeId}:${key}:label`)}: {t(`foreignCredentials:${schemeId}:${key}:rule`)}
                        </Text>
                      )
                    })}
                    ...
                    {'message' in he.foreignCredentials[schemeId] && (
                      <Text
                        style={styles.infoMessageText}>{t(`foreignCredentials:${schemeId}:message`)}</Text>
                    )}

                    {is409Error && (
                      <Text style={styles.errMessageText}>{'פרטי הכניסה קיימים במערכת'}</Text>
                    )}
                  </View>
                  <Button
                    disabled={(!ALL_BANK_CREDENTIALS_SCHEME[bankId] || inProgress || !isEmpty(formErrors))}
                    disabledTitleStyle={{
                      color: '#ffffff',
                      opacity: 0.8,
                    }}
            ...
            <View style={{
              ...
              <Image
                style={[{ width: 188, height: 23, marginVertical: 15 }]}
