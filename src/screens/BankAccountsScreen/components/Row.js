...
import { combineStyles as cs, getBankTransIcon, getFormattedValueArray } from '../../../utils/func'
...
export default class Row extends PureComponent {
  sectionString (highlight, origStr, caseSensitive) {
    ...
    render () {
      ...
      const mainDescs = bankTrans.mainDesc ? bankTrans.mainDesc.trim() : bankTrans.mainDesc
        ...
        <TouchableOpacity style={styles.dataRowAnimatedWrapper} onPress={onToggle}>
          <View style={wrapperStyles}>
            <View style={cs(isRtl, styles.dataValueDescWrapperLevel2, commonStyles.rowReverse)}>
              ...
              <Text
                style={[cs(isOpen, [styles.dataValue, commonStyles.regularFont, { flex: 0 }], commonStyles.boldFont), { flex: 1 }]}
                numberOfLines={1}>{this.renderTextYellow(mainDescs)}</Text>
