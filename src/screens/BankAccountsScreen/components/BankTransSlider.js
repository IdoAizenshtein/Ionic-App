...
import styles, { SLIDER_IMG_HEIGHT } from '../BankAccountsStyles'
...
export default class BankTransSlider extends PureComponent {
    ...
    renderItem = ({ item, index }) => {
      ...
      return (
        ...
        >
          {item
            ? (bankTrans.pictureLink ? this.blockWithImg(item) : this.simpleBlock(item))
            : (<Text>Not found</Text>)}
        </LinearGradient>
      )
    };
    ...
    simpleBlock = (item) => {
      const { bankTrans, isRtl, account, t } = this.props
      ...
      return (
        <Fragment>
          <View style={rowStyles}>
            <View style={styles.sliderRowTextGroup}>
              ...
              <View style={cs(isRtl, commonStyles.row, [commonStyles.rowReverse])}>
                ...
                <Text style={styles.sliderRowValue}>{item.branchtransfernumber}</Text>
              </View>
            </View>
            ...
            <View style={styles.sliderRowTextGroup}>
              ...
              <TextIcon
                isRtl={isRtl}
                text={`${getCurrencyChar(account.currency)}${item.transfertotal}`}
          ...
          <View style={rowStyles}>
            <View style={styles.sliderRowTextGroup}>
              <Text style={styles.sliderRowTitle}>{t('bankAccount:invoice')}</Text>
              <TextIcon
                isRtl={isRtl}
            ...
            <View style={styles.sliderRowTextGroup}>
              <Text style={styles.sliderRowTitle}>
                {t(`bankAccount:${bankTrans.hova ? 'nameOfBeneficiary' : 'nameOfTransferor'}`)}
              </Text>
    ...
    blockWithImg = (item) => {
      const { isRtl, account, t } = this.props
        ...
        <Fragment>
          <View style={rowStyles}>
            <View style={styles.sliderRowTextGroup}>
              ...
              <View style={cs(isRtl, commonStyles.row, [commonStyles.rowReverse])}>
                ...
                <View style={commonStyles.spaceDivider} />
                <Text style={styles.sliderRowValue}>{item.chequeBranchNumber}</Text>
              </View>
            </View>
            ...
            <View style={styles.sliderRowTextGroup}>
              ...
              <TextIcon
                isRtl={isRtl}
                text={`${getCurrencyChar(account.currency)}${item.chequeTotal}`}
                textStyle={styles.sliderRowValue}
                iconName='money'
                iconSize={18}
                iconColor={colors.white}
          ...
          <View style={rowStyles}>
            ...
            <View style={styles.sliderRowTextGroup}>
              ...
              <TextIcon
                isRtl={isRtl}
                text={AppTimezone.moment(item.depositDate).format(DEFAULT_DATE_FORMAT)}
                textStyle={styles.sliderRowValue}
                iconName='calendar'
                iconSize={14}
                iconColor={colors.white}
          ...
          <View style={[rowStyles, styles.sliderRowLast]}>
            <View style={styles.sliderRowTextGroup}>
              <Text style={styles.sliderRowTitle}>{t('bankAccount:checkNumber')}</Text>
    ...
    render () {
      ...
      const wrapperStyles = cs(bankTrans.pictureLink, styles.sliderItemWrapper, styles.sliderItemWrapperHasImg)

      return (
        <View
          style={[wrapperStyles, data.length === 0 ? {
            // height: 60,
          } : {}]}
