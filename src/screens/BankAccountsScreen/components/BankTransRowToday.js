...
export default class BankTransRowToday extends AnimatedRow {
    ...
    componentDidMount () {
      this.setState({ expandedData: this.props.data })
    }
      ...
      if (nextProps.data !== this.props.data) {
        ...
        if (isOpen) {
          setTimeout(() => {
            ...
            this.setState({ expandedData: nextProps.data, isOpen: false })
          }, 20)
        }
      } else {
    ...
    fixRowHeightChild = (heightVal) => {
      ...
      Animated.timing(height, { toValue: heightVal, duration: 10, easing: Easing.bounce }).start()
    };
    ...
    render () {
      ...
      const wrapperStyles = cs(isRtl, cs(isOpen, [styles.dataRow, commonStyles.bgWhite], styles.dataRowActive), commonStyles.rowReverse)
      ...
      const isTodayTrans = date && AppTimezone.moment().diff(accountAggregatedTodayTrans.transDate, 'days') === 0
            ...
            <TouchableOpacity onPress={this.handleToggle}>
              <View style={wrapperStyles} onLayout={this.setMinHeight}>
                <View style={[{ flex: 1 }, commonStyles.column]}>
                  ...
                  <Text style={cs(isOpen, [styles.transData, commonStyles.boldFont], colorWhite)}>
                    {date
                      ? `${date} ${isTodayTrans ? `(${t('bankAccount:notFinal')})` : ''}`
