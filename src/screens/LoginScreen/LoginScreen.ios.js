...
import { getEmoji, getErrText, goTo, isRtl as checkRtl, sp } from '../../utils/func'
import jwtDecode from 'jwt-decode'
import { OVERVIEW, SIGNUP } from '../../constants/navigation'
import { getVersionApi, smsApi } from '../../api'
...
import { ALERTS_TRIAL } from '../../constants/config'
...
export default class LoginScreen extends PureComponent {
  ...
  setScreen = (screen) => {
    this.setState({ screen, username: '', password: '', confirmPassword: '', code: '', inProgress: false, err: '' })
  }
  ...
  handleUpdateField = name => value => {
    if (name === 'password' || name === 'confirmPassword') {
      this.setState({ [name]: value.toString().replace(getEmoji(), '').replace(/\s+/g, '') })
    } else if (name === 'code') {
  ...
  handleLoginSubmit = () => {
    const { username, password, inProgress, rememberMeState } = this.state
    const { dispatch } = this.props
    const re = /\S+@\S+\.\S+/
    ...
    if (!username || !password || inProgress || errMail) return
    Keyboard.dismiss()
    this.setState({ inProgress: true, err: '' })
    ALERTS_TRIAL.showAlertActivated = true
    return dispatch(login({ username, password, rememberMe: rememberMeState }))
      .then(({ token, tokenInfo }) => {
        dispatch(setRememberMe(rememberMeState))
        const decodedToken = jwtDecode(token)
        if (decodedToken.type !== 'PRE_AUTH') return this.login()
      ...
      .catch(() => {
        this.setState({ inProgress: false, password: '', err: 'הפרטים לא תואמים, אנא נסו שוב' })
      })
  }
  ...
  handleOtpLoginSubmit = () => {
    ...
    Keyboard.dismiss()
    this.setState({ inProgress: true, smsRemainedOTP: (smsRemainedOTP + 1) })

    return dispatch(otpLogin(code))
      .then((res) => {
        if (res.token === 'Incorrect one time token code') {
          if (this.state.screen === SCREEN.OTP_LOGIN && this.state.smsRemainedOTP >= 4) {
          ...
          this.setState({ err: '' })
          // this.removeSmsListener()
          if (this.state.screen === SCREEN.OTP_LOGIN) return this.login()
      ...
      .catch(err => {
        ...
        this.setState({ inProgress: false, code: '', err: '' })
  ...
  handleResendSms = () => {
    ...
    smsApi.post()
      .then(({ smsRemained, maskedPhoneNumber }) => {
        this.setState({ tokenInfo: { smsRemained, maskedPhoneNumber }, smsRemainedOTP: 0 })
      })
      .catch(err => {
        if (err.status === 401) {
  ...
  handleResetPassword = () => {
    ...
    if (!username || inProgress || errMail) return
    Keyboard.dismiss()
    ...
    return dispatch(resetPassword(username))
      .then(({ tokenInfo }) => {
        this.setState({ tokenInfo })
        if (tokenInfo && tokenInfo.maskedPhoneNumber && tokenInfo.maskedPhoneNumber === 'User not found') {
  ...
  handleChangePassword = () => {
    ...
    const { dispatch } = this.props
    if (!password || password !== confirmPassword || inProgress || password.length < 8 || password.length >= 12 || password.replace(/[^\d]/g, '').length === 0 || password.replace(/[^A-Za-z]/g, '').length === 0 || (password.length > 0 && (jwtDecode(this.props.token) && jwtDecode(this.props.token).sub === password))) return
    Keyboard.dismiss()
    this.setState({ inProgress: true })
  ...
  getAllData = () => {
    ...
    return dispatch(getUser())
      .then(() => {
        if (this.props.user.changePasswordRequired === 1 || this.props.user.changePasswordRequired === true) {
          this.setScreen(SCREEN.CHANGE_PASSWORD)
        } else {
          return Promise.all([
  ...
  getVersion () {
    getVersionApi.get()
      .then(latestVersion => {
        this.setState({
          isVersionType: latestVersion ? (latestVersion.verType === 1) : false,
        })
      })
  }

  render () {
    const { inProgress, screen, tokenInfo, username, password, confirmPassword, code, isReady, err, secureTextEntry, rememberMeState, errMail, secureTextEntryNew, secureTextEntryRepeat, smsRemainedOTP, isVersionType } = this.state
    const { t } = this.props
      ...
      <ScrollView
        ...
        <View style={[
              ...
              <View style={{
                ...
                <TextInput
                  ...
                  style={[
                    styles.input,
                    {
                      borderBottomColor: (err !== '' || errMail) ? colors.red2 : colors.blue3,
                      borderBottomWidth: 2,
                ...
                <View style={{
                  ...
                  <View>
                    <CheckBox
                      ...
                      title='זכור אותי'
                      iconRight
                      checkedIcon={<Image style={{ width: 30, height: 32.5, marginTop: 5 }}
                        source={require('BiziboxUI/assets/checkboxChecked.png')} />}
                      uncheckedIcon={<Image style={{ width: 30, height: 32.5, marginTop: 5 }}
                        source={require('BiziboxUI/assets/checkbox.png')} />}
                      checked={rememberMeState}
                      onPress={() => this.setState({ rememberMeState: !rememberMeState })}
                    />
                  </View>

                  <TouchableOpacity style={styles.linkBtn} onPress={this.handleGoToResetPassword}>
                    <Text style={styles.linkBtnText}>{t('login:forgotPassword')}</Text>
                  </TouchableOpacity>
                </View>
          ...
          {(screen === SCREEN.OTP_LOGIN) && (
            <Fragment>
              <Text style={[commonStyles.fill, {
                marginTop: 50,
                textAlign: 'right',
                fontSize: sp(16.5),
                color: colors.blue29,
              }]}>{'קוד אימות לכניסה חד פעמית נשלח למספר הטלפון'} {tokenInfo.maskedPhoneNumber}</Text>
              <Text style={[commonStyles.fill, {
                textAlign: 'right',
                fontSize: sp(16.5),
                color: colors.blue29,
              }]}>{'שימו לב, הקוד תקף ל-10 דקות בלבד'}</Text>
          ...
          {(screen === SCREEN.OTP_RESET_PASSWORD) && (
            <Fragment>
              <View style={{
                ...
                <TextInput
                  placeholder={tokenInfo.maskedPhoneNumber + 'הקלידו את הקוד שנשלח לנייד '}
                  placeholderTextColor='#202020'
                  autoCorrect={false}
                  autoCapitalize='none'
                  returnKeyType='done'
            ...
            <Fragment>
              <View style={{
                ...
                <View style={{
                  ...
                  <View style={[styles.formControlsWrapper, {
                    margin: 0,
                    paddingTop: 15,
                    height: 30,
                  }]}>
          ...
          {screen === SCREEN.CHANGE_PASSWORD && (
            <Fragment>
              <View style={{
                ...
                <TextInput
                  ...
                  style={[
                    styles.input,
                    {
                      borderBottomColor: ((password.length > 0 && confirmPassword.length > 0 && password !== confirmPassword) || (password.length > 0 && jwtDecode(this.props.token) && jwtDecode(this.props.token).sub === password)) ? colors.red2 : colors.blue3,
                      borderBottomWidth: 2,
                      fontFamily: fonts.semiBold,
                      fontWeight: 'normal',
                      color: colors.blue29,
                ...
                <TouchableOpacity style={{
                  ...
                  alignItems: 'center',
                  justifyContent: 'center',
                }} onPress={this.handleTogglePasswordVisibilityDyna('secureTextEntryNew')}>
              ...
              {password.length > 0 && (jwtDecode(this.props.token) && jwtDecode(this.props.token).sub === password) && (
                <View>
                  <Text style={{
                    fontSize: sp(16),
                    fontFamily: fonts.regular,
                    color: colors.red2,
                    textAlign: 'center',
                  }}>{'בחרו סיסמה השונה משם המשתמש'}</Text>
                </View>
              )}
              ...
              <View style={{
                ...
                <TextInput
                  ...
                  style={[
                    styles.input,
                    {
                      borderBottomColor: ((password.length > 0 && confirmPassword.length > 0 && password !== confirmPassword)) ? colors.red2 : colors.blue3,
                      borderBottomWidth: 2,
                      fontFamily: fonts.semiBold,
                      fontWeight: 'normal',
                      color: colors.blue29,
                  ...
                  onEndEditing={(e) => {
                    this.setState({
                      confirmPassword: e.nativeEvent.text.toString().replace(getEmoji(), '').replace(/\s+/g, ''),
                    })
                  }}
                ...
                <TouchableOpacity style={{
                  ...
                  alignItems: 'center',
                  justifyContent: 'center',
                }} onPress={this.handleTogglePasswordVisibilityDyna('secureTextEntryRepeat')}>
              ...
              {password.length > 0 && confirmPassword.length > 0 && password !== confirmPassword && (
                <View>
                  <Text style={{
                    ...
                    color: colors.red2,
                    textAlign: 'center',
                  }}>{'הסיסמאות לא זהות, אנא בדקו שוב'}</Text>
                </View>
              )}
          ...
          {screen === SCREEN.CAN_NOT_BE_CHANGED && (
            <Fragment>
              <Text style={[styles.infoText, {
                textAlign: 'right',
              }]}>{t('login:cantProceedResetPass')}</Text>
              <Text style={[styles.infoText, {
                textAlign: 'right',
              }]}>{t('login:contactSupportTeam')}</Text>
          ...
          {screen === SCREEN.ERROR_401 && (
            <Fragment>
              <Text style={[styles.infoText]}>{'נעשו 3 ניסיונות כניסה לא נכונים'}</Text>
              <Text style={[styles.infoText]}>{'לא ניתן להמשיך בתהליך ההזדהות'}</Text>
