...
export default class LoginScreen extends PureComponent {
  ...
  // }
    ...
    handleChangePassword = () => {
      ...
      const { dispatch } = this.props
      if (!password || password !== confirmPassword || inProgress || password.length < 8 || password.length >= 12 || password.replace(/[^\d]/g, '').length === 0 || password.replace(/[^A-Za-z]/g, '').length === 0) return
      Keyboard.dismiss()
      this.setState({ inProgress: true })
    ...
    render () {
      const { inProgress, screen, tokenInfo, username, password, confirmPassword, code, isReady, err, secureTextEntry, rememberMeState, errMail, secureTextEntryNew, secureTextEntryRepeat, smsRemainedOTP } = this.state
      const { t } = this.props
