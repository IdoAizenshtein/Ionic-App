...
import { combineStyles as cs, getErrText, getTransCategoryIcon, goTo, goToBack, sp } from '../../utils/func'
...
import { getDefaultUserScreen, updateDefaultUserScreen } from '../../redux/actions/user'
...
import { DEFAULT_DATE_FORMAT, IS_IOS, USER_SCREENS } from '../../constants/common'
...
export default class ChecksScreen extends PureComponent {
  constructor (props) {
    ...
    const paramsLink = props.navigation.getParam('paramsLink')

    this.state = {
      isDefDates: true,
      currentIndex: (paramsLink && paramsLink.queryStatus) ? paramsLink.queryStatus : null,
      accountsILS: [],
      isReady: false,
      inProgress: false,
      ...
      error: null,
      selectedAccountIds: (paramsLink && paramsLink.selectedAccountIds) ? paramsLink.selectedAccountIds : [],
      _widthIncome: 0,
      _widthOut: 0,
      sizeFontStatusTotals: 25,
      statusTotals: [],
      querySearch: null,
      queryStatus: (paramsLink && paramsLink.queryStatus) ? paramsLink.queryStatus : null,
      currentOpenItemIndex: null,
      // if null then it is aggregated request
      currentSelectedAccountId: null,
      selectedGroup: (paramsLink && paramsLink.selectedGroup) ? paramsLink.selectedGroup : null,
  ...
  get headerMinHeight () {
    const minHeight = (IS_IOS ? 30 : 35) + (this.state.isSearchOpen ? (IS_IOS ? 85 : 90) : 0)
  ...
  get headerMinHeightCalc () {
    ...
    return this.hasHeaderAlert ? minHeight + HEADER_ALERT_BORDER_HEIGHT + 30 : minHeight
  }
  ...
  get hasHeaderAlert () {
    if (!this.selectedAccounts.length || (!this.selectedNotUpdatedAccounts.length)) return false
  ...
  get hasPreviousAccount () {
    ...
    getNameOfStatus = (sn) => {
      ...
      switch (sn) {
        ...
        case 'mechake_lehafkada':
          return this.screenSwitchState ? ('sumsTitles:notDeposited') : ('sumsTitles:notRepaid')
    ...
    selectDefaultAccounts = () => {
      ...
      const findIdsToSelect = (currency) => {
        ...
        return this.handleSelectAccounts(currency, selectedIds, this.getScreenData)
      }
    ...
    getStatus (statusKey, dueDate) {
      ...
      } else if (status === 'ufkad_vehazar' || status === 'huavar_lesapak_vehazar') {
        return 'comeBack'
    ...
    setStatusDef () {
      ...
      if (!queryStatus || !statusTotals.some(status => status.status.toLowerCase() === queryStatus)) {
        if ((this.screenSwitchState && statusTotals.some(status => status.status.toLowerCase() === 'not_paid')) || (!this.screenSwitchState && statusTotals.some(status => status.status.toLowerCase() === 'mechake_lehafkada'))) {
          this.setState({
            queryStatus: (this.screenSwitchState) ? 'not_paid' : 'mechake_lehafkada',
          })
        } else if ((this.screenSwitchState && statusTotals.some(status => status.status.toLowerCase() === 'mechake_lehafkada')) || (!this.screenSwitchState && statusTotals.some(status => status.status.toLowerCase() === 'future_due_date'))) {
          this.setState({
            queryStatus: (this.screenSwitchState) ? 'mechake_lehafkada' : 'future_due_date',
          })
        } else if ((this.screenSwitchState && statusTotals.some(status => status.status.toLowerCase() === 'mishmeret_babank')) || (!this.screenSwitchState && statusTotals.some(status => status.status.toLowerCase() === 'past_due_date'))) {
          this.setState({
            queryStatus: (this.screenSwitchState) ? 'mishmeret_babank' : 'past_due_date',
          })
        } else if (statusTotals.some(status => status.status.toLowerCase() === 'ufkad_veshulam')) {
          this.setState({
            queryStatus: 'ufkad_veshulam',
          })
        } else if ((this.screenSwitchState && statusTotals.some(status => status.status.toLowerCase() === 'lenicaion'))) {
          this.setState({
            queryStatus: 'lenicaion',
          })
        } else if (statusTotals.some(status => status.status.toLowerCase() === 'nimhak_aliadei_user')) {
          this.setState({
            queryStatus: 'nimhak_aliadei_user',
          })
        } else if ((this.screenSwitchState && statusTotals.some(status => status.status.toLowerCase() === 'ufkad_vehazar') && statusTotals.some(status => status.status.toLowerCase() === 'huavar_lesapak_vehazar')) || (!this.screenSwitchState && statusTotals.some(status => status.status.toLowerCase() === 'return_check'))) {
          this.setState({
            queryStatus: (this.screenSwitchState) ? 'ufkad_vehazar' : 'return_check',
          })
        }
      }
    ...
    getScreenData = (isLoader = true) => {
      if (this.props.navigation.state.params.paramsLink) {
        delete this.props.navigation.state.params.paramsLink
      }
      const objParamas = this.props.navigation.getParam('objParamas')
      if (isLoader) this.setState({ inProgress: true, isLayoutComplete: false, currentOpenItemIndex: null })
      if (!isLoader) this.setState({ inProgress: true, isLayoutComplete: true, currentOpenItemIndex: null })
      ...
      return this.getChecksData()
        .then((data) => {
          ...
          if (data['statusTotals'] && data['statusTotals'].length) {
            sizeFontStatusTotals = (data['statusTotals'].find((item) => item.total.toString().length > 5)) ? 20 : 25
          ...
          if (data['chequeDetails'] && data['chequeDetails'].length) {
            ...
            const { t } = this.props
            programNameList = Array.from(new Set(chequeDetails.map(s => s.programName))).map(id => {
              return {
                id: id,
                press: false,
              }
            ...
            for (const item of chequeDetails) {
              if (!map.has(item.status)) {
                ...
                statusList.push({
                  press: false,
                  id: item.status,
                  text: t(`sumsTitles:${this.getStatus(item.status, item.dueDate)}`),
                })
              }
            }
            categoriesMatch = Array.from(new Set(chequeDetails.map(s => s.transTypeId))).map(id => {
              let obj = this.state.categoriesArr.find((cat) => cat.transTypeId === id)
              if (!obj) {
          ...
          if (objParamas && data['statusTotals'].length && data['chequeDetails'].length) {
            this.setState({
              sizeFontStatusTotals: sizeFontStatusTotals,
              accountsILS: this.props.accounts.filter((a) => a.currency === 'ILS'),
              error: null,
              inProgress: false,
              isLayoutComplete: true,
              isHeaderChartSliderPanEnable: true,
        ...
        if (data['chequeDetails'] && data['chequeDetails'].length) {
          ...
          if (queryStatus && !isSearchOpen) {
            if (queryStatus === 'ufkad_vehazar') {
              chequeDetails = chequeDetails.filter((item) => (item.status === queryStatus.toUpperCase() || item.status === 'huavar_lesapak_vehazar'.toUpperCase()))
            } else if (queryStatus === 'return_check') {
              chequeDetails = chequeDetails.filter((item) => (item.status === queryStatus.toUpperCase() || item.status === 'huavar_lesapak_vehazar'.toUpperCase() || item.status === 'ufkad_vehazar'.toUpperCase()))
            } else if (queryStatus === 'not_paid') {
              chequeDetails = chequeDetails.filter((item) => (item.status === queryStatus.toUpperCase() || item.status === 'mechake_lehafkada'.toUpperCase() || item.status === 'mishmeret_babank'.toUpperCase()))
            } else if (queryStatus === 'future_due_date') {
              chequeDetails = chequeDetails.filter((item) => (item.status === 'MECHAKE_LEHAFKADA' && item.dueDate >= AppTimezone.moment().valueOf()))
            } else if (queryStatus === 'past_due_date') {
              chequeDetails = chequeDetails.filter((item) => (item.status === 'MECHAKE_LEHAFKADA' && item.dueDate < AppTimezone.moment().valueOf()))
            } else {
              chequeDetails = chequeDetails.filter((item) => (item.status === queryStatus.toUpperCase()))
            }
          }
          ...
          if ((querySearch !== null && querySearch !== '') || changeTypes) {
            ...
            const programNameListPressFilter = programNameList.filter(a => a.press === true)

            try {
              chequeDetails = chequeDetails.filter((item, index) => {
                ...
                const statusListPress = statusList.find(a => a.id === item.status).press
                const programNamePress = programNameList.find(a => a.id === item.programName).press

                item.chequeComment = (item.chequeComment) ? item.chequeComment : (item.deleteAlYadei) ? `נמחק על ידי ${item.deleteAlYadei} בתאריך ${AppTimezone.moment(item.deleteDate).format(DEFAULT_DATE_FORMAT)}` : ''

                if (querySearch !== null && querySearch !== '') {
                  return (
                    (categoriesMatchPressFilter.length ? categoriesMatchPress : !categoriesMatchPress) &&
                                    (statusListPressFilter.length ? statusListPress : !statusListPress) &&
                                    (programNameListPressFilter.length ? programNamePress : !programNamePress) &&
                                    (
                                      (item.mainDescription && item.mainDescription.toString().toLowerCase().includes(querySearch.toLowerCase())) ||
                                        (item.chequeComment && item.chequeComment.toString().toLowerCase().includes(querySearch.toLowerCase())) ||
                                        (item.chequeNo && item.chequeNo.toString().toLowerCase().includes(querySearch.toLowerCase())) ||
                                        (item.total && item.total.toString().includes(querySearch)) ||
                                        (item.dueDate &&
                                            (querySearch.toLowerCase() === AppTimezone.moment(item.dueDate).format('DD/MM') ||
                                                querySearch.toLowerCase() === AppTimezone.moment(item.dueDate).format('MM/YY') ||
                                                querySearch.toLowerCase() === AppTimezone.moment(item.dueDate).format('DD/MM/YY')
                                            )
                                        )
                                    )
                  )
          ...
          if (chequeDetails.length) {
            chequeDetails = chequeDetails.reduce((memo, item) => {
              ...
              item.account = accounts.find(a => a.companyAccountId === item.companyAccountId)
              item.statusText = t(`sumsTitles:${this.getStatus(item.status, item.dueDate)}`)
              item.isRemovable = this.isRemovable(item.status, item.dueDate)
              item.isEditable = {
                dueDateAsDate: item.isRemovable,
                account: item.isRemovable && ['CHEQUE', 'ERP_CHEQUE'].includes(item.targetType),
                mainDescription: true,
                transType: true,
                chequeNo: item.isRemovable && ['CHEQUE'].includes(item.targetType),
                chequeComment: true,
          ...
          if (dataGraph.length) {
            ...
            listDays.forEach((it) => {
              ...
              const zchut = dataGraph.filter((item) => it === AppTimezone.moment(item.dueDate).format('DD/MM/YY'))
            ...
            const yAxisSums = Array.from(new Array(numberTickZchut)).map((it, i) => {
              return max - ((max / 5) * i)
            })
    ...
    getChecksData = () => {
      ...
      return dispatch(getChecks({
        ...
        dateFrom: (dateFromTimestamp) ? AppTimezone.moment(dateFromTimestamp).toISOString() : dateFromTimestamp,
        dateTill: (dateTillTimestamp) ? AppTimezone.moment(dateTillTimestamp).toISOString() : dateTillTimestamp,
      }))
    };
    ...
    handleChangeScreenMode = (state) => {
      ...
      this.setState({ changeScreenModeInProgress: true, currentOpenItemIndex: null })
      this.setDefaultScrollPosition()

      return dispatch(updateDefaultUserScreen(USER_SCREENS.checksMobile, newScreenMode))
    ...
    componentDidMount () {
      ...
      const { dispatch, currentCompanyId, navigation } = this.props
      const paramsLink = navigation.getParam('paramsLink')
      if (currentCompanyId) {
        getAccountCflTransTypeApi.post({ body: { uuid: currentCompanyId } })
          .then(data => {
            ...
            if (paramsLink && paramsLink.checksMobileState) {
              return dispatch(updateDefaultUserScreen(USER_SCREENS.checksMobile, paramsLink.checksMobileState))
            } else {
          ...
          .then(() => {
            ...
            return this.setState({
              ...
              isReady: true,
            }, (paramsLink && paramsLink.checksMobileState) ? this.getScreenData : this.selectDefaultAccounts)
          })
    ...
    isSearchOpen = (isSearchOpen, filter) => {
      if (isSearchOpen) {
        this.setState({ isSearchOpen: !isSearchOpen, querySearch: null })
        if (filter) {
          setTimeout(() => {
            // console.log(this.state.querySearch)
    ...
    render () {
      ...
      let lenStatusListCopy = statusListCopy.filter((it) => it.press === true).length
      let lenProgramNameCopy = programNameListCopy.filter((it) => it.press === true).length
      let lenCategoriesMatchCopy = categoriesMatchCopy.filter((it) => it.press === true).length
            ...
            <View style={[styles.fill, {
              ...
              {openModalCreatePayments && (
                <CreatePayment
                  ...
                  accounts={accounts.filter((a) => a.currency === 'ILS')}
              ...
              {(programNameListModal) && (
                <View style={{
                  ...
                  <View style={{
                    ...
                    >
                      {programNameListCopy.length > 0 && programNameListCopy.map((f, i) => {
                        return (
                          <View key={i.toString()}
                            style={{
                              height: 50,
