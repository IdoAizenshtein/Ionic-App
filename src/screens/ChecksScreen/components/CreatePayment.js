...
import { combineStyles as cs, getFormattedValueArray, goTo, sp } from '../../../utils/func'
...
import { setPopAddCheckShow } from '../../../redux/actions/user'
...
export default class CreatePayment extends PureComponent {
  ...
  constructor (props) {
    ...
    const objParamas = props.navigation.getParam('objParamas')

    this.state = {
      currentDate: null,
      reommendationsList: [],
      fadeAnim: new Animated.Value(1),
      showAlert: (!objParamas && props.showPopAlertCheck),
      idxObj: (objParamas && objParamas.idxObj !== undefined && objParamas.idxObj !== null) ? objParamas.idxObj : 0,
      obj: (objParamas) ? (objParamas.obj) : {
        'companyAccountId': (props.selectedAccountIds.length === 1) ? props.accounts.find(a => a.companyAccountId === props.selectedAccountIds[0]).companyAccountId : null,
  ...
  update = () => {
    ...
    this.setState({
      ...
      companyAccountIdNoValid: valuesSave.companyAccountId === null || valuesSave.companyAccountId === '',
      totalNoValid: valuesSave.transes[this.state.idxObj].total === null || valuesSave.transes[this.state.idxObj].total === '',
      paymentDescValid: valuesSave.transes[this.state.idxObj].paymentDesc === null || valuesSave.transes[this.state.idxObj].paymentDesc === '',
    })
    if (this.state.idxObj !== valuesSave.transes.length - 1) {
      valuesSave.transes.forEach((itemInside, idx) => {
        if ((valuesSave.transes.length > (idx + 1)) && (itemInside.paymentDesc === null || itemInside.paymentDesc === '' || itemInside.total === null || itemInside.total === '')) {
          isValid = false
        }
      })
      if (isValid) {
      ...
      valuesSave.transes.forEach((itemInside) => {
        if (itemInside.paymentDesc === null || itemInside.paymentDesc === '' || itemInside.total === null || itemInside.total === '') {
          isValid = false
        }
      })
    }
  ...
  removePayments = (index) => () => {
    ...
    this.setState({ idxObj: (valuesSave.transes.length - 1), obj: valuesSave })
  }
    ...
    if (this.state.idxObj !== params.transes.length - 1) {
      ...
      if (params.companyAccountId && params.targetType && isValid) {
        ...
        valuesSave.transes[valuesSave.transes.length - 1] = {
          ...
          paymentDesc: (valuesSave.transes[valuesSave.transes.length - 2].paymentDesc) ? String(valuesSave.transes[valuesSave.transes.length - 2].paymentDesc) : '',
          asmachta: (valuesSave.transes[valuesSave.transes.length - 2].asmachta) ? String(Number(valuesSave.transes[valuesSave.transes.length - 2].asmachta) + 1) : '',
          dueDate: AppTimezone.moment(valuesSave.transes[valuesSave.transes.length - 2].dueDate).add(1, 'month').valueOf(),
        }
        this.setState({ idxObj: (valuesSave.transes.length - 1), obj: valuesSave })
      ...
      if (params.companyAccountId && params.targetType && isValid) {
        ...
        valuesSave.transes.push({
          ...valuesSave.transes[valuesSave.transes.length - 1],
          paymentDesc: (valuesSave.transes[valuesSave.transes.length - 1].paymentDesc) ? String(valuesSave.transes[valuesSave.transes.length - 1].paymentDesc) : '',
          asmachta: (valuesSave.transes[valuesSave.transes.length - 1].asmachta) ? String(Number(valuesSave.transes[valuesSave.transes.length - 1].asmachta) + 1) : '',
          dueDate: AppTimezone.moment(valuesSave.transes[valuesSave.transes.length - 1].dueDate).add(1, 'month').valueOf(),
        })
        this.setState({ idxObj: (this.state.idxObj + 1), obj: valuesSave })
        // console.log(this.state.obj)
  ...
  setModalInsideVisible = (visible) => () => {
    this.setState({ editModalInsideIsOpen: visible, currentDate: null })
  }
  ...
  existingCheck = (text) => {
    let valuesSave = Object.assign({}, this.state.obj)
    const lastRow = valuesSave.transes[this.state.idxObj !== undefined ? this.state.idxObj : valuesSave.transes.length - 1]

    if (this.state.obj.companyAccountId &&
      ((text && text.length) || (valuesSave.biziboxMutavId) || (Number(lastRow.total) !== 0 && lastRow.dueDate))
    ) {
      existingCheckApi.post({
        body: {
          ...
          'accountMutavName': (valuesSave.biziboxMutavId)
            ? (this.state.biziboxMutavList && this.state.biziboxMutavList.length > 0 && this.state.biziboxMutavList.find((it) => it.biziboxMutavId === valuesSave.biziboxMutavId) ? (this.state.biziboxMutavList.find((it) => it.biziboxMutavId === valuesSave.biziboxMutavId).accountMutavName) : null)
            : null,
  ...
  dueDate = () => {
    const selectAcc = this.props.accounts.find((item) => item.companyAccountId === this.state.obj.companyAccountId)
    if (this.state.obj.companyAccountId && selectAcc && selectAcc.nonUpdateDays === 0) {
  ...
  getRecommendation = (month) => {
    ...
    const selectAcc = this.props.accounts.find((item) => item.companyAccountId === obj.companyAccountId)
    if (this.state.obj.companyAccountId && selectAcc && selectAcc.nonUpdateDays === 0) {
      const params = {
        'total': this.state.obj.transes[this.state.idxObj].total,
        'companyAccountId': this.state.obj.companyAccountId,
        'dateFrom': AppTimezone.moment(month).startOf('month').valueOf(),
        'dateTill': AppTimezone.moment(month).endOf('month').valueOf(),
      }
      return recommendationApi.post({ body: params })
        .then(data => {
          this.setState({
            reommendationsList: data.reommendationsList,
          })
        })
        .catch(() => {
          this.setState({
  ...
  setMutav = (mutav) => () => {
    if (this.state.obj.biziboxMutavId && this.state.obj.biziboxMutavId === mutav.biziboxMutavId) {
    ...
    return (
      <TouchableOpacity
        ...
        style={{
          ...
          backgroundColor: (item.biziboxMutavId === this.state.obj.biziboxMutavId) ? '#f5f5f5' : '#ffffff',
        }}>
        <View style={{
          flex: 12,
          justifyContent: 'center',
  ...
  editInput = (param) => () => {
    if (param === 'accounts') {
      ...
      this.getRecommendation(AppTimezone.moment(this.state.obj.transes[this.state.idxObj].dueDate).valueOf())
  ...
  render () {
    const { categoriesModalIsOpen, currentEditBankTrans, obj, showAlert, fadeAnim, reommendationsList, modalMutavListShow, currentMutav, biziboxMutavListFilter, inProgressMutavList, addMutavModal, mutavRow } = this.state
    const dataTypePayments = obj.receiptTypeId
    const { isRtl, currentCompanyId, accountsAll, selectedAccountIds } = this.props
    const total = getFormattedValueArray(this.state.obj.transes[this.state.idxObj].total)
    const numberStyle = cs(dataTypePayments === 44, [styles.dataValue, { color: colors.green4 }], { color: colors.red2 })
    const rowStyle = !isRtl ? 'row-reverse' : 'row'
    const selectAcc = this.props.accounts.find((item) => item.companyAccountId === obj.companyAccountId)
    const itemRec = (dataTypePayments === 44 && this.state.obj.companyAccountId && selectAcc && selectAcc.nonUpdateDays === 0 && reommendationsList.length > 0) ? reommendationsList.find((item) => AppTimezone.moment(item.transDate).isSame(AppTimezone.moment(this.state.obj.transes[this.state.idxObj].dueDate), 'day')) : null

    let isCheckExistText = ''
    if (this.state.obj.transes[this.state.idxObj].asmachtaExist) {
      const dueDate = AppTimezone.moment(this.state.obj.transes[this.state.idxObj].asmachtaExist.dueDate).format('DD/MM/YY') === AppTimezone.moment().format('DD/MM/YY')
        ? 'היום'
        : AppTimezone.moment(this.state.obj.transes[this.state.idxObj].asmachtaExist.dueDate).format('DD/MM/YY')
      const totals = String(getFormattedValueArray(this.state.obj.transes[this.state.idxObj].asmachtaExist.total)[0]).includes('-') ? (String(getFormattedValueArray(this.state.obj.transes[this.state.idxObj].asmachtaExist.total)[0]).replace('-', '') + '-') : getFormattedValueArray(this.state.obj.transes[this.state.idxObj].asmachtaExist.total)[0]
          ...
          <Modal
            ...
            {addMutavModal && (
              <AddMutav
                ...
                companyId={currentCompanyId}
                companyAccountId={obj.companyAccountId ? obj.companyAccountId : accountsAll.find(a => a.companyAccountId === selectedAccountIds[0]).companyAccountId}
                ...
                <View style={{
                  ...
                  <KeyboardAwareScrollView>
                    ...
                    {this.state.typeEditModal === 'dueDate' && (<View style={{
                      ...
                      <Calendar
                        minDate={this.today}
                        current={(this.state.currentDate) ? null : AppTimezone.moment(this.state.obj.transes[this.state.idxObj].dueDate).format('YYYY-MM-DD')}
                        markedDates={{
                          [AppTimezone.moment(this.state.obj.transes[this.state.idxObj].dueDate).format('YYYY-MM-DD')]: {
                            startingDay: true,
                            selected: true,
                            color: colors.blue3,
                          },
                        ...
                        onMonthChange={(month) => {
                          this.setState({
                            currentDate: month.timestamp,
                          })
                          this.getRecommendation(month.timestamp)
                        }}
                        onDayPress={(day) => {
                      ...
                      {dataTypePayments === 44 && !obj.companyAccountId && (
                        <Text style={{
                          color: colors.dark3,
                          fontSize: sp(18.5),
                          lineHeight: 30,
                          ...
                          להצגת יתרות צפויות אנא בחרו חשבון
                        </Text>
                      )}
                      {dataTypePayments === 44 && obj.companyAccountId && selectAcc.nonUpdateDays > 0 && (
                        <Text style={{
                          color: colors.dark3,
                          fontSize: sp(18.5),
                          lineHeight: 30,
                          ...
                          לא ניתן להפעיל מחשבון המלצה
                          {'\n'}
                          עבור חשבון לא מעודכן
                        </Text>)}
                      {itemRec && (<View>
                        <Text style={{
                          color: colors.dark3,
                          ...
                          צפי יתרה לתאריך נבחר
                        </Text>
                        <Text style={{
                          color: itemRec.color === 'green' ? colors.green14 : colors.red2,
                          fontSize: sp(26),
                          lineHeight: 30,
                          fontFamily: fonts.regular,
                          textAlign: 'center',
                        }}>
                          {'₪'}{getFormattedValueArray(itemRec.itra)[0]}
                        </Text>
                      </View>)}
                    </View>)}
                  </KeyboardAwareScrollView>
                </View>
                {(this.state.typeEditModal === 'dueDate' && dataTypePayments === 44) && (
                  <View style={[cs(isRtl, {
                    position: 'absolute',
                    bottom: -100,
                    right: -100,
                    ...
                    <TouchableOpacity
                      style={
                        { opacity: (this.state.obj.companyAccountId && selectAcc && selectAcc.nonUpdateDays === 0) ? 1 : 0.5 }
                      }
                      activeOpacity={!(this.state.obj.companyAccountId && selectAcc && selectAcc.nonUpdateDays === 0) ? 0.5 : 1}
                ...
                <View style={cs(
                  ...
                  <View style={{ alignItems: 'center' }}>
                    <Text style={{ fontSize: sp(20), color: '#ffffff', fontFamily: fonts.semiBold }}>
                      {'הוספת'} {'צ׳ק'} {dataTypePayments === 44 ? 'יוצא' : 'נכנס'}
                    </Text>
                  </View>
                  <View>
                    <TouchableOpacity
              ...
              {showAlert && (
                <Animated.View style={{
                  ...
                  <View style={{
                    ...
                    {/* textAlign: 'right', */}
                    {/* }}> */}
                    {/* צ׳קים ללא מספר לא יופיעו במסך הצ׳קים, */}
                    {/* {'\n'} */}
                    {/* {'אלא '} */}
                    {/* <Text style={{ */}
                    {/* color: '#0f3860', */}
                              ...
                              <View style={{
                                ...
                                <TouchableOpacity
                                  ...
                                  <Text
                                    ...
                                    numberOfLines={1}
                                    ellipsizeMode='tail'>
                                    {AppTimezone.moment(c.dueDate).format('DD/MM/YY')}
                                  </Text>
                                </TouchableOpacity>
                              </View>
                              <View style={{
                  ...
                  <View style={[cs(!isRtl, commonStyles.row, [commonStyles.rowReverse]), {
                    ...
                    <View style={[{
                      ...
                      <View
                        ...
                        <Text style={[{
                          textAlign: 'right',
                          color: (!this.state.obj.biziboxMutavId) ? '#d0cece' : '#0f3860',
                          fontSize: sp(15),
                          lineHeight: 42,
                        }, commonStyles.regularFont]}>
                          {(this.state.obj.biziboxMutavId)
                            ? (this.state.biziboxMutavList && this.state.biziboxMutavList.length > 0 && this.state.biziboxMutavList.find((it) => it.biziboxMutavId === this.state.obj.biziboxMutavId) ? (this.state.biziboxMutavList.find((it) => it.biziboxMutavId === this.state.obj.biziboxMutavId).accountMutavName) : '')
                            : 'לא חובה'
                          }
                        </Text>
                      </View>
                  ...
                  <View
                    ...
                    <View style={{
                      ...
                      <TouchableOpacity
                        ...
                        onPress={this.handleOpenCategoriesModal(this.state.obj.transes[this.state.idxObj].transTypeId)}
                      >
                        <View style={{
                          marginRight: 'auto',
                        }}>
                    ...
                    <View style={{
                      ...
                      <TouchableOpacity
                        ...
                        <Text style={[{
                          ...
                          fontSize: sp(15),
                          lineHeight: 42,
                        }, commonStyles.regularFont]}>{AppTimezone.moment(this.state.obj.transes[this.state.idxObj].dueDate).format('DD/MM/YY')}</Text>
                      </TouchableOpacity>
                    </View>
                  </View>
                  <View
                    ...
                    <View style={{
                      ...
                      <TextInput
                        ...
                        onBlur={(e) => IS_IOS ? this.existingCheck(e.nativeEvent.text) : null}
                        onEndEditing={(e) => !IS_IOS ? this.existingCheck(e.nativeEvent.text) : null}
                        onChangeText={(asmachta) => {
                          let valuesSave = Object.assign({}, this.state.obj)
                          valuesSave.transes[this.state.idxObj].asmachta = asmachta
                          this.setState({ obj: valuesSave })
                          this.existingCheck(asmachta)
                        }}
                        value={this.state.obj.transes[this.state.idxObj].asmachta}
                        underlineColorAndroid='transparent'
                      />
                    </View>
                  </View>
                  {this.state.obj.transes[this.state.idxObj].asmachtaExist && (<View
                    style={[cs(!isRtl, commonStyles.row, [commonStyles.rowReverse]), {
                      marginBottom: 8,
                    }]}>
                    <View style={{ flex: 1.76, alignItems: 'flex-end' }} />
                    <View style={{
                      ...
                      <Text style={{
                        color: colors.red2,
                        fontSize: sp((getFormattedValueArray(this.state.obj.transes[this.state.idxObj].asmachtaExist.total)[0].toString().length > 4) ? 11 : 13),
                        lineHeight: 20,
                        textAlign: 'right',
                    ...
                    <View style={[{
                      ...
                      {this.state.editSum && (
                        <TextInput
                          ...
                          value={this.state.obj.transes[this.state.idxObj].total !== null ? String(this.state.obj.transes[this.state.idxObj].total) : ''}
                          underlineColorAndroid='transparent'
                        />
                      )}
                      ...
                      {!this.state.editSum && (
                        <TouchableOpacity
                          ...
                          {(this.state.obj.transes[this.state.idxObj].total !== null && this.state.obj.transes[this.state.idxObj].total !== '') && (
                            <Text
