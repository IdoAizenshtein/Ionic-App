...
class ScrollCompWithHeader extends PureComponent {
    ...
    constructor (props) {
      ...
      let currentIdx = 0
      if (statusTotals.length > 0 && selectedAccounts.length > 0 && !changeScreenModeInProgress && currentIndex) {
        const idddxxx = statusTotals.findIndex((it) => it.status.toLowerCase() === currentIndex)
    ...
    handleStartScroll = (e) => {
      ...
      if (numberOfPoints > 1) {
        const currentIndex = Math.ceil((winWidth + e.nativeEvent.contentOffset.x) / winWidth) - 1
        this.setState({
    ...
    render () {
      ...
      const showOperators = (screenSwitchState) ? (statusTotals.length > 2 && statusTotals[0].status.toLowerCase() === 'not_paid' && statusTotals[1].status.toLowerCase() === 'mechake_lehafkada' && statusTotals[2].status.toLowerCase() === 'mishmeret_babank') : (statusTotals.length > 2 && statusTotals[0].status === 'MECHAKE_LEHAFKADA' && statusTotals[1].status === 'FUTURE_DUE_DATE' && statusTotals[2].status === 'PAST_DUE_DATE')
      ...
      >
        <View>
          ...
          {!isSearchOpen && (
            <View>
                ...
                <View style={{ flex: 1 }}>
                  <TouchableOpacity onPress={this.setCheckTitleMarks(true)}>
                    <View style={{
                      ...
                      <Text style={[{
                        fontFamily: widthIncome === 0 ? fonts.regular : fonts.semiBold,
                        textAlign: 'center',
                        width: '100%',
                ...
                <View style={{ flex: 1 }}>
                  <TouchableOpacity onPress={this.setCheckTitleMarks(false)}>
                    <View style={{
                      ...
                      <Text style={[{
                        fontFamily: widthOut === 0 ? fonts.regular : fonts.semiBold,
                        textAlign: 'center',
                        width: '100%',
              ...
              {statusTotals.length > 0 && selectedAccounts.length > 0 && !changeScreenModeInProgress && (
                <View style={{
                        ...
                        <View key={i.toString()} style={{
                        ...
                        }}>
                          {
                            gr.map((c, i1) => {
                              ...
                              const redTotal = ((screenSwitchState && c.status.toLowerCase() === 'huavar_lesapak_vehazar') || (screenSwitchState && c.status.toLowerCase() === 'ufkad_vehazar') || (!screenSwitchState && c.status.toUpperCase() === 'RETURN_CHECK'))
                              const isActive = ((c.status.toLowerCase() === queryStatus) || (screenSwitchState && c.status.toLowerCase() === 'huavar_lesapak_vehazar' && queryStatus === 'ufkad_vehazar'))
                              return (
                                      ...
                                      <View>
                                        <Text
                                          style={[styles.dataValueWrapper, styles.dataValueWrapperLevel2, {
                                            ...
                                            lineHeight: 25,
                                          }, commonStyles.semiBoldFont, cs(isActive, {
                                            color: redTotal ? colors.red2 : '#fff',
                                          }, {
                                            color: redTotal ? colors.red2 : '#022258',
                                          })]} numberOfLines={1}
                                          ellipsizeMode='tail'>
                                          <Text style={{
                                            fontSize: sp(18),
          ...
          {!isSearchOpen && selectedAccountIds.length > 1 && !changeScreenModeInProgress && statusTotals.length > 0 && numberOfPoints > 1 && (
            <View>
              <Pagination
        ...
        {!isLoader && chequeDetails && (chequeDetails.length === 0) && (selectedAccounts.length > 0) && !error && (
          <View
            contentContainerStyle={[{
              flexGrow: 1,
              paddingTop: 0,
...
// KeyboardAwareSectionList is not working on Android
// https://github.com/APSL/react-native-keyboard-aware-scroll-view/issues/268
...
export default class DataList extends PureComponent {
  constructor (props) {
    ...
    this.state = {
      ...
      titleSticky: props.chequeDetails && props.chequeDetails.length > 0 ? props.chequeDetails[0].title : '',
    }
  }
    ...
    renderBankTransScrollItem = ({ item, index }) => {
      ...
      return (
        <CheckRow
          ...
          account={accountsILS.find(a => a.companyAccountId === item.companyAccountId)}
          isOpen={(querySearch && querySearch !== null && querySearch !== '') ? true : item.chequePaymentId === currentOpenItemIndex}
    ...
    componentWillReceiveProps ({ screenSwitchState, currentSelectedAccountId }) {
    ...
    render () {
      ...
      return (<AnimatedSectionList
        ...
        sections={(chequeDetails && chequeDetails.length > 0) ? chequeDetails : []}
