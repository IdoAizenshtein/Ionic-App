...
export default class BankAccountsHeader extends PureComponent {
  ...
  componentWillReceiveProps (props) {
    if (this.scrollViewTop && this.scrollViewTop._component &&
      ((props.categoriesMatchArr && (props.categoriesMatchArr !== this.props.categoriesMatchArr)) ||
        (props.statusListArr && (props.statusListArr !== this.props.statusListArr)) ||
        (props.programNameListArr && (props.programNameListArr !== this.props.programNameListArr)))
    ) {
      setTimeout(() => {
        if (this.scrollViewTop && this.scrollViewTop._component) {
          if (this.props.isRtl) {
  ...
  render () {
    ...
    const opacityTitlesX = scrollAnim.interpolate({
      inputRange: [0, (headerScrollDistance && yFixed < headerScrollDistance) ? yFixed : 0, headerScrollDistance + 50],
      outputRange: [0, 0, 1],
      extrapolate: 'clamp',
    })
            ...
            <Animated.View style={{ elevation: 6, zIndex: 6 }}>
              ...
              <Animated.View style={[cs(isRtl, styles.bankAccountHeaderWrapper, commonStyles.rowReverse),
                ...
                <TouchableOpacity
                  ...
                  {!isDefDates && dateFromTimestamp && dateTillTimestamp && AppTimezone.moment().startOf('month').isSame(dateFromTimestamp, 'day') && (
                    <Text style={[styles.bankAccountHeaderText, {
                      color: colors.blue32,
                    }]}>{t('bankAccount:lastMonthDays')}</Text>
                  )}
                  {!isDefDates && dateFromTimestamp && dateTillTimestamp && (AppTimezone.moment(dateTillTimestamp).subtract(2, 'month').isSame(dateFromTimestamp, 'day')) && (
                    <Text style={[styles.bankAccountHeaderText, {
                      color: colors.blue32,
                    }]}>{t('bankAccount:lastTwoMonthsDays')}</Text>
                  )}
                  {!isDefDates && dateFromTimestamp && dateTillTimestamp && AppTimezone.moment(dateTillTimestamp).subtract(3, 'month').isSame(dateFromTimestamp, 'day') && (
                    <Text style={[styles.bankAccountHeaderText, {
                      color: colors.blue32,
                    }]}>{t('bankAccount:lastThreeMonthsDays')}</Text>
                  )}

                  {!isDefDates && dateFromTimestamp && dateTillTimestamp && !AppTimezone.moment().startOf('month').isSame(dateFromTimestamp, 'day') && !AppTimezone.moment(dateTillTimestamp).subtract(2, 'month').isSame(dateFromTimestamp, 'day') && !AppTimezone.moment(dateTillTimestamp).subtract(3, 'month').isSame(dateFromTimestamp, 'day') && (
                    <Text style={[styles.bankAccountHeaderText, {
                      color: colors.blue32,
                    }]}>{AppTimezone.moment(dateFromTimestamp).format('DD/MM/YY')} - {AppTimezone.moment(dateTillTimestamp).format('DD/MM/YY')}</Text>
                  )}
              ...
              {(isEdit) && (
                <View style={{
                  ...
                  >
                    ...
                    {categoriesMatchArr.length > 0 && categoriesMatchArr.map((f, i) => {
                      return (
                        <TouchableOpacity
                          key={i.toString()}
                          style={{
                    ...
                    {programNameListArr.length > 0 && programNameListArr.map((f, i) => {
                      return (
                        <TouchableOpacity
                          key={i.toString()}
                          style={{
