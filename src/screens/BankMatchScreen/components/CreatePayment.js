...
export default class CreatePayment extends PureComponent {
    ...
    constructor (props) {
      ...
      props.dataTransaction.autoUpdateTypeName = 'AVG_3_MONTHS'

      if (props.dataTransaction.mutavArray && Array.isArray(props.dataTransaction.mutavArray) && props.dataTransaction.mutavArray.length) {
        const mutavArray = props.dataTransaction.mutavArray.filter((item) => item.biziboxMutavId)
        props.dataTransaction.mutavArray = mutavArray.map((it) => {
          return {
            total: null,
        ...
        props.dataTransaction.mutavArray = [
          {
            total: null,
            biziboxMutavId: null,
            transTypeId: {
      ...
      this.state = {
        ...
        disabledVal: (targetType) ? this.getRulesEdit(targetType) : {
          ...
          paymentDesc: false,
          transDate: false,
          total: targetType === 'CYCLIC_TRANS' && props.dataTransaction.autoUpdateTypeName === 'AVG_3_MONTHS',
          transFrequencyName: targetType === 'CYCLIC_TRANS' && props.dataTransaction.autoUpdateTypeName === 'AVG_3_MONTHS',
        },
        obj: (props.dataTransaction),
        expirationDate: null,
    ...
    componentDidMount () {
      getMutavApi.post({ body: { uuid: this.props.currentCompanyId } })
        .then(data => {
          ...
          setTimeout(() => {
            const mutavArray = this.state.obj.mutavArray
    ...
    handleToggleCheckBox = () => {
      ...
      obj.autoUpdateTypeName = obj.autoUpdateTypeName ? 'USER_DEFINED_TOTAL' : 'AVG_3_MONTHS'
    ...
    modalMutavClose = () => {
      ...
      if (mutavArray.length === 1 && mutavArray[0].biziboxMutavId !== null) {
        ...
        const allSums = mutavArray.reduce((memo, it) => {
          return memo + Number(it.total)
        }, 0)
        valuesSave.total = allSums
