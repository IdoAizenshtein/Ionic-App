...
import { banktransForMatch, cashflowMatch, getBankMatchAccount } from '../../redux/actions/bankMatch'
  ...
  removeFromMatches = (item, isBanktransForMatchData) => {
    ...
    if (isBanktransForMatchData) {
      ...
      if (typeExpece.length) {
        cashflowMatchForMatchTemp = cashflowMatchForMatch.filter((item) => {
          return (typeExpece.some((row) => row === item.expence))
        })
      }
      ...
      if (typeExpece.length && typeIdsExist.length) {
        cashflowMatchDataOrigin = cashflowMatchDataOrigin.filter((item) => {
          return ((typeExpece.some((row) => row === item.expence)) && (typeIdsExist.every((row) => row !== row.targetId)))
        })
      }
  ...
  pressItem = (type, item) => {
    ...
    if (type) {
      if (cashflowMatchForMatch.length < 2 || banktransForMatchForMatch.length === 0) {
        ...
        const typeExpece = Object.values(objectTypeExpece)
        const objectIdsExist = {}
        const cashflowMatchForMatchTemp = cashflowMatchForMatch.filter((item) => {
          return (typeExpece.some((row) => row === item.expence))
        })
        cashflowMatchForMatchTemp.forEach((thisItem) => {
          objectIdsExist[thisItem.targetId] = thisItem.targetId
        ...
        let cashflowMatchDataStateArr = cashflowMatchDataOrigin.filter((item) => {
          return (typeExpece.some((row) => row === item.expence) && typeIdsExist.every((row) => row !== row.targetId))
        })

        let arrContainerCash = []
        typePaymentDesc.forEach((type) => {
          arrContainerCash = arrContainerCash.concat(cashflowMatchDataStateArr.filter((items) => items.paymentDesc === type))
        })
        cashflowMatchDataStateArr = arrContainerCash.concat(cashflowMatchDataStateArr.filter((type) => typePaymentDesc.every((row) => row !== type.paymentDesc)))
  ...
  render () {
    ...
    return (
      <View style={commonStyles.mainContainer}>
        <AlertsTrial onlyShowPopUp navigation={navigation} refresh={this._onRefresh}
