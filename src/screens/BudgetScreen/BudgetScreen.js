...
export default class BudgetScreen extends PureComponent {
  ...
  constructor (props) {
    ...
    this.props.navigation.setParams({ openedBottomSheet: false })
  ...
  get accountTitle () {
    ...
    const trimTitle = (title, currency) => {
      ...
      if (newTitle.length > 20) return `${title.slice(0, 18)}... ${currency}`
      return newTitle
    }
    ...
    if (this.selectedAccounts.length === 1) {
      return (
        <View
          style={cs(this.props.isRtl, [commonStyles.row, commonStyles.alignItemsCenter], commonStyles.rowReverse)}
        >
          <AccountIcon account={account} />
          <Text
            style={[styles.bankAccountHeaderText]}>{trimTitle(account.accountNickname, currencySign)}</Text>
        </View>
      )
    }

    const title = this.selectedAccounts.length === accountGroups[selectedGroup].length
    ...
    return (
      ...
      >
        <Text style={[styles.bankAccountHeaderText]}>{trimTitle(title, currencySign)}</Text>
      </View>
    )
  }
    ...
    if (currentCompanyId) {
      ...
      getAccountCflTransTypeApi.post({ body: { uuid: currentCompanyId } })
        .then(categories => {
          ...
          const accountsNotUpdatesOldestTransDate = JSON.parse(JSON.stringify(accountsNotUpdates))
        ...
        if (dataBudgets.length === 0) {
          ...
          if (dataBudgets.length && dataBudgets[0].budgetId) {
            dataBudgets.forEach((budget) => {
              ...
              if (this.props.accounts.find(a => a.companyAccountId === budget.budgetAccounts[0])) {
                budget.currency = getCurrencyChar(this.props.accounts.find(a => a.companyAccountId === budget.budgetAccounts[0]).currency)
              } else {
            ...
            if (isSaveBudgetId && dataBudgets.find((it) => it.budgetId === isSaveBudgetId)) {
              this.budgetActive(dataBudgets.find((it) => it.budgetId === isSaveBudgetId), true)
            } else {
  ...
  budgetActive = async (budget, isActive, isOpenPopupCreate, isOpenPopupEdit) => {
    ...
    if (((!isActive && budget.showPanelDD) || isActive || isOpenPopupCreate) && !budget.budgetsDetails) {
      const { currentCompanyId } = this.props
      const response = await new Api({ endpoint: `budget/cfl/${budget.budgetType}/get-budget-details` }).post({
        body: {
      ...
      dataBudgets.find((bud) => budget.budgetId === bud.budgetId).budgetsDetails = response
      this.setState({
        dataBudgets,
        actionSheetTooltipCopy: dataBudgets.find((bud) => budget.budgetId === bud.budgetId),
      }, () => {
        ...
        if (!isOpenPopupCreate) {
          ...
          this.openPopUpAddEditDuplication(isOpenPopupEdit ? 'edit' : 'duplication')
        }
      })
    ...
    if (this.state.accountsNotUpdatesOldestTransDate) {
      if (item.frequencyDesc === 'MONTH') {
        ...
        if (this.state.accountsNotUpdatesOldestTransDate.length) {
          monthDiffOldestTransDate = AppTimezone.moment(item.dates.dateFrom).diff(
            AppTimezone.moment(this.state.accountsNotUpdatesOldestTransDate[0].oldestTransDate),
            'months')
        }
        if (type === 'prev') {
        ...
        if (this.state.accountsNotUpdatesOldestTransDate.length) {
          yearsDiffOldestTransDate = AppTimezone.moment(item.dates.dateFrom).diff(
            AppTimezone.moment(this.state.accountsNotUpdatesOldestTransDate[0].oldestTransDate),
            'years')
        }
        if (type === 'prev') {
    ...
    if (!item.budgetsDetails) {
      if (item.frequencyDesc === 'MONTH') {
        ...
        if ((monthDiff <= 11) && (monthDiff <= monthDiffOldestTransDate)) {
          item.dateFrom = AppTimezone.moment(item.dateFrom).subtract(1, 'months').startOf('month').valueOf()
          item.dateTill = AppTimezone.moment(item.dateTill).subtract(1, 'months').endOf('month').valueOf()
        } else {
        ...
        if ((yearsDiff <= 3) && (yearsDiff <= yearsDiffOldestTransDate)) {
          item.dateFrom = AppTimezone.moment(item.dateFrom).subtract(1, 'years').startOf('month').valueOf()
          item.dateTill = AppTimezone.moment(item.dateTill).subtract(1, 'years').endOf('month').valueOf()
        } else {
          this.budgetActive(item, true)
          return
        }
  ...
  nextDate = (item) => () => {
    ...
    if (!item.budgetsDetails) {
      if (item.frequencyDesc === 'MONTH') {
        ...
        if (monthDiff > 0) {
          item.dateFrom =
            AppTimezone.moment(item.dateFrom).add(1, 'months').startOf('month').valueOf()
          item.dateTill =
            AppTimezone.moment(item.dateTill).add(1, 'months').endOf('month').valueOf()
        } else {
          this.budgetActive(item, true)
          return
        }
        ...
        if (yearsDiff > 0) {
          item.dateFrom =
            AppTimezone.moment(item.dateFrom).add(1, 'years').startOf('month').valueOf()
          item.dateTill =
            AppTimezone.moment(item.dateTill).add(1, 'years').endOf('month').valueOf()
        } else {
          this.budgetActive(item, true)
          return
        }
  ...
  handleItemToggle = (budgetSrc) => () => {
    const { currentOpenItemIndexSlideInside } = this.state
    this.setState({ currentOpenItemIndexSlideInside: currentOpenItemIndexSlideInside === budgetSrc.budgetId ? null : budgetSrc.budgetId })
  ...
  openActionSheet = (type, item) => {
    if (type && (type === 'incomeAlert' || type === 'outcomeAlert')) {
      this.setState({
        actionSheetTooltip: type === 'incomeAlert' ? 'קבלת התראות עבור תקציב הכנסות לאחר' : 'קבלת התראות עבור תקציב הוצאות לאחר',
        actionSheetItemList: [{}],
        alertsItem: Object.assign({}, item),
        alertsItemType: type,
        prcAlertSetVal: type === 'incomeAlert' ? item.prcIncome : item.prcOutcome,
      }, () => {
        setTimeout(() => {
      ...
      this.setState({
        ...
        actionSheetItemList: actionSheetItemList.length ? actionSheetItemList : [{}],
        alertsItem: false,
        alertsItemType: false,
        prcAlertSetVal: 0,
      }, () => {
  ...
  deleteBudgetTrans = () => {
    ...
    return deleteBudgetTransApi.post({
      ...
      categoryHistory.history.transes = categoryHistory.history.transes.filter((it) => it.transId !== val.transId)
      categoryHistory.history.transesTotal = categoryHistory.history.transes.reduce((a, b) => a + b.total, 0)
      categoryHistory.history.monthsTotal.forEach((it) => {
        const monthDiff = AppTimezone.moment(it.month).isSame(AppTimezone.moment(val.transDate), 'month')
        if (monthDiff) {
      ...
      categoryHistory.history.currentMonth.forEach((it) => {
        if (AppTimezone.moment().isSame(val.transDate, 'month')) {
          const monthDiff = AppTimezone.moment(val.transDate).format('D') === ((it.transDay).toString())
          if (monthDiff) {
      ...
      const idxSlices = categoryHistory.history.monthsTotal.findIndex((v) => {
        return AppTimezone.moment(v.month).isBefore(AppTimezone.moment(), 'month') && v.total !== 0
      })
      const arrOldMonths = categoryHistory.history.monthsTotal.slice(idxSlices, categoryHistory.history.monthsTotal.length - 1)
      if (arrOldMonths.length) {
        const averageTotal = arrOldMonths.reduce((a, b) => a + b.total, 0)
        categoryHistory.history.average = (averageTotal / arrOldMonths.length).toFixed(1)
      }
      this.setState({
        categoryHistory,
        categoriesModalIsOpen: false,
  ...
  onDrawerSnap = (states) => {
    ...
    if (index === 0) {
      this.props.navigation.setParams({ openedBottomSheet: true })
      // console.log('Top')
      this.setState({
      ...
      if (this.numLengOrgHistory) {
        const itemPanelOfCategories = Object.assign({}, this.state.itemPanelOfCategories)
  ...
  showPanelOfCategories = (itemPanelOfCategories) => () => {
    ...
    if (itemPanelOfCategories && itemPanelOfCategories.budgetsDetails && itemPanelOfCategories.budgetsDetails.details.length > 0) {
      const expences = itemPanelOfCategories.budgetsDetails.details.filter((it) => {
        const nameIcon = this.state.categories.find((item) => item.transTypeId === it.keyId)
        if (nameIcon) {
          it.nameIcon = getTransCategoryIcon(nameIcon.iconType)
        }
        if (it.expence) {
          if ((it.total === 0 && it.totalKeyUse !== 0) || (Math.round(it.totalKeyUse / (it.total / 100)) <= 33.999999)) {
            it.bgColor = styles.red1
          } else if (!(it.total === 0 && it.totalKeyUse !== 0) && Math.round(it.totalKeyUse / (it.total / 100)) >= 34 && Math.round(it.totalKeyUse / (it.total / 100)) <= 66.999999) {
            it.bgColor = styles.red2
          } else if (!(it.total === 0 && it.totalKeyUse !== 0) && Math.round(it.totalKeyUse / (it.total / 100)) >= 67 && Math.round(it.totalKeyUse / (it.total / 100)) <= 99.999999) {
            it.bgColor = styles.red3
          } else if (!(it.total === 0 && it.totalKeyUse !== 0) && Math.round(it.totalKeyUse / (it.total / 100)) >= 100) {
      ...
      const incomes = itemPanelOfCategories.budgetsDetails.details.filter((it) => {
        const nameIcon = this.state.categories.find((item) => item.transTypeId === it.keyId)
        if (nameIcon) {
          it.nameIcon = getTransCategoryIcon(nameIcon.iconType)
        }
      ...
      const totals = itemPanelOfCategories.budgetsDetails.details.reduce((a, it) => {
        if (it.deleted) {
          return [a[0], a[1]]
        } else {
          if (it.expence) {
          ...
          if (this.state.budgetActive.budgetId === bud.budgetId) {
            if (isAlert) {
              bud.budgetsDetails.details.forEach((it) => {
                if (it.expence === categoryHistory.expence && it.keyId === categoryHistory.keyId) {
  ...
  onValueCompleteChange = (val) => {
    ...
    alertsItem[this.state.alertsItemType === 'incomeAlert' ? 'prcIncome' : 'prcOutcome'] = Math.round(val)
    ...
    this.setState({
      ...
      if (this.state.categoryHistory) {
        ...
        this.setState({
          ...
          const categoryHistory = Object.assign({}, this.state.categoryHistory)
          if (!categoryHistory.alertPrc) {
            categoryHistory.alertPrc = 0
          }
          this.updateBudgetDetails(categoryHistory, true)
        })
      } else {
        const alertsItem = Object.assign({}, this.state.alertsItem)

        console.log({
          'budgetId': alertsItem.budgetId,
          'indDefault': alertsItem.indDefault,
          'incomeAlert': this.state.alertsItemType === 'incomeAlert' ? Number(alertsItem.prcIncome) > 0 : alertsItem.incomeAlert,
          'outcomeAlert': this.state.alertsItemType === 'outcomeAlert' ? Number(alertsItem.prcOutcome) > 0 : alertsItem.outcomeAlert,
          'prcIncome': Number(alertsItem.prcIncome),
          'prcOutcome': Number(alertsItem.prcOutcome),
        })
        ...
        updateBudgetPrcApi.post({
          ...
          .then(response => {
            const dataBudgets = JSON.parse(JSON.stringify(this.state.dataBudgets))
            dataBudgets.forEach((bud) => {
              if (alertsItem.budgetId === bud.budgetId) {
                if (this.state.alertsItemType === 'incomeAlert') {
                  bud.prcIncome = Number(alertsItem.prcIncome)
                  ...
                  bud.prcOutcome = Number(alertsItem.prcOutcome)
    ...
    this.setState({
      ...
      keyHistoryApi.post({
        ...
        .then(response => {
          ...
          console.log(response)
          categoryHistory.history.monthsTotal = categoryHistory.history.monthsTotal.reverse().map((it, idx) => {
            const d = new Date()
            d.setMonth((categoryHistory.history.monthsTotal[0].monthNumber) - 1)
            return {
              month: AppTimezone.moment(d).subtract((idx), 'months').startOf('month').valueOf(),
              total: it.monthTotal,
            }
          })
          this.setState({
  ...
  handleUpdateTotalCategory = (total) => {
    ...
    if (setTotalCategoryInProgress) return
    const categoryHistory = Object.assign({}, this.state.categoryHistory)
  ...
  openPopUpAddEditDuplication = (type) => {
    this.setState({
      ...
      const dataPopUpAddEditDuplication = {
        ...
        'budgetTotalType': type === 'add' ? 'outcome' : actionSheetTooltipCopy.budgetTotalType,
        'budgetType': 'CATEGORY',
        'companyAccountIds': accountSelect.map((acc) => acc.companyAccountId),
        'companyId': currentCompanyId,
        'creditCardCalcTypeDesc': type === 'add' ? 'CALC' : actionSheetTooltipCopy ? actionSheetTooltipCopy.creditCardCalcTypeDesc : 'CALC',
        'dateFrom': type === 'add' ? AppTimezone.moment().toDate() : (actionSheetTooltipCopy ? AppTimezone.moment(actionSheetTooltipCopy.dateFrom) : AppTimezone.moment().toDate()),
        'dateTill': type === 'add' ? AppTimezone.moment().toDate() : (actionSheetTooltipCopy ? AppTimezone.moment(actionSheetTooltipCopy.dateTill) : AppTimezone.moment().toDate()),
        'frequencyDesc': type === 'add' ? 'MONTH' : actionSheetTooltipCopy ? actionSheetTooltipCopy.frequencyDesc : 'MONTH',
        'totalIncome': type === 'add' ? '' : actionSheetTooltipCopy ? actionSheetTooltipCopy.totalIncome : '',
        'totalOutcome': type === 'add' ? '' : actionSheetTooltipCopy ? actionSheetTooltipCopy.totalOutcome : '',
        'keyDetails': type === 'add' ? [] : actionSheetTooltipCopy ? actionSheetTooltipCopy.budgetsDetails.details.map((it) => {
          return {
            'expence': it.expence,
            'keyId': it.keyId,
            'keyName': it.keyName,
      ...
      if (type === 'edit' && actionSheetTooltipCopy) {
        disableEditPopPast = (actionSheetTooltipCopy.frequencyDesc === 'MONTH' || actionSheetTooltipCopy.frequencyDesc === 'YEAR')
      ...
      this.setState({
        typeOfPopup: type,
        selectedAccountIdsSave: JSON.parse(JSON.stringify(accountSelect.map((acc) => acc.companyAccountId))),
        selectedGroupSave: accountSelect[0].currency.toLowerCase(),
        selectedAccountIds: JSON.parse(JSON.stringify(accountSelect.map((acc) => acc.companyAccountId))),
        selectedGroup: accountSelect[0].currency.toLowerCase(),
        disabledValuesPopUp: {
          budgetTotalType: (disableEditPopPast || (type === 'edit' && !actionSheetTooltipCopy) || (type === 'duplication' && actionSheetTooltipCopy && actionSheetTooltipCopy.frequencyDesc !== 'ONE_TIME')),
          budgetType: true,
        },
        ...
        ddCategories: [],
        copySourceKeyDetails: (actionSheetTooltipCopy && type === 'edit') ? actionSheetTooltipCopy.budgetsDetails.details.map((it) => {
          return {
            'expence': it.expence,
            'keyId': it.keyId,
            'keyName': it.keyName,
  ...
  calculateCategories = (isAddCateg) => {
    ...
    if (keyDetails) {
      ddCategories.push(...keyDetails
        .filter(det => !ddCategories.some(cat => cat.transTypeId === det.keyId && cat.hova === det.expence))
        .map(det => {
          return {
            hova: det.expence,
            add: false,
      ...
      ddCategories.forEach((it) => {
        if (keyDetails.find((id) => id.keyId === it.transTypeId && it.hova === id.expence)) {
          it.press = true

          if (budgetTotalType === 'both') {
            const theOtherTypeTrans = ddCategories.find((item) => item.transTypeId === it.transTypeId && item.hova === !it.hova)
    ...
    if (budgetTotalType === 'both') {
      this.setState({
        copyTransTypesIncome: companyTransTypes.filter((cat) => {
          return !incomes.find((ddCat) => cat.transTypeId === ddCat.transTypeId) && !outcome.find((ddCat) => (cat.transTypeId === ddCat.transTypeId) && ddCat.press)
        }),
        copyTransTypesExpence: companyTransTypes.filter((cat) => {
          return !outcome.find((ddCat) => cat.transTypeId === ddCat.transTypeId) && !incomes.find((ddCat) => (cat.transTypeId === ddCat.transTypeId) && ddCat.press)
        }),
      })
      incomes.sort((a, b) => {
        const lblA = a.transTypeName
        const lblB = b.transTypeName
        return (lblA || lblB) ? (!lblA ? 1 : !lblB ? -1 : lblA.localeCompare(lblB)) : 0
      })
      incomes.push({
        hova: false,
        add: true,
      ...
      this.setState({
        ...
        const lengthSlides = Array.from({ length: Math.ceil(this.state.ddCategories.length / 6) }, (v, i) => i)
        // debugger
        // console.log('this.state.ddCategories', this.state.ddCategories, lengthSlides, isAddCateg)

        this.setState({
          ddCategoriesSource: JSON.parse(JSON.stringify(this.state.ddCategories)),
          lengthSlides: lengthSlides,
          activeSlide: isAddCateg ? this.state.activeSlide : (lengthSlides.length - 1),
        })
      })
  ...
  pressCategory = (category) => () => {
    if (!category.add) {
      ...
      ddCategories.find((a) => a.hova === category.hova && a.transTypeId === category.transTypeId).press = !category.press

      let dataPopUpAddEditDuplication = Object.assign({}, this.state.dataPopUpAddEditDuplication)
      if (dataPopUpAddEditDuplication['budgetTotalType'] === 'both') {
        const theOtherTypeTrans = ddCategories.find((it) => it.transTypeId === category.transTypeId && it.hova === !category.hova)
        if (theOtherTypeTrans) {
          theOtherTypeTrans.disabled = !category.press
        }

        const categoriesMatch = category.hova ? JSON.parse(JSON.stringify(this.state.copyTransTypesIncome)) : JSON.parse(JSON.stringify(this.state.copyTransTypesExpence))
        const copyTransTypes = categoriesMatch.find((it) => it.transTypeId === category.transTypeId)
        if (copyTransTypes) {
      ...
      dataPopUpAddEditDuplication['keyDetails'] = ddCategories.filter((cate) => (!cate.add && cate.press))
        .map((it) => {
          return {
            'expence': it.hova,
      ...
      if (this.state.typeOfPopup === 'edit') {
        const isEqual = this.state.copySourceKeyDetails.filter(o =>
          dataPopUpAddEditDuplication['keyDetails'].some(v => v.keyId === o.keyId && v.expence === o.expence))
        this.setState({
          isKeyDetailsNotEqual: (isEqual.length !== this.state.copySourceKeyDetails.length),
        })
      }
    } else {
      const categoriesMatch = category.hova ? this.state.copyTransTypesExpence.filter((it) => !it.hide) : this.state.copyTransTypesIncome.filter((it) => !it.hide)
      this.setState({
  ...
  handleSaveCategories = () => {
    ...
    const addCat = categoriesMatchCopy.filter((it) => it.press).map((item) => {
      ...
      if (dataPopUpAddEditDuplication['budgetTotalType'] === 'both') {
        const theOtherTypeTrans = ddCategories.find((it) => it.transTypeId === item.transTypeId && it.hova === !hova)
        if (theOtherTypeTrans) {
          theOtherTypeTrans.disabled = true
        }
    ...
    dataPopUpAddEditDuplication['keyDetails'] = dataPopUpAddEditDuplication['keyDetails'].concat(addCat
      .map((it) => {
        return {
          'expence': it.hova,
          'keyId': it.transTypeId,
            ...
            <Fragment key={a.transTypeId + a.hova.toString()}>
              ...
              {!a.disabled && (
                <TouchableOpacity
                  ...
                  style={[{
                    height: 26.5,
                    flex: a.transTypeName && a.transTypeName.length ? (a.add ? (a.transTypeName.length + 0.2) : a.transTypeName.length) : 0,
                    borderWidth: a.add ? 2 : 1,
                    borderColor: a.hova ? '#cd1010' : '#229f88',
                    marginBottom: 7,
                    backgroundColor: a.press ? (a.hova ? '#cd1010' : '#229f88') : '#ffffff',
                    flexDirection: 'row-reverse',
                    justifyContent: 'center',
                    alignItems: 'center',
                    alignContent: 'center',
                ...
                >
                  ...
                  <Text style={{
                    ...
                    lineHeight: 17,
                    fontSize: sp(17),
                    color: a.press ? '#ffffff' : (a.add ? (a.hova ? '#cd1010' : '#229f88') : '#022258'),
  ...
  savePopUp = () => {
    ...
    if (dataPopUpAddEditDuplication.budgetTotalType === 'income') {
      ...
      dataPopUpAddEditDuplication.totalIncome = Number(dataPopUpAddEditDuplication.totalIncome)
      ...
      dataPopUpAddEditDuplication.totalOutcome = Number(dataPopUpAddEditDuplication.totalOutcome)
      dataPopUpAddEditDuplication.keyDetails = dataPopUpAddEditDuplication.keyDetails
        .filter(kd => kd.expence === true)
    } else {
      dataPopUpAddEditDuplication.totalOutcome = Number(dataPopUpAddEditDuplication.totalOutcome)
    ...
    try {
      if (
        (dataPopUpAddEditDuplication['budgetTotalType'] === 'both' && ((!dataPopUpAddEditDuplication.totalIncome || (dataPopUpAddEditDuplication.totalIncome && !dataPopUpAddEditDuplication.totalIncome.toString().length)) || (!dataPopUpAddEditDuplication.totalOutcome || (dataPopUpAddEditDuplication.totalOutcome && !dataPopUpAddEditDuplication.totalOutcome.toString().length)))) ||
        (dataPopUpAddEditDuplication['budgetTotalType'] === 'income' &&
          (!dataPopUpAddEditDuplication.totalIncome || (dataPopUpAddEditDuplication.totalIncome && !dataPopUpAddEditDuplication.totalIncome.toString().length))
        ) ||
        (dataPopUpAddEditDuplication['budgetTotalType'] === 'outcome' &&
          (!dataPopUpAddEditDuplication.totalOutcome || (dataPopUpAddEditDuplication.totalOutcome && !dataPopUpAddEditDuplication.totalOutcome.toString().length))
        ) ||
        (!dataPopUpAddEditDuplication.budgetName || (dataPopUpAddEditDuplication.budgetName && !dataPopUpAddEditDuplication.budgetName.length)) ||
    ...
    this.setState({ hasError: false, dataPopUpAddEditDuplication: dataPopUpAddEditDuplication }, () => {
    ...
    if (param === 'budgetTotalType') {
      const types = this.state.ddBudgetTotalType.map((item) => {
        return {
          text: item.label,
          id: item.value,
          selected: (item.value === this.state.dataPopUpAddEditDuplication.budgetTotalType),
        }
      })
      this.setState({
      ...
      const types = this.state.ddBudgetType.map((item) => {
        return {
          text: item.label,
          id: item.value,
          selected: (item.value === this.state.dataPopUpAddEditDuplication.budgetType),
        }
      })
      this.setState({
      ...
      const types = this.state.ddFrequencyDesc.map((item) => {
        return {
          text: item.label,
          id: item.value,
          selected: (item.value === this.state.dataPopUpAddEditDuplication.frequencyDesc),
        }
      })
      this.setState({
  ...
  handleToggleCheckBox = (type) => () => {
    let dataPopUpAddEditDuplication = Object.assign({}, this.state.dataPopUpAddEditDuplication)
    dataPopUpAddEditDuplication['creditCardCalcTypeDesc'] = this.state.dataPopUpAddEditDuplication.creditCardCalcTypeDesc === this.state.ddCreditCardCalcTypeDesc[0].value ? this.state.ddCreditCardCalcTypeDesc[1].value : this.state.ddCreditCardCalcTypeDesc[0].value
    this.setState({ dataPopUpAddEditDuplication: dataPopUpAddEditDuplication })
  }
  ...
  openPopUpCreateBudgetTrans = (isEdit) => {
    ...
    this.setState({
      ...
      popUpCreateBudgetTrans: {
        'companyId': currentCompanyId,
        'transDate': isEdit ? itemTransFromHistory.transDate : AppTimezone.moment().startOf('day').valueOf(),
        'expence': categoryHistory.expence,
        'transName': isEdit ? itemTransFromHistory.transName : (categoryHistory.expence ? 'הוצאה' : 'הכנסה'),
  ...
  savePopUpCreateBudgetTrans = () => {
    ...
    if (popUpCreateBudgetTrans.transTotal === null || popUpCreateBudgetTrans.transTotal === '' || popUpCreateBudgetTrans.transTotal === 0 || popUpCreateBudgetTrans.transName === '') {
      return this.setState({
        totalNoValid: (popUpCreateBudgetTrans.transTotal === null || popUpCreateBudgetTrans.transTotal === '' || popUpCreateBudgetTrans.transTotal === 0),
        hasErrorCreateBudgetTrans: true,
      })
    }

    const copyPopUpCreateBudgetTrans = Object.assign({}, popUpCreateBudgetTrans)
    copyPopUpCreateBudgetTrans.transTotal = Number(copyPopUpCreateBudgetTrans.transTotal)
    const categoryHistory = Object.assign({}, this.state.categoryHistory)
    console.log(copyPopUpCreateBudgetTrans, categoryHistory)
    const sender = popUpCreateBudgetTransEdit ? updateBudgetTransApi : createBudgetTransApi
    sender.post({
      ...
      .then(response => {
        ...
        if (!popUpCreateBudgetTransEdit) {
          ...
          const monthDiffThisMonth = AppTimezone.moment().isSame(popUpCreateBudgetTrans.transDate, 'month')
          if (monthDiffThisMonth) {
            // this.transCalc += copyPopUpCreateBudgetTrans.transTotal
          }
          categoryHistory.history.monthsTotal.forEach((it1) => {
            const monthDiff = AppTimezone.moment(it1.month).isSame(popUpCreateBudgetTrans.transDate, 'month')
            if (monthDiff) {
              it1.total = it1.total + copyPopUpCreateBudgetTrans.transTotal
            }
          })
          categoryHistory.history.currentMonth.forEach((it1) => {
            if (AppTimezone.moment().isSame(popUpCreateBudgetTrans.transDate, 'month')) {
              const monthDiff = AppTimezone.moment(popUpCreateBudgetTrans.transDate).format('D') === ((it1.transDay).toString())
              if (monthDiff) {
                it1.dayTotal = it1.dayTotal + copyPopUpCreateBudgetTrans.transTotal
              }
            }
          })
          categoryHistory.history.transes = categoryHistory.history.transes.filter((ite) => ite.keyId === categoryHistory.keyId)
          categoryHistory.history.transesTotal = categoryHistory.history.transes.reduce((a, b) => a + b.total, 0)
          const idxSlices = categoryHistory.history.monthsTotal.findIndex((v) => {
            return AppTimezone.moment(v.month).isBefore(AppTimezone.moment(), 'month') && v.total !== 0
          })
          ...
          if (arrOldMonths.length) {
            const averageTotal = arrOldMonths.reduce((a, b) => a + b.total, 0)
            categoryHistory.history.average = Number((averageTotal / arrOldMonths.length).toFixed(1))
          }
        } else {
          let totalDefDaily
          let totalMinusDate = null
          let totalDef
          if (Number(itemTransFromHistory.total) !== Number(copyPopUpCreateBudgetTrans.transTotal)) {
            if (AppTimezone.moment().isSame(AppTimezone.moment(copyPopUpCreateBudgetTrans.transDate), 'month')) {
              totalDefDaily = Number(copyPopUpCreateBudgetTrans.transTotal) - Number(itemTransFromHistory.total)
            }
          } else {
            if (!AppTimezone.moment().isSame(AppTimezone.moment(copyPopUpCreateBudgetTrans.transDate), 'month')) {
              totalDefDaily = -Number(copyPopUpCreateBudgetTrans.transTotal)
            } else {
              if (AppTimezone.moment(copyPopUpCreateBudgetTrans.transDate).isSame(AppTimezone.moment(itemTransFromHistory.transDate), 'month')) {
                if (!AppTimezone.moment(copyPopUpCreateBudgetTrans.transDate).isSame(AppTimezone.moment(itemTransFromHistory.transDate), 'day')) {
          ...
          const monthDefMonths =
            AppTimezone.moment(copyPopUpCreateBudgetTrans.transDate).isSame(AppTimezone.moment(itemTransFromHistory.transDate), 'month')
          if (monthDefMonths) {
            totalDef = Number(copyPopUpCreateBudgetTrans.transTotal) - Number(itemTransFromHistory.total)
            if (AppTimezone.moment().isSame(AppTimezone.moment(copyPopUpCreateBudgetTrans.transDate), 'month')) {
            ...
            const monthDefThisMonth =
              AppTimezone.moment().isSame(AppTimezone.moment(itemTransFromHistory.transDate), 'month')
            if (monthDefThisMonth) {
              // this.budgetHistoryPrompt.transCalc -= Number(copyPopUpCreateBudgetTrans.transTotal);
            } else {
              const isThisMonth =
                AppTimezone.moment().isSame(AppTimezone.moment(copyPopUpCreateBudgetTrans.transDate), 'month')
              if (isThisMonth) {
                // this.budgetHistoryPrompt.transCalc += Number(copyPopUpCreateBudgetTrans.transTotal);
              }
            }
          ...
          copyPopUpCreateBudgetTrans.transTotal = Number(copyPopUpCreateBudgetTrans.transTotal)
          const idxTransEdit = categoryHistory.history.transes.findIndex(it => it.transId === itemTransFromHistory.transId)
          ...
          categoryHistory.history.monthsTotal.forEach((it1) => {
            const monthDiff = AppTimezone.moment(it1.month).isSame(AppTimezone.moment(val.transDate), 'month')
            if (monthDiff) {
              it1.total = it1.total + totalDef
            }
            if (totalMinusDate) {
              if (AppTimezone.moment(it1.month).isSame(Number(totalMinusDate), 'month')) {
                it1.total = it1.total - copyPopUpCreateBudgetTrans.transTotal
              }
            }
          })
          ...
          categoryHistory.history.currentMonth.forEach((it1) => {
            if (typeof totalDefDaily === 'number') {
              if (AppTimezone.moment().isSame(val.transDate, 'month')) {
                const monthDiff = AppTimezone.moment(val.transDate).format('D') === ((it1.transDay).toString())
                if (monthDiff) {
              ...
              if (AppTimezone.moment().isSame(val.transDate, 'month')) {
                if (AppTimezone.moment(val.transDate).format('D') === ((it1.transDay).toString())) {
                  it1.dayTotal = it1.dayTotal + val.total
                }
              }

              if (AppTimezone.moment().isSame(Number(totalDefDaily), 'month')) {
                if (AppTimezone.moment(Number(totalDefDaily)).format('D') === ((it1.transDay).toString())) {
                  it1.dayTotal = it1.dayTotal - val.total
                }
              }
            }
  ...
  handleSelectCategory = (category) => {
    const { itemTransFromHistory } = this.state
    if (!itemTransFromHistory || itemTransFromHistory.transTypeId === category.transTypeId) return
  ...
  handleUpdateBankTrans = (newBankTrans) => {
    ...
    if (newBankTrans.transTypePopup) {
      ...
      const params = {
        ...
        'companyAccountId': newBankTrans.paymentDesc !== 'Budget' ? newBankTrans.companyAccountId : null,
        'transId': newBankTrans.transId,
        'biziboxMutavId': newBankTrans.paymentDesc !== 'Budget' ? newBankTrans.biziboxMutavId : null,
        'companyId': currentCompanyId,
        'updateType': 'row update',
        'transSource': newBankTrans.paymentDesc !== 'Budget' ? newBankTrans.transSource : 'BudgetTrans',
        'paymentDesc': newBankTrans.paymentDesc,
        'searchkeyId': newBankTrans.paymentDesc !== 'Budget' ? newBankTrans.searchkeyId : null,
        'kvua': newBankTrans.paymentDesc !== 'Budget' ? newBankTrans.kvua : false,
      }
  ...
  updateTypeOfTrans = () => {
    ...
    const params = {
      ...
      'companyAccountId': updateTypePopup.paymentDesc !== 'Budget' ? updateTypePopup.companyAccountId : null,
      'transId': updateTypePopup.transId,
      'biziboxMutavId': updateTypePopup.paymentDesc !== 'Budget' ? updateTypePopup.biziboxMutavId : null,
      'companyId': currentCompanyId,
      'updateType': 'row update',
      'transSource': updateTypePopup.paymentDesc !== 'Budget' ? updateTypePopup.transSource : 'BudgetTrans',
      'paymentDesc': updateTypePopup.paymentDesc,
      'searchkeyId': updateTypePopup.paymentDesc !== 'Budget' ? updateTypePopup.searchkeyId : null,
      'kvua': updateTypePopup.paymentDesc !== 'Budget' ? updateTypePopup.kvua : false,
    }
  ...
  render () {
    ...
    if (!isLayoutComplete) return (<Loader overlay containerStyle={{ backgroundColor: 'white' }} />)
    ...
    if (categoryHistory !== null && budgetActive) {
      disableEditPopPast = ((budgetActive.frequencyDesc === 'MONTH' && AppTimezone.moment(budgetActive.dates.dateFrom).diff(AppTimezone.moment(budgetActive.dateFrom), 'months') > 0) ||
        (budgetActive.frequencyDesc === 'YEAR' && AppTimezone.moment(budgetActive.dates.dateFrom).diff(AppTimezone.moment(budgetActive.dateFrom), 'years') > 0))
    }
    ...
    return (
      <Fragment>
        <View style={{
              ...
              {(accountsNotUpdates && accountsNotUpdates.length === 1) && (
                <View style={{
                  ...
                  <Text style={{
                    ...
                    {'הנתונים לא מעודכנים לחשבון'} {t(`bankName:${accountsNotUpdates[0]['bankId']}`)} {accountsNotUpdates[0].bankAccountId} {getCurrencyChar(accountsNotUpdates[0].currency)}
                    ...
                    <Text style={{
                      ...
                      {t(
                        ...
                        { days: AppTimezone.moment().diff(accountsNotUpdates[0].balanceLastUpdatedDate, 'days') },
                      )}
                    </Text>
                  </Text>
                </View>
              ...
              {(accountsNotUpdates && accountsNotUpdates.length > 1) && (
                <TouchableOpacity
                  ...
                  <Text style={{
                    ...
                    {t('bankAccount:accountsNotUpdates', { count: accountsNotUpdates.length })}
                  </Text>
                </TouchableOpacity>
              )}
            </View>
                  ...
                  return (
                    <TouchableOpacity
                      ...
                      <Text style={styles.alertDetailsText} numberOfLines={1}>
                        {t(`bankName:${a['bankId']}`)} {a.bankAccountId} {getCurrencyChar(a.currency)}
                      </Text>

                      <Text style={[styles.alertAdditionalText, { color: colors.red2 }]}>
                        {t(
                          'bankAccount:lastUpdatedXDaysAgo',
                          { days: AppTimezone.moment().diff(a.balanceLastUpdatedDate, 'days') },
                        )}
                      </Text>
                    </TouchableOpacity>
                  )
          ...
          {(dataBudgets && dataBudgets.length > 0 && dataBudgets[0].budgetId && (!loaderPrivScreen && !privScreenEmpty)) && (
            <Fragment>
              <AnimatedFlatList
                refreshControl={
                  <RefreshControl
              ...
              <ActionButton
                ...
                buttonTextStyle={{ fontSize: 54, textAlign: 'center', marginTop: (IS_IOS ? -6 : -7) }}
              />
            </Fragment>
          )}
            ...
            <Interactable.View
              ...
              <View style={styles.panel}>
                ...
                <View style={{ left: 0, right: 0, height: Screen.height - 90 }}>
                  {categoryHistory === null && (itemPanelOfCategories && itemPanelOfCategories.indDefault !== null && (itemPanelOfCategories.indDefault === 1 || itemPanelOfCategories.indDefault === 2)) && (
                    <View style={{
                      height: 45,
                      borderBottomColor: '#f0f1f2',
                      borderBottomWidth: 1,
                      ...
                      <TouchableOpacity
                        ...
                        onPress={this.restore(itemPanelOfCategories.indDefault === 1 ? 'refresh' : 'restore', itemPanelOfCategories)}>
                        <Text style={{
                          color: '#037dba',
                          fontFamily: fonts.regular,
                          fontSize: sp(18),
                        }}>{(itemPanelOfCategories.indDefault === 1) ? 'רענון נתונים' : 'שיחזור תקציב bizibox'}</Text>
                    ...
                    <View>
                      <View style={{
                        ...
                        <View style={{
                          ...
                          {categoryHistory.withDetails && (
                            <Text style={styles.panelTitle}>{'פירוט תנועות - '}</Text>
                          )}
                          <Text style={styles.panelTitle}>{categoryHistory.keyName}</Text>
                        </View>
                  ...
                  <Animated.ScrollView
                    ...
                    {categoryHistory === null && (itemPanelOfCategories && itemPanelOfCategories.incomes && itemPanelOfCategories.incomes.length > 0) && (
                      <View style={{
                        height: 53,
                        flexDirection: 'row-reverse',
                        alignItems: 'center',
                        ...
                        <Text style={{
                          ...
                          fontFamily: fonts.semiBold,
                          fontSize: sp(19),
                        }}>{itemPanelOfCategories.currency} {getFormattedValueArray(itemPanelOfCategories.totals[1])[0]}</Text>
                      </View>
                    )}

                    {categoryHistory === null && (itemPanelOfCategories && itemPanelOfCategories.incomes && itemPanelOfCategories.incomes.length > 0) && itemPanelOfCategories.incomes.map((item, i) => {
                      return (<View
                        style={{
                          paddingRight: 10,
                          paddingLeft: 20,
                        ...
                        <View style={{
                          ...
                          <View style={{
                            ...
                            <View style={{
                              ...
                              <Text style={{
                                ...
                                fontSize: sp(18),
                                fontFamily: fonts.regular,
                              }}> {itemPanelOfCategories.currency} {getFormattedValueArray(item.total)[0]}</Text>
                            </View>
                            <View style={{
                              flexDirection: 'row',
                              justifyContent: 'center',
                          ...
                          <View style={{
                            ...
                            <View style={{
                              ...
                              <Text style={{
                                ...
                                fontSize: sp(18),
                                fontFamily: fonts.regular,
                              }}>{itemPanelOfCategories.currency} {item.totalKeyUse < 0 ? '' : getFormattedValueArray(item.totalKeyUse)[0]}{' '}</Text>
                            </View>
                          ...
                          <View>
                            <View style={styles.bar}>
                              <View style={[styles.fillBar, item.bgColor, {
                                borderBottomRightRadius: (!(item.total === 0 && item.totalKeyUse !== 0) && (item.totalKeyUse / (item.total / 100)) >= 100) ? 4 : 0,
                                borderTopRightRadius: (!(item.total === 0 && item.totalKeyUse !== 0) && (item.totalKeyUse / (item.total / 100)) >= 100) ? 4 : 0,
                                width: (item.totalKeyUse < 0) ? '0%'
                                  : ((item.total === 0 && item.totalKeyUse !== 0)
                                    ? ('20%')
                                    : (((item.totalKeyUse / (item.total / 100)) > 100 ? 100 : (item.totalKeyUse / (item.total / 100))) + '%')),
                              }]} />
                            </View>
                          </View>
                        </View>
                    ...
                    {categoryHistory === null && (itemPanelOfCategories && itemPanelOfCategories.expences && itemPanelOfCategories.expences.length > 0) && (
                      <View style={{
                        height: 53,
                        flexDirection: 'row-reverse',
                        alignItems: 'center',
                        ...
                        <Text style={{
                          ...
                          fontFamily: fonts.semiBold,
                          fontSize: sp(19),
                        }}>{itemPanelOfCategories.currency} {getFormattedValueArray(itemPanelOfCategories.totals[0])[0]}</Text>
                      </View>
                    )}

                    {categoryHistory === null && (itemPanelOfCategories && itemPanelOfCategories.expences && itemPanelOfCategories.expences.length > 0) && itemPanelOfCategories.expences.map((item, i) => {
                      return (<View
                        style={{
                          paddingRight: 10,
                          paddingLeft: 20,
                        ...
                        <View>
                          <View style={{
                            ...
                            <TouchableOpacity
                              ...
                              <View style={{
                                ...
                                <View style={{
                                  ...
                                  <Image
                                    ...
                                    source={require('BiziboxUI/assets/money.png')}
                                  />
                                </View>
                                <Text style={styles.dataRowLevel3Text}>{'סכום'}</Text>
                              </View>
                              ...
                              <View style={{
                                ...
                                <Text
                                  style={styles.dataRowLevel3Text}>{itemPanelOfCategories.currency} {getFormattedValueArray(categoryHistory.total)[0]}</Text>
                                <View style={commonStyles.spaceDividerDouble} />
                                {!disableEditPopPast && (
                                  <Icon
                                    name='chevron-left'
                            ...
                            <TouchableOpacity
                              ...
                              <View style={{
                                ...
                                <View style={{
                                  ...
                                  <CustomIcon name='bell' size={22} color={'#022258'}
                                    style={{
                                      alignSelf: 'flex-end',
                                    }} />
                                </View>
                          ...
                          <View style={{
                            ...
                            <TouchableOpacity
                              ...
                              activeOpacity={0.6}
                              onPress={this.budgetHistoryPrompt(categoryHistory, true)}>
                              <View style={{
                                flexDirection: 'row-reverse',
                                alignItems: 'center',
                                alignContent: 'center',
                                ...
                                <View style={{
                                  ...
                                  <Image
                                    ...
                                    resizeMode='contain'
                                    source={require('BiziboxUI/assets/getCategoryHistoryWithDetails.png')}
                                  />
                                </View>
                                <Text
                        ...
                        <View>
                          <View style={{
                            ...
                            <Text style={{
                              ...
                              {getFormattedValueArray(categoryHistory.history.transesTotal)[0]}{'.'}{getFormattedValueArray(categoryHistory.history.transesTotal)[1]}
                            </Text>
                          </View>
                          {categoryHistory.history.transes && categoryHistory.history.transes.length > 0 && categoryHistory.history.transes.map((item, i) => {
                            return (<View
                              style={{
                              ...
                              <View style={{
                                ...
                                <Text style={{
                                  ...
                                  fontSize: sp(14),
                                  fontFamily: fonts.regular,
                                }}>{AppTimezone.moment(item.transDate).format('DD/MM/YY')}</Text>
                                ...
                                {item.paymentDesc !== 'Budget' && item.paymentMessage && (
                                  <Text style={{
                                    color: '#b4b4b4',
                                    fontSize: sp(14),
                                    fontFamily: fonts.light,
                              ...
                              <View style={{
                                ...
                                <Text style={{
                                  ...
                                  <Text style={{
                                    fontFamily: fonts.semiBold,
                                    color: ((categoryHistory.expence && !item.hova) || (!categoryHistory.expence && !item.hova)) ? colors.green4 : colors.red2,
                                  }}>{((categoryHistory.expence && !item.hova) || (!categoryHistory.expence && item.hova)) ? '-' : ''}{getFormattedValueArray(Math.abs(item.total))[0]}</Text>
                                  <Text style={[{
                                    color: colors.gray7,
                                    fontFamily: fonts.light,
                                  }]}>.{getFormattedValueArray(item.total)[1]}</Text>
                                </Text>
                              </View>
                              <View style={{
                                flex: 40,
                ...
                <KeyboardAwareScrollView>
                    ...
                    <View style={[
                      {
                        ...
                        alignItems: 'flex-end',
                      }, cs(hasError && !this.state.dataPopUpAddEditDuplication['budgetName'].length, {}, {
                        borderWidth: 1,
                  ...
                  <View
                    style={[cs(isRtl, commonStyles.row, [commonStyles.rowReverse]), {
                      height: 42,
                      marginBottom: 8,
                      opacity: this.state.typeOfPopup === 'edit' && this.state.dataPopUpAddEditDuplication.frequencyDesc !== 'ONE_TIME' ? 0.3 : 1,
                    }]}>
                    <View style={{ flex: 2, alignItems: 'flex-end' }}>
                      <Text style={{
                        color: '#0f3860',
                    ...
                    <View style={{
                      ...
                      <TouchableOpacity
                        ...
                        onPress={(this.state.typeOfPopup === 'edit' && this.state.dataPopUpAddEditDuplication.frequencyDesc !== 'ONE_TIME') ? null : this.handleOpenAccountsModal}
                      >
                        <View style={{
                          marginRight: 'auto',
                        }}>
                  ...
                  <View
                    style={[cs(isRtl, commonStyles.row, [commonStyles.rowReverse]), {
                      height: 42,
                      marginBottom: 8,
                      opacity: this.state.disabledValuesPopUp.budgetTotalType ? 0.3 : 1,
                    }]}>
                    <View style={{ flex: 2, alignItems: 'flex-end' }}>
                      <Text style={{
                        color: '#0f3860',
                    ...
                    <View style={{
                      ...
                      <TouchableOpacity
                        ...
                        onPress={this.state.disabledValuesPopUp.budgetTotalType ? null : this.editInput('budgetTotalType')}
                      >
                        <View style={{
                          marginRight: 'auto',
                        }}>
                        ...
                        <Text style={[{
                          ...
                          {this.state.dataPopUpAddEditDuplication.budgetTotalType ? this.state.ddBudgetTotalType.find(it => it.value === this.state.dataPopUpAddEditDuplication.budgetTotalType).label : ''}
                        </Text>
                      </TouchableOpacity>
                    </View>
                  </View>
                  ...
                  <View
                    style={[cs(isRtl, commonStyles.row, [commonStyles.rowReverse]), {
                      height: 42,
                      marginBottom: 8,
                      opacity: this.state.disabledValuesPopUp.budgetType ? 0.3 : 1,
                    }]}>
                    <View style={{ flex: 2, alignItems: 'flex-end' }}>
                      <Text style={{
                        color: '#0f3860',
                    ...
                    <View style={{
                      ...
                      <TouchableOpacity
                        ...
                        onPress={this.state.disabledValuesPopUp.budgetType ? null : this.editInput('budgetType')}>
                        <View style={{
                          marginRight: 'auto',
                        }}>
                          <Icon name='chevron-left' size={24} color={colors.blue34} />
                        ...
                        <Text style={[{
                          ...
                          {this.state.dataPopUpAddEditDuplication.budgetType ? this.state.ddBudgetType.find(it => it.value === this.state.dataPopUpAddEditDuplication.budgetType).label : ''}
                        </Text>
                      </TouchableOpacity>
                    </View>
                  </View>

                  {(lengthSlides === null || !lengthSlides || (lengthSlides.length === 0)) ? (
                    <View
                    ...
                    <Fragment>
                      {hasError && !this.state.dataPopUpAddEditDuplication['keyDetails'].length && (
                        <Text style={{
                          fontSize: sp(13),
                          color: '#cd1010',
                          textAlign: 'right',
                      ...
                      <View
                        ...
                        <View style={{
                          ...
                          <CheckBox
                            ...
                            checkedIcon='check'
                            uncheckedIcon='check'
                            checked={this.state.dataPopUpAddEditDuplication.creditCardCalcTypeDesc === this.state.ddCreditCardCalcTypeDesc[0].value}
                            onPress={this.state.dataPopUpAddEditDuplication.creditCardCalcTypeDesc === this.state.ddCreditCardCalcTypeDesc[0].value ? null : this.handleToggleCheckBox(this.state.ddCreditCardCalcTypeDesc[0].value)}
                          />
                        </View>
                      </View>
                      <View
                        ...
                        <View style={{
                          ...
                          <CheckBox
                            ...
                            checkedIcon='check'
                            uncheckedIcon='check'
                            checked={this.state.dataPopUpAddEditDuplication.creditCardCalcTypeDesc === this.state.ddCreditCardCalcTypeDesc[1].value}
                            onPress={this.state.dataPopUpAddEditDuplication.creditCardCalcTypeDesc === this.state.ddCreditCardCalcTypeDesc[1].value ? null : this.handleToggleCheckBox(this.state.ddCreditCardCalcTypeDesc[1].value)}
                          />
                        </View>
                      </View>
                  ...
                  <View
                    ...
                    <View style={{
                      ...
                      <TouchableOpacity
                        ...
                        onPress={this.state.typeOfPopup === 'edit' ? null : this.editInput('frequencyDesc')}
                      >
                        <View style={{
                          marginRight: 'auto',
                        }}>
                        ...
                        <Text style={[{
                          ...
                          {this.state.dataPopUpAddEditDuplication.frequencyDesc ? this.state.ddFrequencyDesc.find(it => it.value === this.state.dataPopUpAddEditDuplication.frequencyDesc).label : ''}
                        </Text>
                      </TouchableOpacity>
                    </View>
                  </View>

                  {(this.state.dataPopUpAddEditDuplication.frequencyDesc && this.state.dataPopUpAddEditDuplication.frequencyDesc === 'ONE_TIME') && (
                    <Fragment>
                      <View
                        style={[cs(isRtl, commonStyles.row, [commonStyles.rowReverse]), {
                          height: 42,
                        ...
                        <View style={{
                          ...
                          <TouchableOpacity
                            ...
                            <View style={{
                              ...
                              <CustomIcon name={'calendar'} size={22} color={'#022258'} />
                            </View>
                            <Text style={[{
                              textAlign: 'right',
                              color: '#0f3860',
                              fontSize: sp(15),
                              lineHeight: 42,
                            }, commonStyles.regularFont]}>{AppTimezone.moment(this.state.dataPopUpAddEditDuplication.dateFrom).format('DD/MM/YY')}</Text>
                          </TouchableOpacity>
                        </View>
                      </View>
                        ...
                        <View style={{
                          ...
                          <TouchableOpacity
                            ...
                            <Text style={[{
                              ...
                              fontSize: sp(15),
                              lineHeight: 42,
                            }, commonStyles.regularFont]}>{AppTimezone.moment(this.state.dataPopUpAddEditDuplication.dateTill).format('DD/MM/YY')}</Text>
                          </TouchableOpacity>
                        </View>
                      </View>
                    </Fragment>
                    ...
                    <View style={[cs(isRtl, commonStyles.row, [commonStyles.rowReverse]), {
                      ...
                      <View style={{
                        ...
                        <Text style={{
                          ...
                          {this.state.dataPopUpAddEditDuplication.frequencyDesc === 'MONTH' ? 'התקציב יתחדש אוטומטית כאשר \nהחודש הקלנדרי יתחיל' : 'התקציב יתחדש אוטומטית כאשר \nהשנה הקלנדרית תתחיל'}
                        </Text>
                      </View>
                    </View>
                  )}

                  {(this.state.dataPopUpAddEditDuplication['budgetTotalType'] === 'both' || this.state.dataPopUpAddEditDuplication['budgetTotalType'] === 'income') && (
                    <View
                      style={[cs(isRtl, commonStyles.row, [commonStyles.rowReverse]), {
                        height: 42,
                        marginBottom: 8,
                      ...
                      <View style={[{
                        ...
                        borderBottomRightRadius: 20,
                        borderTopRightRadius: 20,
                      }, cs(hasError && (!this.state.dataPopUpAddEditDuplication['totalIncome'] || (this.state.dataPopUpAddEditDuplication['totalIncome'] && !this.state.dataPopUpAddEditDuplication['totalIncome'].toString().length)), {}, {
                        borderWidth: 1,
                        borderColor: colors.red,
                      })]}>
                        {this.state.editSumIn && (
                          <TextInput
                            ...
                            onEndEditing={(e) => {
                              let dataPopUpAddEditDuplication = Object.assign({}, this.state.dataPopUpAddEditDuplication)
                              dataPopUpAddEditDuplication['totalIncome'] = e.nativeEvent.text.toString().replace(/[^\d]/g, '')
                              this.setState({
                                dataPopUpAddEditDuplication: dataPopUpAddEditDuplication,
                                editSumIn: false,
                              })
                            ...
                            onChangeText={(totals) => {
                              let dataPopUpAddEditDuplication = Object.assign({}, this.state.dataPopUpAddEditDuplication)
                              dataPopUpAddEditDuplication['totalIncome'] = totals.toString().replace(/[^\d]/g, '')
                              this.setState({ dataPopUpAddEditDuplication: dataPopUpAddEditDuplication })
                            }}
                            value={this.state.dataPopUpAddEditDuplication['totalIncome'] ? inputWorkaround.getWorkaroundChar() + String(this.state.dataPopUpAddEditDuplication['totalIncome']) : null}
                            underlineColorAndroid='transparent'
                          />
                        )}
                        ...
                        {!this.state.editSumIn && (
                          <TouchableOpacity
                            ...
                            {this.state.dataPopUpAddEditDuplication['totalIncome'] !== null && this.state.dataPopUpAddEditDuplication['totalIncome'] !== '' && (
                              <Text
                                ...
                                <Text style={[{
                                  ...
                                  lineHeight: 42,
                                  color: '#0f3860',
                                }, commonStyles.regularFont]}>{getFormattedValueArray(this.state.dataPopUpAddEditDuplication['totalIncome'])[0]}</Text>
                              </Text>
                            )}
                          </TouchableOpacity>
                        )}
                  ...
                  {(this.state.dataPopUpAddEditDuplication['budgetTotalType'] === 'both' || this.state.dataPopUpAddEditDuplication['budgetTotalType'] === 'outcome') && (
                    <View
                      style={[cs(isRtl, commonStyles.row, [commonStyles.rowReverse]), {
                        height: 42,
                        marginBottom: 20,
                      ...
                      <View style={[{
                        ...
                        borderBottomRightRadius: 20,
                        borderTopRightRadius: 20,
                      }, cs(hasError && (!this.state.dataPopUpAddEditDuplication['totalOutcome'] || (this.state.dataPopUpAddEditDuplication['totalOutcome'] && !this.state.dataPopUpAddEditDuplication['totalOutcome'].toString().length)), {}, {
                        borderWidth: 1,
                        borderColor: colors.red,
                      })]}>
                        {this.state.editSumEx && (
                          <TextInput
                            ...
                            onEndEditing={(e) => {
                              let dataPopUpAddEditDuplication = Object.assign({}, this.state.dataPopUpAddEditDuplication)
                              dataPopUpAddEditDuplication['totalOutcome'] = e.nativeEvent.text.toString().replace(/[^\d]/g, '')
                              this.setState({
                                dataPopUpAddEditDuplication: dataPopUpAddEditDuplication,
                                editSumEx: false,
                              })
                            ...
                            onChangeText={(totals) => {
                              let dataPopUpAddEditDuplication = Object.assign({}, this.state.dataPopUpAddEditDuplication)
                              dataPopUpAddEditDuplication['totalOutcome'] = totals.toString().replace(/[^\d]/g, '')
                              this.setState({ dataPopUpAddEditDuplication: dataPopUpAddEditDuplication })
                            }}
                            value={this.state.dataPopUpAddEditDuplication['totalOutcome'] ? inputWorkaround.getWorkaroundChar() + String(this.state.dataPopUpAddEditDuplication['totalOutcome']) : null}
                            underlineColorAndroid='transparent'
                          />
                        )}
                        ...
                        {!this.state.editSumEx && (
                          <TouchableOpacity
                            ...
                            {this.state.dataPopUpAddEditDuplication['totalOutcome'] !== null && this.state.dataPopUpAddEditDuplication['totalOutcome'] !== '' && (
                              <Text
                                style={[styles.dataValueWrapper, styles.dataValueWrapperLevel2, {
                                  fontSize: sp(15),
                                  direction: 'ltr',
                                ...
                                <Text style={[{
                                  ...
                                  lineHeight: 42,
                                  color: '#0f3860',
                                }, commonStyles.regularFont]}>{getFormattedValueArray(this.state.dataPopUpAddEditDuplication['totalOutcome'])[0]}</Text>
                              </Text>
                            )}
                          </TouchableOpacity>
                        )}
                  ...
                  {isKeyDetailsNotEqual && (
                    <TouchableOpacity
                      ...
                      <Image
                        style={[{ width: 16, height: 15.5, marginLeft: 5 }]}
                        source={require('BiziboxUI/assets/iconAler.png')}
                      />
                      <Text
                        style={[{
                  ...
                  <View style={{
                    ...
                    <View style={{
                      ...
                      <View style={{
                        ...
                        <TouchableOpacity
                          activeOpacity={(categoriesMatchCopy.filter((it) => it.press === true).length > 0) ? 0.2 : 1}
                          hitSlop={{ top: 20, bottom: 20, left: 20, right: 20 }}
                          onPress={this.handleRemoveAllCategories}>
                          <Text style={{
                            color: categoriesMatchCopy.filter((it) => it.press === true).length > 0 ? '#2aa1d9' : '#c2c3c3',
                            fontSize: sp(15),
                            fontFamily: fonts.regular,
                          }}>
                            {'הסר הכל'}
                                  ...
                                  return (
                                    <TouchableOpacity key={i1.toString()}
                                      ...
                                      style={{
                                        ...
                                        width: 82,
                                        borderWidth: 1,
                                        borderColor: f.press ? categoriesMatchModal : '#0c2b5f',
                                        borderRadius: 5,
                                        backgroundColor: f.press ? categoriesMatchModal : 'white',
                                      }}>
                                      <View style={{}}>
                                        <CustomIcon
                ...
                <SafeAreaView style={{
                  ...
                  <View style={{
                    ...
                    <View style={{
                      ...
                      <KeyboardAwareScrollView>
                        {(this.state.typeEditModal !== 'dateFrom' && this.state.typeEditModal !== 'dateTill') && (
                          <CheckList
                        ...
                        {(this.state.typeEditModal === 'dateFrom' || this.state.typeEditModal === 'dateTill') && (
                          <Calendar
                            current={AppTimezone.moment(this.state.dataPopUpAddEditDuplication[this.state.typeEditModal]).format('YYYY-MM-DD')}
                            markedDates={{
                              [AppTimezone.moment(this.state.dataPopUpAddEditDuplication[this.state.typeEditModal]).format('YYYY-MM-DD')]: {
                                startingDay: true,
                                selected: true,
                                color: '#022258',
                              },
                            ...
                            onDayPress={(day) => {
                              ...
                              } else if (this.state.typeEditModal === 'dateTill') {
                                if (dateFrom > dateTill) {
            ...
            <View style={{
              ...
              <View style={{
                ...
                <View style={cs(
                  ...
                  <View>
                    <TouchableOpacity onPress={this.closePopUpCreateBudgetTrans}>
                      <Text style={{
                        fontSize: sp(16),
                        color: '#ffffff',
                        fontFamily: fonts.semiBold,
                  ...
                  <View style={{ alignItems: 'center' }}>
                    <Text style={{ fontSize: sp(20), color: '#ffffff', fontFamily: fonts.semiBold }}>
                      {popUpCreateBudgetTransEdit ? 'עריכת' : 'הוספת'}{' תנועה ידנית'}
                    </Text>
                  </View>
                  <View>
                    <TouchableOpacity
                    ...
                    <View style={{
                      ...
                      <TouchableOpacity
                        ...
                        <Text style={[{
                          ...
                          fontSize: sp(15),
                          lineHeight: 42,
                        }, commonStyles.regularFont]}>{AppTimezone.moment(popUpCreateBudgetTrans.transDate).format('DD/MM/YY')}</Text>
                      </TouchableOpacity>
                    </View>
                  </View>
                    ...
                    <View style={[
                      {
                        ...
                        borderTopRightRadius: 20,
                        alignItems: 'flex-end',
                      }, cs(hasErrorCreateBudgetTrans && !popUpCreateBudgetTrans.transName.length, {}, {
                        borderWidth: 1,
                        borderColor: colors.red,
                      }),
                    ]}>
                      <TextInput
                        ...
                        onEndEditing={(e) => {
                          let datapopUpCreateBudgetTrans = Object.assign({}, popUpCreateBudgetTrans)
                          datapopUpCreateBudgetTrans['transName'] = e.nativeEvent.text
                          this.setState({ popUpCreateBudgetTrans: datapopUpCreateBudgetTrans })
                        }}
                    ...
                    <View style={[{
                      ...
                      {this.state.editSumCreateBudget && (
                        <TextInput
                          ...
                          onEndEditing={(e) => {
                            let popUpCreateBudgetTransSave = Object.assign({}, popUpCreateBudgetTrans)
                          ...
                          value={popUpCreateBudgetTrans.transTotal !== null ? String(popUpCreateBudgetTrans.transTotal) : ''}
                          underlineColorAndroid='transparent'
                        />
                      )}
                      ...
                      {!this.state.editSumCreateBudget && (
                        <TouchableOpacity
                          ...
                          onPress={this.setStates({ editSumCreateBudget: true })}>
                          {(popUpCreateBudgetTrans.transTotal !== null && popUpCreateBudgetTrans.transTotal !== '') && (
                            <Text
                              style={[styles.dataValueWrapper, styles.dataValueWrapperLevel2, {
                                fontSize: sp(15),
                                lineHeight: 42,
                            ...
                            >
                              <Text style={[{
                                ...
                                lineHeight: 42,
                                color: '#0f3860',
                              }, commonStyles.regularFont]}>{getFormattedValueArray(popUpCreateBudgetTrans.transTotal)[0]}</Text>
                              <Text style={[styles.fractionalPart, {
                                fontSize: sp(15),
                                lineHeight: 42,
                                color: '#0f3860',
                              }, commonStyles.regularFont]}>.{getFormattedValueArray(popUpCreateBudgetTrans.transTotal)[1]}</Text>
                            </Text>
                          )}
                        </TouchableOpacity>
                      )}
                ...
                <SafeAreaView style={{
                  ...
                  <View style={{
                    ...
                    <View style={{
                      ...
                      <KeyboardAwareScrollView>
                        <Calendar
                          minDate={AppTimezone.moment().subtract(11, 'month').startOf('month').format('YYYY-MM-DD')}
                          maxDate={AppTimezone.moment().format('YYYY-MM-DD')}
                          current={AppTimezone.moment(popUpCreateBudgetTrans.transDate).isBefore(AppTimezone.moment().valueOf()) ? AppTimezone.moment(popUpCreateBudgetTrans.transDate).format('YYYY-MM-DD') : AppTimezone.moment().format('YYYY-MM-DD')}
                          markedDates={{
                            [AppTimezone.moment(popUpCreateBudgetTrans.transDate).isBefore(AppTimezone.moment().valueOf()) ? AppTimezone.moment(popUpCreateBudgetTrans.transDate).format('YYYY-MM-DD') : AppTimezone.moment().format('YYYY-MM-DD')]: {
                              startingDay: true,
                              selected: true,
                              color: '#022258',
                            },
                          ...
                          onDayPress={(day) => {
                            day.timestamp = AppTimezone.moment(day.dateString).startOf('day').valueOf()
                            let datapopUpCreateBudgetTrans = Object.assign({}, popUpCreateBudgetTrans)
        ...
        {isReady && (
          ...
          >
            <View style={{
              height: (actionSheetTooltip === 'tooltipB' || actionSheetTooltip === 'tooltipExam') ? 192 : (alertsItem ? 115 : ((actionSheetTooltip === 'refresh' || actionSheetTooltip === 'restore') ? 138 : 0)),
              marginBottom: 20,
            }}>

              {(actionSheetTooltip !== null && (actionSheetTooltip === 'refresh' || actionSheetTooltip === 'restore')) && (
                <Fragment>
                  <View style={{
                    flexDirection: 'column',
                    borderBottomWidth: 1,
              ...
              {(actionSheetTooltip !== null && actionSheetTooltip === 'tooltipB') && (
                <Fragment>
                  <View style={{
                    flexDirection: 'row-reverse',
                    justifyContent: 'center',
              ...
              {(actionSheetTooltip !== null && actionSheetTooltip === 'tooltipExam') && (
                <Fragment>
                  <View style={{
                    flexDirection: 'row-reverse',
                    justifyContent: 'center',
              ...
              {alertsItem && (
                <Fragment>
                  <Text style={{
                    ...
                    fontSize: sp(19),
                    fontFamily: fonts.regular,
                  }}>{alertsItemType === 'incomeAlert' ? 'קבלת התראות עבור תקציב הכנסות לאחר' : 'קבלת התראות עבור תקציב הוצאות לאחר'}</Text>
                  ...
                  <View style={{
                    ...
                    <Text style={{
                      color: (alertsItem[alertsItemType === 'incomeAlert' ? 'prcIncome' : 'prcOutcome'] > 0 && alertsItem[alertsItemType]) ? '#022258' : '#9e9e9e',
                      fontSize: sp(19),
                      fontFamily: fonts.regular,
                    }}>{alertsItem[alertsItemType === 'incomeAlert' ? 'prcIncome' : 'prcOutcome']}%</Text>

                    {(alertsItem[alertsItemType === 'incomeAlert' ? 'prcIncome' : 'prcOutcome'] > 0 && alertsItem[alertsItemType]) && (
                      <CustomIcon name='bell-enable' size={22} color={'#022258'}
                        style={{ marginHorizontal: 8 }} />
                    )}
                    {(alertsItem[alertsItemType === 'incomeAlert' ? 'prcIncome' : 'prcOutcome'] === 0 || !alertsItem[alertsItemType]) && (
                      <CustomIcon name='bell-disable' size={22} color={'#9e9e9e'}
                        style={{ marginHorizontal: 8 }} />
                    )}
                  </View>

                  <Slider
                    style={{ flex: 1, height: 40, marginHorizontal: 20 }}
                    minimumValue={0}
                    maximumValue={100}
            ...
            {(actionSheetItemList && actionSheetItemList.length > 0) && actionSheetItemList.map((item, i) => {
              if (item.value) {
                  ...
                  icon={
                    item.icon.name === 'duplication'
                      ? <Image style={item.icon.style}
                        source={require('BiziboxUI/assets/duplication.png')} />
                      : (item.icon.name === 'restore'
                        ? <Image style={item.icon.style}
                          source={require('BiziboxUI/assets/restore.png')} />
                        : <CustomIcon
                          name={item.icon.name}
                          size={item.icon.size}
