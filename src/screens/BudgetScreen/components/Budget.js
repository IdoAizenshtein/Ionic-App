...
export default class Budget extends AnimatedControlledRow {
  ...
  render () {
    ...
    let prcIncomesArr = []
    let prcIncome = 0
    if (!inProgress && (item.budgetTotalType === 'both' || item.budgetTotalType === 'income') && item.budgetsDetails) {
      prcIncome = (item.budgetsDetails.totalIncomeUse < 0)
        ? 0
        : ((item.totalIncome === 0 && item.budgetsDetails.totalIncomeUse !== 0) ? 20 : (item.budgetsDetails.totalIncomeUse / (item.totalIncome / 100)))
    ...
    let prcOutcomeArr = []
    let prcOutcome = 0
    if (!inProgress && (item.budgetTotalType === 'both' || item.budgetTotalType === 'outcome') && item.budgetsDetails) {
      prcOutcome = (item.budgetsDetails.totalOutcomeUse < 0)
        ? 0
        : ((item.totalOutcome === 0 && item.budgetsDetails.totalOutcomeUse !== 0) ? 20 : (item.budgetsDetails.totalOutcomeUse / (item.totalOutcome / 100)))
    ...
    if (item.budgetAccounts.length === 1) {
      account = accounts.find((acc) => acc.companyAccountId === item.budgetAccounts[0])
    }
              ...
              <View style={{
                ...
                <View style={{
                  ...
                  <Text style={{
                    ...
                    {item.frequencyDesc === 'MONTH' ? 'חודשי' : (item.frequencyDesc === 'ONE_TIME' ? 'חד פעמי' : 'שנתי')}
                  </Text>
                </View>
                <View style={{
                  flexDirection: 'row-reverse',
                    ...
                    <Fragment>
                      <TouchableOpacity
                        ...
                        <Icon
                          ...
                          type='entypo'
                          size={19}
                          color={!disabledNavDatePrevConst ? '#022258' : '#a2a2a2'}
                        />
                      </TouchableOpacity>
                      ...
                      <Text style={{
                        ...
                        fontSize: sp(16),
                        fontFamily: fonts.regular,
                      }}>{item.frequencyDesc === 'MONTH' ? LocaleConfig.locales['he'].monthNames[parseFloat(AppTimezone.moment(item.dateTill).format('MM')) - 1] : ''} {AppTimezone.moment(item.dateTill).format(item.frequencyDesc === 'MONTH' ? 'YY' : 'YYYY')}</Text>
                      ...
                      <TouchableOpacity
                        ...
                        <Icon
                          ...
                          type='entypo'
                          size={19}
                          color={!disabledNavDateNextConst ? '#022258' : '#a2a2a2'}
                        />
                      </TouchableOpacity>
                    </Fragment>
                  )}
                        ...
                        <View style={{
                            ...
                            <View style={{
                                ...
                                data={
                                  prcIncomesArr.map((key, index) => {
                                    return {
                                      value: key['prc'],
                                      key: `pie-${index}`,
                                      svg: { fill: key.active ? '#229f88' : '#bbf6eb' },
                              ...
                              <View style={{
                                ...
                                <Text
                                  ...
                                  {item.budgetsDetails.totalIncomeUse < 0 ? '0%' : `${item.totalIncome !== 0 ? Math.round((item.budgetsDetails.totalIncomeUse / (item.totalIncome / 100))) : (0)}%`}
                                </Text>
                                <Text style={{
                                  textAlign: 'center',
                                  color: '#022258',
                          ...
                          <View style={{
                            ...
                            <Text style={{
                              ...
                              fontFamily: fonts.regular,
                              fontSize: sp(19.5),
                            }}>{item.currency} {getFormattedValueArray(Math.round(item.totalIncome))[0]}</Text>
                            <Text style={{
                              textAlign: 'center',
                              color: '#022258',
                          ...
                          {item.budgetsDetails && (
                            <View style={{
                              ...
                              <Text style={{
                                ...
                                fontFamily: fonts.regular,
                                fontSize: sp(19.5),
                              }}>{item.currency} {(item.budgetsDetails.totalIncomeUse < 0) ? '' : (item.totalIncome !== 0 ? (item.budgetsDetails.totalIncomeUse > item.totalIncome) ? 0 : getFormattedValueArray(Math.round((item.totalIncome - item.budgetsDetails.totalIncomeUse)))[0] : '')}</Text>
                              <Text style={{
                                textAlign: 'center',
                                color: '#022258',
                                fontFamily: fonts.light,
                        ...
                        <View style={{
                                ...
                                data={
                                  prcOutcomeArr.map((key, index) => {
                                    return {
                                      value: key['prc'],
                                      key: `pie-${index}`,
                                      svg: { fill: key.active ? '#cd1010' : '#fedddd' },
                                      arc: { outerRadius: 100 + '%', padAngle: 0 },
                                    }
                                  })
                                }
                              ...
                              <View style={{
                                ...
                                <Text
                                  ...
                                  {item.budgetsDetails.totalOutcomeUse < 0 ? 0
                                    : (item.totalOutcome !== 0 ? Math.round((item.budgetsDetails.totalOutcomeUse / (item.totalOutcome / 100))) : (0)) + '%'}
                                </Text>
                                <Text style={{
                                  textAlign: 'center',
                                  color: '#022258',
                          ...
                          <View style={{
                            ...
                            <Text style={{
                              ...
                              fontFamily: fonts.regular,
                              fontSize: sp(18),
                            }}>{item.currency} {getFormattedValueArray(Math.round(item.totalOutcome))[0]}</Text>
                            <Text style={{
                              textAlign: 'center',
                              color: '#022258',
                              fontFamily: fonts.light,
                          ...
                          {item.budgetsDetails && (
                            <View style={{
                              ...
                              <Text style={{
                                ...
                                fontFamily: fonts.regular,
                                fontSize: sp(18),
                              }}>{item.currency} {(item.budgetsDetails.totalOutcomeUse < 0) ? '' : (item.totalOutcome !== 0 ? (item.budgetsDetails.totalOutcomeUse > item.totalOutcome) ? 0 : getFormattedValueArray(Math.round(item.totalOutcome - item.budgetsDetails.totalOutcomeUse))[0] : '')}</Text>
                              <Text style={{
                                textAlign: 'center',
                                color: '#022258',
                                fontFamily: fonts.light,
