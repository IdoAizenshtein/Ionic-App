...
const STATUS_BAR_HEIGHT = Platform.select({ ios: 95, android: 95, default: 95 })
...
export default class OverviewScreen extends PureComponent {
  ...
  constructor (props) {
    ...
    const openPopupAfterUpgrade = props.navigation.getParam('openPopupAfterUpgrade')
    ...
    this.state = {
      ...
      dateFrom: AppTimezone.moment().startOf('month').format('YYYY-MM-DD'),
    ...
    if (this.props.navigation.state.params.openPopupAfterUpgrade) {
      delete this.props.navigation.state.params.openPopupAfterUpgrade
    }
  }
  ...
  get selectedDeviantAccounts () {
    if (IS_LIGHT.light) {
      return this.selectedAccounts.filter(a => a.accountBalance < a.creditLimit)
    } else {
  ...
  get selectedHarigaDatetAccounts () {
    ...
    if (cashFlowAggregatedData) {
      return cashFlowAggregatedData.filter(d => d.accountUuid !== null && d.harigaDate !== null)
    } else {
      return []
    }
  }
  ...
  get hasHeaderAlert () {
    const { cashFlowAggregatedData } = this.state
    if (!cashFlowAggregatedData) return false

    if ((!this.selectedAccounts.length || (!this.selectedHarigaDatetAccounts.length && !this.selectedDeviantAccounts.length)) && !this.selectedNotUpdatedAccounts.length) return false
  ...
  get headerAlertState () {
    ...
    const accountExceeded = selectedAccounts.length === 1 && !selectedDeviantAccounts.length && selectedHarigaDatetAccounts.length

    const accountExceptionExpected = selectedAccounts.length === 1 && selectedDeviantAccounts.length === 1

    const accountNicknameAccountException = selectedAccounts.length > 1 && selectedDeviantAccounts.length === 1

    const xAccountsExceeded = selectedAccounts.length > 1 && selectedDeviantAccounts.length > 1

    const expectedDateExceeded = selectedAccounts.length > 1 && selectedHarigaDatetAccounts.length === 1 && !selectedDeviantAccounts.length

    const expectedXExceeded = selectedAccounts.length > 1 && selectedHarigaDatetAccounts.length > 1 && !selectedDeviantAccounts.length
    ...
    return {
      ...
      isSelectedNotUpdatedAccounts: this.selectedNotUpdatedAccounts.length > 0,
    }
  }
    ...
    if (this.selectedAccounts.length === 1) {
      return (
        ...
        >
          ...
          <Text style={styles.bankAccountHeaderText}>{account.accountNickname} {currencySign}</Text>
        </TouchableOpacity>
      )
    }

    const title = this.selectedAccounts.every(a => a._isUpdated) &&
    this.selectedAccounts.length === accountGroups[selectedGroup].filter(a => a._isUpdated).length
  ...
  get accountsBalanceTotal () {
    if (!this.selectedAccounts.length) return 'false'
  ...
  get accountsBalance () {
    if (this.accountsBalanceTotal === 'false') {
      ...
      return `${numberFormat.format(Math.round(this.accountsBalanceTotal))}`
    }
  }
  ...
  get isLightNotUpdatedAccounts () {
    ...
    if (newTokenStatus && statusToken === BANK_TOKEN_STATUS.INVALID_PASSWORD) {
      return (
        <TouchableOpacity
          onPress={this.handleOpenUpdateTokenModal(this.selectedNotUpdatedAccounts[0])}
          style={{
            alignItems: 'center',
          ...
          <Text style={{
            ...
            {'סיסמתך לבנק'} {t(`bankName:${newTokenStatus.websiteTargetTypeId}`)} {'שגויה'}
          </Text>
          <Icon name='chevron-left' size={22} color={colors.blue32} />
        </TouchableOpacity>
      )
    } else if (newTokenStatus && statusToken === BANK_TOKEN_STATUS.PASSWORD_EXPIRED) {
      return (
        <TouchableOpacity
          onPress={this.handleOpenUpdateTokenModal(this.selectedNotUpdatedAccounts[0])}
          style={{
          ...
          <Text style={{
            ...
            {'סיסמתך לבנק'} {t(`bankName:${newTokenStatus.websiteTargetTypeId}`)} {'פגה'}
          </Text>
          <Icon name='chevron-left' size={22} color={colors.blue32} />
        </TouchableOpacity>
      )
    } else if (newTokenStatus && statusToken === BANK_TOKEN_STATUS.BLOCKED) {
      return (
        <View style={{
          alignItems: 'center',
          flexDirection: 'row-reverse',
    ...
    } else if (newTokenStatus && statusToken === BANK_TOKEN_STATUS.AGREEMENT_REQUIRED) {
      return (
        <View style={{
          alignItems: 'center',
          flexDirection: 'row-reverse',
    ...
    } else if (newTokenStatus && statusToken === BANK_TOKEN_STATUS.INVALID_PASSWORD_AND_ACCESS) {
      return (
        <View style={{
          alignItems: 'center',
          flexDirection: 'row-reverse',
  ...
  componentDidMount () {
    ...
    const timeoutAcc = this.props.navigation.getParam('timeoutAcc')
    let time = 0
    if (timeoutAcc) {
      time = 1500
      if (this.props.navigation.state.params.timeoutAcc) {
        delete this.props.navigation.state.params.timeoutAcc
      }
    }
    setTimeout(() => {
  ...
  selectDefaultAccounts = () => {
    ...
    if (!accountGroups || !Object.keys(accountGroups).length) return
    ...
    const findIdsToSelect = (currency) => {
      ...
      if (hasUpdatedAccount) {
        ...
        this.setState({ ...getAccountIdForSelection(currency, accountId, selectedAccountIds, selectedGroup) })
      }
      setTimeout(() => {
    ...
    if (accountGroups.hasOwnProperty(DEFAULT_PRIMARY_CURRENCY)) return findIdsToSelect(DEFAULT_PRIMARY_CURRENCY)
    return findIdsToSelect(Object.keys(accountGroups)[0])
  }

  handleSelectAccounts = (selectedGroup, selectedAccountIds = [], fn) => {
    this.setState({ selectedAccountIds, selectedGroup, currentSelectedAccountId: null }, () => {
  ...
  getAggregateData = () => {
    ...
    const balanceLastUpdatedDate = accounts.find((element) => element.companyAccountId === this.accountIdsForRequest[0]).balanceLastUpdatedDate

    return cashFlowAggregateDataApi.post({
      body: {
        companyAccountIds: this.accountIdsForRequest,
        companyId: currentCompanyId,
        dateFrom: AppTimezone.moment(balanceLastUpdatedDate).subtract(2, 'day').toISOString(),
        dateTill: AppTimezone.moment(balanceLastUpdatedDate).add(5, 'day').toISOString(),
        expence: -1,
      },
    })
  }
  ...
  getFinancialData = () => {
    return new Api({ endpoint: `over-view/cfl/cfl/financial-data` }).post({
      body: {
        companyAccountIds: this.accountIdsForRequest,
        dateFrom: null,
        dateTill: null,
  ...
  getBankTrans = () => {
    ...
    return new Api({ endpoint: `account/cfl/bank-trans` }).post({
      body: {
        companyAccountIds: this.accountIdsForRequest,
        companyId: currentCompanyId,
        dateFrom: AppTimezone.moment().subtract(30, 'day').toISOString(),
  ...
  getBankTransPeulotToday = () => {
    const { currentCompanyId } = this.props
    return new Api({ endpoint: `account/cfl/bank-trans-peulot-today` }).post({
      body: {
        companyAccountIds: this.accountIdsForRequest,
        companyId: currentCompanyId,
        dateTill: null,
  ...
  getChartData = () => {
    ...
    const balanceLastUpdatedDate = accounts.find((element) => element.companyAccountId === this.accountIdsForRequest[0])
    if (IS_LIGHT.light) {
  ...
  getScreenData = (isLoader = true) => {
    ...
    if (isLoader) {
      this.setState({ inProgress: true, isLayoutComplete: false, progressPies: true })
    } else {
      this.setState({ inProgress: true, progressPies: true })
    }
    ...
    return this.getData(tasks)
      ...
      .then((data) => {
        if (this._mounted) {
          ...
          const tasksAggregateDataType = this.getAggregateDataType(this.state.dateFrom, this.state.dateTill)
      ...
      .then((data) => {
        if (this._mounted) {
          if (data) {
            if (!data.expenses && !data.incomes && !data.totalExpenses && !data.totalIncomes) {
              this.setState({
                aggregateDataType: {
                  expenses: [],
                  incomes: [],
      ...
      .then((data) => {
        if (this._mounted) {
          ...
          const tasksLast = [this.getBankTrans(), this.getBankTransPeulotToday()]
      ...
      .then((data) => {
        if (this._mounted) {
            ...
            getBankTransPeulotToday = [{
              ...
              data: getBankTransPeulotToday.map((trans) => {
                return Object.assign(trans, {
                  account: accounts.find(a => a.companyAccountId === trans.companyAccountId),
                })
              }),
            }]
          }
          if (getBankTrans.bankTransList && getBankTrans.bankTransList.length) {
            getBankTrans.bankTransList = getBankTrans.bankTransList.reduce((memo, trans) => {
              const title = dateToFromNowDaily(trans.transDate, t, 'DD/MM')
              trans.account = accounts.find(a => a.companyAccountId === trans.companyAccountId)
          ...
          let bankTrans = getBankTransPeulotToday.concat(getBankTrans.bankTransList)
          this.headerChartX = new Animated.ValueXY()
          this.setState({
            bankTrans: bankTrans,
            inProgress: false,
          ...
          getAccountCflTransTypeApi.post({ body: { uuid: currentCompanyId } })
            .then(data => {
              this.setState({ categoriesArr: data })
            })
        }
      })
      .catch((err) => {
  ...
  linkToPage = (page) => () => {
    ...
    if (page === CASH_FLOW || page === BANK_ACCOUNTS) {
      const selectedAccounts = this.selectedAccounts.map(a => a.companyAccountId)
  ...
  setCheckTitleMarks = (state) => () => {
    this.setState({
      ...
      setTimeout(() => {
        this.getAggregateDataType(this.state.dateFrom, this.state.dateTill)
          .then((data) => {
            if (data) {
              if (!data.expenses && !data.incomes && !data.totalExpenses && !data.totalIncomes) {
                this.setState({
  ...
  pressItemPie = (income, index) => {
    ...
    if ((income && !typePieChooseExpense) || (!income && typePieChooseExpense)) {
      this.setState({ currentOpenItemIndex: currentOpenItemIndex === index ? null : index })
    }
  }
  ...
  _renderItem = ({ item, index }) => {
    ...
    const total = (!item.hova) ? getFormattedValueArray(item.total) : getFormattedValueArray(item.total * (-1))
    const numberStyle = !item.hova ? { color: colors.green4 } : { color: colors.red2 }
  ...
  goToMatch = () => {
    ...
    if (selectedAccounts.length > 1) {
      const primaryAccount = selectedAccounts.filter(a => a.primaryAccount === true)
  ...
  onSetDates = (dateFrom, dateTill, isDef) => {
    const isStartMonth = (AppTimezone.moment(dateFrom).isSame(AppTimezone.moment().startOf('month').format('YYYY-MM-DD'), 'year') && AppTimezone.moment(dateFrom).isSame(AppTimezone.moment().startOf('month').format('YYYY-MM-DD'), 'month') && AppTimezone.moment(dateTill).isSame(AppTimezone.moment().format('YYYY-MM-DD'), 'year') && AppTimezone.moment(dateTill).isSame(AppTimezone.moment().format('YYYY-MM-DD'), 'month'))
    const isLastMonth = (AppTimezone.moment(dateFrom).isSame(AppTimezone.moment().subtract(1, 'month').startOf('month').format('YYYY-MM-DD'), 'year') && AppTimezone.moment(dateFrom).isSame(AppTimezone.moment().subtract(1, 'month').startOf('month').format('YYYY-MM-DD'), 'month') && AppTimezone.moment(dateTill).isSame(AppTimezone.moment().subtract(1, 'month').endOf('month').format('YYYY-MM-DD'), 'year') && AppTimezone.moment(dateTill).isSame(AppTimezone.moment().subtract(1, 'month').endOf('month').format('YYYY-MM-DD'), 'month'))
    const isFromStartYear = (AppTimezone.moment(dateFrom).isSame(AppTimezone.moment().startOf('year').format('YYYY-MM-DD'), 'year') && AppTimezone.moment(dateFrom).isSame(AppTimezone.moment().startOf('year').format('YYYY-MM-DD'), 'month') && AppTimezone.moment(dateTill).isSame(AppTimezone.moment().endOf('month').format('YYYY-MM-DD'), 'year') && AppTimezone.moment(dateTill).isSame(AppTimezone.moment().endOf('month').format('YYYY-MM-DD'), 'month'))
    const isLastYear = (AppTimezone.moment(dateFrom).isSame(AppTimezone.moment().subtract(1, 'year').startOf('year').format('YYYY-MM-DD'), 'year') && AppTimezone.moment(dateFrom).isSame(AppTimezone.moment().subtract(1, 'year').startOf('year').format('YYYY-MM-DD'), 'month') && AppTimezone.moment(dateTill).isSame(AppTimezone.moment().subtract(1, 'year').endOf('year').format('YYYY-MM-DD'), 'year') && AppTimezone.moment(dateTill).isSame(AppTimezone.moment().subtract(1, 'year').endOf('year').format('YYYY-MM-DD'), 'month'))
    let stateTextMonth = ''
    if (isStartMonth) {
      ...
      stateTextMonth = LocaleConfig.locales['he'].monthNames[parseFloat(AppTimezone.moment(dateFrom).format('MM')) - 1] + ' ' + AppTimezone.moment(dateFrom).format('YYYY') +
        ' - ' + LocaleConfig.locales['he'].monthNames[parseFloat(AppTimezone.moment(dateTill).format('MM')) - 1] + ' ' + AppTimezone.moment(dateTill).format('YYYY')
    }
    if (isDef) {
    ...
    if (this.state.calendarModalIsOpen) {
      ...
      const tasksAggregateDataType = this.getAggregateDataType(AppTimezone.moment(dateFrom).valueOf(), AppTimezone.moment(dateTill).valueOf())
  ...
  handleOpenBankSite = () => {
    ...
    Linking.canOpenURL(link)
      .then(s => {
        if (s) Linking.openURL(PASSWORD_RECOVERY_LINKS[this.state.newTokenStatus.websiteTargetTypeId])
      })
  }
  ...
  markRow = (item, index, isExpense) => () => {
    ...
    if (isExpense) {
      ...
      const arrFiltered = aggregateDataTypeCopy.expenses.filter(it => it.pressed)
      const isChanged = arrFiltered.length !== aggregateDataType.expenses.length
      const totalSums = arrFiltered.reduce((sum, it) => sum + it.dateTotals[0].total, 0)
      aggregateDataTypeCopy.expenses.forEach((it) => {
        it.prc = !isChanged ? it.dateTotals[0].prc : Math.round(it.dateTotals[0].total / (totalSums / 100))
      })
    } else {
      aggregateDataTypeCopy.incomes[index].pressed = !aggregateDataTypeCopy.incomes[index].pressed
      const arrFiltered = aggregateDataTypeCopy.incomes.filter(it => it.pressed)
      const isChanged = arrFiltered.length !== aggregateDataType.incomes.length
      const totalSums = arrFiltered.reduce((sum, it) => sum + it.dateTotals[0].total, 0)
      aggregateDataTypeCopy.incomes.forEach((it) => {
        it.prc = !isChanged ? it.dateTotals[0].prc : Math.round(it.dateTotals[0].total / (totalSums / 100))
      })
    }
    this.setState({ currentOpenItemIndex: null, aggregateDataType: aggregateDataTypeCopy })
  }
  ...
  renderHeaderModalDetails = () => {
    ...
    const numberStyle = cs(dataPopUpDetails.hova, [styles.dataValue, { color: colors.green4 }], { color: colors.red2 })
      ...
      <View style={{
        ...
        <View style={{
          ...
          <Text numberOfLines={1}
            ...
            ellipsizeMode='tail'>
            <Text style={[numberStyle, { fontSize: sp(18), fontFamily: fonts.semiBold }]}>{total[0]}</Text>
          ...
          <Text style={{
            ...
            {'סה”כ '}{dataPopUpDetails && dataPopUpDetails.transes ? dataPopUpDetails.transes.length : ''}{' תנועות'}
          </Text>
        </View>
  ...
  renderItemModalDetails = ({ item, index }) => {
    ...
    const numberStyle = cs((
      (item.transId === '00000000-0000-0000-0000-000000000000' && item.total < 0)
    ...
    if (typePie === 'PAYMENT_DESC') {
      const isExist = categoriesArr.find((cat) => cat.transTypeId === item.transTypeId)
      transTypeName = isExist ? isExist.transTypeName : ''
      iconName = isExist ? getTransCategoryIcon(isExist.iconType) : null
    } else {
      transTypeName = searchkey && searchkey.length > 0 && searchkey.find((it) => it.paymentDescription === item.paymentDesc) ? searchkey.find((it) => it.paymentDescription === item.paymentDesc).name : ''
      iconName = item.paymentDesc ? getBankTransIcon(item.paymentDesc) : getBankTransIcon(null)
    }
        ...
        <View style={{
          ...
          <View style={{
            ...
            <Text numberOfLines={1} ellipsizeMode='tail'>
              <Text style={[numberStyle, { fontSize: sp(17), fontFamily: fonts.semiBold }]}>{total[0]}</Text>
              <Text style={[styles.fractionalPart, { fontSize: sp(17) }]}>.{total[1]}</Text>
            </Text>
          </View>
        </View>
      </View>
  ...
  render () {
    ...
    StatusBar.setBarStyle((IS_IOS || openedBottomSheet) ? 'dark-content' : 'light-content', true)
    ...
    const cashFlowAggregatedDataAll = cashFlowAggregatedData ? cashFlowAggregatedData.find(d => d.accountUuid === null) : null
    ...
    //   extrapolate: 'clamp',
    // })
    const currencySign = selectedGroup ? getCurrencyChar(selectedGroup) : (this.selectedAccounts.length ? getCurrencyChar(this.selectedAccounts[0].currency) : '')

    let accChoose = accounts.find((element) => element.companyAccountId === this.accountIdsForRequest[0])
    if (!accChoose) {
      accChoose = accounts[0]
    }
    const balanceLastUpdatedDateClose = (selectedAccountIds.length === 1 && accChoose && AppTimezone.moment().diff(accChoose.balanceLastUpdatedDate, 'days') <= 7)
    const balanceLastUpdatedDateFar = selectedAccountIds.length === 1 && accChoose && AppTimezone.moment().diff(accChoose.balanceLastUpdatedDate, 'days') > 7
    ...
    return (
      <Fragment>
        <AlertsTrial navigation={navigation} refresh={this._onRefresh}
          updateToken={navigation.getParam('updateToken')} />
        ...
        <View
          ...
          <Animated.ScrollView
            ...
            <Animated.View style={[{
              ...
              <View style={{
                ...
                {cashFlowAggregatedData && this.selectedAccounts && (
                  <Fragment>
                ...
                {(alertDetailsIsOpen && IS_IOS) && (
                  <AccountAlertDetails
                    ...
                    isRtl={isRtl}
                    top={100}
                    accounts={(this.xAccountsExceeded) ? this.selectedDeviantAccounts : ((cashFlowAggregatedData && accounts) ? accounts.filter(a => cashFlowAggregatedData.filter(a => a.accountUuid !== null && a.harigaDate !== null).map((b) => b.accountUuid).includes(a.companyAccountId)) : [])}
              ...
              {IS_LIGHT.light && (
                <View style={{
                  ...
                  <View style={{
                    ...
                    {(newTokenStatus && this.selectedAccounts.length > 0 && this.selectedNotUpdatedAccounts.length !== 0) &&
                    (this.state.statusToken === BANK_TOKEN_STATUS.AGREEMENT_REQUIRED ||
                      this.state.statusToken === BANK_TOKEN_STATUS.INVALID_PASSWORD_AND_ACCESS ||
                      this.state.statusToken === BANK_TOKEN_STATUS.INVALID_PASSWORD ||
                      this.state.statusToken === BANK_TOKEN_STATUS.PASSWORD_EXPIRED ||
                      this.state.statusToken === BANK_TOKEN_STATUS.BLOCKED
                    ) && (<Image style={[styles.imgIcon, { width: 42.5, height: 42.5 }]}
                      source={require('BiziboxUI/assets/passIconRed.png')} />
                    )}
                    {((this.selectedAccounts.length > 0 && this.selectedNotUpdatedAccounts.length === 0) ||
                      !(this.state.statusToken === BANK_TOKEN_STATUS.AGREEMENT_REQUIRED ||
                        this.state.statusToken === BANK_TOKEN_STATUS.INVALID_PASSWORD_AND_ACCESS ||
                        this.state.statusToken === BANK_TOKEN_STATUS.INVALID_PASSWORD ||
                        this.state.statusToken === BANK_TOKEN_STATUS.PASSWORD_EXPIRED ||
                        this.state.statusToken === BANK_TOKEN_STATUS.BLOCKED
                      )) && (<Image style={[styles.imgIcon, { width: 42.5, height: 42.5 }]} source={require('BiziboxUI/assets/passIcon.png')} />)}
                    <Text style={{
                      color: '#022258',
                      fontFamily: fonts.regular,
                      fontSize: sp(19),
                  ...
                  <View style={{
                    marginLeft: 'auto',
                    flex: (newTokenStatus && this.selectedAccounts.length > 0 && this.selectedNotUpdatedAccounts.length !== 0 && (this.state.statusToken === BANK_TOKEN_STATUS.AGREEMENT_REQUIRED || this.state.statusToken === BANK_TOKEN_STATUS.INVALID_PASSWORD_AND_ACCESS)) ? 70 : 50,
                  }}>
                    {(this.selectedAccounts.length === 0 || (!newTokenStatus && this.selectedNotUpdatedAccounts.length > 0)) && (
                      <Fragment>
                        <ActivityIndicator />
                      </Fragment>
                    )}
              ...
              {!IS_LIGHT.light && (
                <View style={{
                  ...
                  {!inProgress && cashFlowAggregatedDataAll && (cashFlowAggregatedDataAll.hovaNigrarot === null || cashFlowAggregatedDataAll.hovaNigrarot === 0) && (cashFlowAggregatedDataAll.zhutNigrarot === null || cashFlowAggregatedDataAll.zhutNigrarot === 0) && ((selectedAccountIds.length !== 1) || balanceLastUpdatedDateClose) && (
                    <Text style={{
                      color: '#0f3860',
                      fontFamily: fonts.regular,
                      fontSize: sp(16.5),
                      textAlign: 'center',
                    }}>לא קיימות תנועות שטרם נפרעו/ לא הותאמו</Text>
                  )}
                  {!balanceLastUpdatedDateFar && !inProgress && cashFlowAggregatedDataAll && ((cashFlowAggregatedDataAll.hovaNigrarot !== null && cashFlowAggregatedDataAll.hovaNigrarot > 0) || (cashFlowAggregatedDataAll.zhutNigrarot !== null && cashFlowAggregatedDataAll.zhutNigrarot > 0)) && (
                    <View style={{
                      flexDirection: 'column',
                      alignSelf: 'center',
                      justifyContent: 'center',
                      ...
                      <View style={{
                        ...
                        <Text numberOfLines={1} ellipsizeMode='tail'>
                          <Text style={[{
                            fontSize: sp(16.5),
                            color: colors.green4,
                          }, commonStyles.regularFont]}>{'₪'}{getFormattedValueArray(cashFlowAggregatedDataAll.zhutNigrarot)[0]}</Text>
                        </Text>
                        <Text style={{
                          color: '#0f3860',
                          fontFamily: fonts.regular,
                        ...
                        <Text numberOfLines={1} ellipsizeMode='tail'>
                          <Text style={[{
                            fontSize: sp(16.5),
                            color: colors.red2,
                          }, commonStyles.regularFont]}>{'₪'}{getFormattedValueArray(cashFlowAggregatedDataAll.hovaNigrarot * (-1))[0]}</Text>
                        </Text>
                      </View>
                      <View>
                        {((selectedAccountIds.length !== 1) || (balanceLastUpdatedDateClose)) && (
                          <TouchableOpacity
                            ...
                            <View style={{
                              ...
                              <Text style={styles.link}>להתאמה</Text>
                              <Icon
                                iconStyle={{
                                  alignSelf: 'center',
                                  justifyContent: 'center',
                        ...
                        {(balanceLastUpdatedDateFar) && (
                          <View style={{
                            ...
                            <Text style={{
                              ...
                              color: '#0f3860',
                              textAlign: 'center',
                            }}>קיימות תנועות שטרם נפרעו/לא תואמו, עדכנו את החשבון על מנת
                              לבצע התאמה</Text>
                          </View>
                        )}
                      </View>
                      ...
                      <View>
                        <View style={{
                          ...
                          <Text style={{
                            ...
                            color: '#0f3860',
                            textAlign: 'center',
                          }}>קיימות תנועות שטרם נפרעו/לא תואמו, עדכנו את החשבון על מנת לבצע
                            התאמה</Text>
                        </View>
                      </View>
                    </View>
            ...
            <View style={{
              ...
              <View style={{
                ...
                {(this.accountsBalanceTotal !== 'false') && (
                  <Text style={{
                    color: (this.accountsBalanceTotal > 0) ? '#229f88' : '#ef3636',
                    fontFamily: fonts.regular,
                    fontSize: sp(27),
                    textAlign: 'center',
                    lineHeight: 26,
              ...
              <View style={{
                ...
                <UncontrolledChartSlider
                  ...
                  dateTillTimestamp={IS_LIGHT.light ? AppTimezone.moment().valueOf() : dateTillTimestamp}
                />
              </View>
              <View style={{
                flexDirection: 'row-reverse',
                ...
                <TouchableOpacity
                  ...
                  <Text style={styles.link}>{'למסך חשבונות בנק'}</Text>
                </TouchableOpacity>
                {!IS_LIGHT.light && (
                  <TouchableOpacity
                    style={{
                    ...
                    <Text style={styles.link}>{'למסך תזרים יומי'}</Text>
                  </TouchableOpacity>
                )}
              </View>
            </View>
            ...
            <View style={{
              ...
              <TouchableOpacity onPress={this.setDatesCalendarOpen}>
                <View style={{
                  ...
                  <Text style={styles.link}>{stateTextMonth}</Text>
                  <Icon
                    iconStyle={{
                      alignSelf: 'center',
                      justifyContent: 'center',
              ...
              <Animated.ScrollView
                ...
                onScroll={(e) => {
                  this.setState({
                    currentOpenItemIndex: null,
                    typePieChooseExpense: e.nativeEvent.contentOffset.x > 110,
                  })
                }}
                ...
                <View style={{
                  ...
                  {aggregateDataType && aggregateDataType.incomes && (
                    <Pie
                      currentOpenItemIndex={!typePieChooseExpense ? currentOpenItemIndex : null}
                      press={this.pressItemPie}
                      title={{
                        label: 'הפקדות',
                        value: aggregateDataType.incomes.filter(it => it.pressed).length > 0 ? ((aggregateDataType.incomes.filter(it => it.pressed).length === aggregateDataType.incomes.length) ? aggregateDataType.totalIncomes : aggregateDataType.incomes.filter(it => it.pressed).reduce((sum, item) => sum + item.dateTotals[0].total, 0)) : null,
                        income: true,
                      }}
                      keys={aggregateDataType.incomes.filter(it => it.pressed).length > 0 ? aggregateDataType.incomes.filter(it => it.pressed).map((item, index) => {
                        return {
                      ...
                      colors={['#02622f', '#11713e', '#46a673', '#58b885', '#68c895', '#85e5b2']}
                      prc={aggregateDataType.incomes.filter(it => it.pressed).length > 0
                        ? ((aggregateDataType.incomes.filter(it => it.pressed).map((item) => item.prc))) : [0]}
                      values={aggregateDataType.incomes.filter(it => it.pressed).length > 0 ? aggregateDataType.incomes.filter(it => it.pressed).map((item) => item.dateTotals[0].total) : [100]}
                    />
                  )}
                </View>
                <View style={{
                  ...
                  {aggregateDataType && aggregateDataType.expenses && (
                    <Pie
                      currentOpenItemIndex={typePieChooseExpense ? currentOpenItemIndex : null}
                      press={this.pressItemPie}
                      title={{
                        label: 'משיכות',
                        value: aggregateDataType.expenses.filter(it => it.pressed).length > 0 ? ((aggregateDataType.expenses.filter(it => it.pressed).length === aggregateDataType.expenses.length) ? aggregateDataType.totalExpenses : aggregateDataType.expenses.filter(it => it.pressed).reduce((sum, item) => sum + item.dateTotals[0].total, 0)) : null,
                        income: false,
                      }}
                      keys={
                        aggregateDataType.expenses.filter(it => it.pressed).length > 0 ? aggregateDataType.expenses.filter(it => it.pressed).map((item, index) => {
                          return {
                            val: item.description,
                            idx: index,
                            prc: item.prc,
                      ...
                      colors={['#d70000', '#ee0000', '#fb4f56', '#fa696e', '#f88387', '#ffaaac']}
                      prc={aggregateDataType.expenses.filter(it => it.pressed).length > 0
                        ? ((aggregateDataType.expenses.filter(it => it.pressed).map((item) => item.prc))) : [0]}
                      values={aggregateDataType.expenses.filter(it => it.pressed).length > 0 ? aggregateDataType.expenses.filter(it => it.pressed).map((item) => item.dateTotals[0].total) : [100]}
                    />
                  )}
                </View>
              </Animated.ScrollView>
              <View
                ...
                <View style={{ flex: 1 }}>
                  <TouchableOpacity onPress={this.setCheckTitleMarks(true)}>
                    <View style={{
                      backgroundColor: this.state.typePie === 'PAYMENT_DESC' ? '#08d3b8' : '#ffffff',
                      height: 33,
                      flex: 1,
                      width: '100%',
                      ...
                      <Text style={[{
                        fontFamily: this.state.typePie !== 'PAYMENT_DESC' ? fonts.regular : fonts.semiBold,
                        textAlign: 'center',
                        width: '100%',
                        color: this.state.typePie !== 'PAYMENT_DESC' ? '#022258' : '#ffffff',
                        fontSize: sp(24),
                      }]}
                      >{'לפי סוג תשלום'}</Text>
                    </View>
                ...
                <View style={{ flex: 1 }}>
                  <TouchableOpacity onPress={this.setCheckTitleMarks(false)}>
                    <View style={{
                      backgroundColor: this.state.typePie === 'TRANS_TYPE' ? '#08d3b8' : '#ffffff',
                      height: 33,
                      flex: 1,
                      width: '100%',
                      ...
                      <Text style={[{
                        fontFamily: this.state.typePie !== 'TRANS_TYPE' ? fonts.regular : fonts.semiBold,
                        textAlign: 'center',
                        width: '100%',
                        color: this.state.typePie !== 'TRANS_TYPE' ? '#022258' : '#ffffff',
                        fontSize: sp(24),
                      }]}
                      >{'לפי קטגוריות'}</Text>
                    </View>
              ...
              {aggregateDataType && typePieChooseExpense && aggregateDataType.expenses && aggregateDataType.expenses.map((item, index) => {
                return (
              ...
              {!progressPies && aggregateDataType && typePieChooseExpense && aggregateDataType.expenses && aggregateDataType.expenses.length === 0 && (
                <View style={styles.errorWrapper}>
                  <View
                    style={[styles.errorDivider, { backgroundColor: colors.blue32 }]} />
                  <Text
                    style={[styles.errorText, { color: colors.blue32 }]}>{'לא נמצאו נתוני הוצאות'}</Text>
                  <View
                    style={[styles.errorDivider, { backgroundColor: colors.blue32 }]} />
                </View>
              )}

              {progressPies && aggregateDataType && typePieChooseExpense && (
                <Fragment>
                  <ActivityIndicator />
                </Fragment>
              )}

              {aggregateDataType && !typePieChooseExpense && aggregateDataType.incomes && aggregateDataType.incomes.map((item, index) => {
                return (
                  <AggregateDataTypeRow
                    translates={typePie === 'PAYMENT_DESC'}
                    expenses={false}
              ...
              {!progressPies && aggregateDataType && !typePieChooseExpense && aggregateDataType.incomes && aggregateDataType.incomes.length === 0 && (
                <View style={styles.errorWrapper}>
                  <View
                    style={[styles.errorDivider, { backgroundColor: colors.blue32 }]} />
                  <Text
                    style={[styles.errorText, { color: colors.blue32 }]}>{'לא נמצאו נתוני הכנסות'}</Text>
                  <View
                    style={[styles.errorDivider, { backgroundColor: colors.blue32 }]} />
                </View>
              )}
            ...
            <View style={{
              ...
              <View style={{
                ...
                <TouchableHighlight
                  ...
                  cs((IS_LIGHT.light || financialData.inChecks !== 0), {
                    shadowOpacity: 0,
                    elevation: 0,
                  ...
                  <View style={[
                    ...
                    cs((financialData.inChecks !== 0), { opacity: 0.8 }, { opacity: 1 }),
                    cs(this.state.pressed && (financialData.inChecks !== 0),
                      {
                        backgroundColor: '#ffffff',
                      },
                      {
                    ...
                    <Image style={{ width: 79 / 2, height: 72 / 2 }} source={require('BiziboxUI/assets/inChecks.png')} />
                    <View style={{
                      width: 7.5,
                    }} />
                    <View style={{
                      ...
                      {IS_LIGHT.light && (
                        ...
                        />)}
                      <Text style={{
                        color: IS_LIGHT.light ? '#aeaeae' : '#0f3860',
                        fontFamily: fonts.regular,
                        fontSize: sp(16),
                      }}>
                        צ׳קים לפירעון
                      ...
                      {!IS_LIGHT.light && (
                        <Text style={{
                          color: (financialData.inChecks !== 0) ? '#1d8a50' : '#0f3860',
                          fontFamily: fonts.semiBold,
                          fontSize: sp(20),
                        }}>
                          {(financialData.inChecks !== 0) ? getFormattedValueArray(Math.round(financialData.inChecks))[0] : '-'}
                        </Text>
                      )}
                    </View>
                  </View>
                ...
                <TouchableHighlight
                  ...
                  cs((IS_LIGHT.light || financialData.outChecks !== 0), {
                    shadowOpacity: 0,
                    elevation: 0,
                  }, { shadowOpacity: 1, elevation: 2 }),
                  ...
                  <View style={[
                    ...
                    cs((financialData.outChecks !== 0), { opacity: 0.8 }, { opacity: 1 }),
                    cs(this.state.pressed1 && (financialData.outChecks !== 0),
                      {
                        backgroundColor: '#ffffff',
                      },
                      {
                    ...
                    <Image style={{ width: 79 / 2, height: 80 / 2 }}
                      source={require('BiziboxUI/assets/outChecks.png')} />
                    <View style={{
                      width: 7.5,
                    }} />
                    <View style={{
                      ...
                      {!IS_LIGHT.light && (
                        <Text style={{
                          color: (financialData.outChecks !== 0) ? '#ef3636' : '#0f3860',
                          fontFamily: fonts.semiBold,
                          fontSize: sp(20),
                        }}>
                          {(financialData.outChecks !== 0) ? getFormattedValueArray(Math.round(financialData.outChecks))[0] : '-'}
                        </Text>
                      )}
                    </View>
                  </View>
              ...
              <View style={{
                ...
                <TouchableHighlight
                  ...
                  <View style={[
                    ...
                    cs((financialData.mishmeretBabank !== 0), { opacity: 0.8 }, { opacity: 1 }),
                    cs(this.state.pressed2 && (financialData.mishmeretBabank !== 0),
                      {
                        backgroundColor: '#ffffff',
                      },
                      {
                    ...
                    <Image style={{ width: 73 / 2, height: 80 / 2 }}
                      source={require('BiziboxUI/assets/mishmeretBabank.png')} />
                    <View style={{
                      width: 7.5,
                    }} />
                    <View style={{
                      ...
                      <Text style={{
                        color: (financialData.mishmeretBabank !== 0) ? '#1d8a50' : '#0f3860',
                        fontFamily: fonts.semiBold,
                        fontSize: sp(20),
                      }}>
                        {(financialData.mishmeretBabank !== 0) ? getFormattedValueArray(Math.round(financialData.mishmeretBabank))[0] : '-'}
                      </Text>
                    </View>
                  </View>
                </TouchableHighlight>
                ...
                <TouchableHighlight
                  ...
                  <View style={[
                    ...
                    cs((financialData.lenicaion !== 0), { opacity: 0.8 }, { opacity: 1 }),
                    cs(this.state.pressed3 && (financialData.lenicaion !== 0),
                      {
                        backgroundColor: '#ffffff',
                      },
                      {
                    ...
                    <Image style={{ width: 73 / 2, height: 80 / 2 }}
                      source={require('BiziboxUI/assets/lenicaion.png')} />
                    <View style={{
                      width: 7.5,
                    }} />
                    <View style={{
                      ...
                      <Text style={{
                        color: (financialData.lenicaion !== 0) ? '#1d8a50' : '#0f3860',
                        fontFamily: fonts.semiBold,
                        fontSize: sp(20),
                      }}>
                        {(financialData.lenicaion !== 0) ? getFormattedValueArray(Math.round(financialData.lenicaion))[0] : '-'}
                      </Text>
                    </View>
                  </View>
                </TouchableHighlight>
              ...
              <View style={{
                ...
                <TouchableHighlight
                  ...
                  <View style={[
                    ...
                    cs((financialData.loans !== 0), { opacity: 0.8 }, { opacity: 1 }),
                    cs(this.state.pressed4 && (financialData.loans !== 0),
                      {
                        backgroundColor: '#ffffff',
                      },
                      {
                    ...
                    <Image style={{ width: 59 / 2, height: 57 / 2 }} source={require('BiziboxUI/assets/loans.png')} />
                    <View style={{
                      width: 7.5,
                    }} />
                    <View style={{
                      ...
                      <Text style={{
                        color: (financialData.loans !== 0) ? '#ef3636' : '#0f3860',
                        fontFamily: fonts.semiBold,
                        fontSize: sp(20),
                      }}>
                        {(financialData.loans !== 0) ? getFormattedValueArray(Math.round(financialData.loans))[0] : '-'}
                      </Text>
                    </View>
                  </View>
                </TouchableHighlight>
                ...
                <TouchableHighlight
                  ...
                  <View style={[
                    ...
                    cs((financialData.deopsit !== 0), { opacity: 0.8 }, { opacity: 1 }),
                    cs(this.state.pressed5 && (financialData.deopsit !== 0),
                      {
                        backgroundColor: '#ffffff',
                      },
                      {
                    ...
                    <Image style={{ width: 58 / 2, height: 58 / 2 }} source={require('BiziboxUI/assets/deopsit.png')} />
                    <View style={{
                      width: 7.5,
                    }} />
                    <View style={{
                      ...
                      <Text style={{
                        color: (financialData.deopsit !== 0) ? '#1d8a50' : '#0f3860',
                        fontFamily: fonts.semiBold,
                        fontSize: sp(20),
                      }}>
                        {(financialData.deopsit !== 0) ? getFormattedValueArray(Math.round(financialData.deopsit))[0] : '-'}
                      </Text>
                    </View>
                  </View>
                </TouchableHighlight>
              ...
              <View style={{
                ...
                <TouchableHighlight
                  ...
                  <View style={[
                    ...
                    cs((financialData.creditCard !== 0), { opacity: 0.8 }, { opacity: 1 }),
                    cs(this.state.pressed6 && (financialData.creditCard !== 0),
                      {
                        backgroundColor: '#ffffff',
                      },
                      {
                    ...
                    <Image style={{ width: 63 / 2, height: 58 / 2 }}
                    ...
                    <View style={{
                      ...
                      <Text style={{
                        color: (financialData.creditCard !== 0) ? '#ef3636' : '#0f3860',
                        fontFamily: fonts.semiBold,
                        fontSize: sp(20),
                      }}>
                        {(financialData.creditCard !== 0) ? getFormattedValueArray(Math.round(financialData.creditCard))[0] : '-'}
                      </Text>
                    </View>
                  </View>
                </TouchableHighlight>
                ...
                <TouchableHighlight
                  ...
                  <View style={[
                    ...
                    cs((financialData.slika !== 0), { opacity: 0.8 }, { opacity: 1 }),
                    cs(this.state.pressed7 && (financialData.slika !== 0),
                      {
                        backgroundColor: '#ffffff',
                      },
                      {
                    ...
                    <Image style={{ width: 57 / 2, height: 63 / 2 }}
                    ...
                    <View style={{
                      ...
                      <Text style={{
                        color: (financialData.slika !== 0) ? '#1d8a50' : '#0f3860',
                        fontFamily: fonts.semiBold,
                        fontSize: sp(20),
                      }}>
                        {(financialData.slika !== 0) ? getFormattedValueArray(Math.round(financialData.slika))[0] : '-'}
                      </Text>
                    </View>
                  </View>
                </TouchableHighlight>
            ...
            <View style={{
              ...
              {(bankTrans && bankTrans.length > 0) && (
                <TouchableOpacity onPress={this.linkToPage(BANK_ACCOUNTS)}>
                  <View style={{
                    flexDirection: 'row-reverse',
                    alignSelf: 'center',
                    justifyContent: 'center',
                    ...
                    <Text style={styles.link}>{'לכל התנועות'}</Text>
                    <Icon
                      iconStyle={{
                        alignSelf: 'center',
                        justifyContent: 'center',
              ...
              {(bankTrans && bankTrans.length === 0) && (
                <View style={{
                  ...
                  <View style={[styles.errorWrapper, {
                    ...
                    <View style={[styles.errorDivider, { backgroundColor: colors.blue32 }]} />
                    <Text
                      style={[styles.errorText, { color: colors.blue32 }]}>{'לא נמצאו תנועות'}</Text>
                    <View style={[styles.errorDivider, { backgroundColor: colors.blue32 }]} />
                  </View>
                </View>
              )}
          ...
          <Animated.View
            ...
            <View style={{
              ...
              <Text style={styles.bankAccountTitle}>{t('mainMenu:overview')}</Text>
              <View style={{
            ...
            <SafeAreaView style={{
              ...
              <View style={{
                  ...
                  <View style={cs(
                    ...
                    <View style={{ alignItems: 'center', flex: 70, alignSelf: 'center' }}>
                      <Text style={{
                        ...
                        {typePie === 'PAYMENT_DESC'
                          ? (this.props.searchkey && this.props.searchkey.length > 0 && this.props.searchkey.length > 0 && this.props.searchkey.find((it) => it.paymentDescription === dataPopUpDetails.description) ? this.props.searchkey.find((it) => it.paymentDescription === dataPopUpDetails.description).name : '')
                ...
                <View style={{
                  ...
                  <FlatList
                    ListEmptyComponent={progressModalDetails ? <ActivityIndicator /> : <View />}
                  ...
                  <SafeAreaView style={{
                    ...
                    <View style={{
                      ...
                      <View style={{
                        ...
                        <View style={cs(
                          ...
                          <View style={{
                            ...
                            <TouchableOpacity
                              onPress={this.setStates({ deleteLoanModal: false })}>
                              <Text style={{
                                fontSize: sp(16),
                                color: '#ffffff',
                                fontFamily: fonts.semiBold,
                      ...
                      <View style={{
                        ...
                        <View>
                          <TouchableOpacity onPress={this.deleteRowLoan}>
                    ...
                    <View style={{
                      ...
                      <View style={cs(
                        ...
                        <View style={{
                          ...
                          <TouchableOpacity
                            onPress={this.setStates({ showLoanRowModal: false })}>
                            <View>
                              <Icon name='chevron-right' size={24} color={colors.white} />
                            </View>
                          </TouchableOpacity>
                    ...
                    <View style={{
                            ...
                            <View style={{}}>
                              {loanRow.companyAccountId && (
                                <View
                                  ...
                                  <AccountIcon
                                    account={accounts.find(a => a.companyAccountId === loanRow.companyAccountId)} />
                                  <View style={commonStyles.spaceDivider} />
                                  <Text style={{
                                    fontFamily: fonts.regular,
                                    textAlign: 'left',
                          ...
                          <View
                            ...
                            <View style={{}}>
                              <Text style={{
                                ...
                                fontSize: sp(15),
                                lineHeight: 30,
                              }}>{AppTimezone.moment(loanRow.loanDate).format('DD/MM/YY')}</Text>
                            </View>
                          </View>
                          <View
                            style={[cs(isRtl, commonStyles.row, [commonStyles.rowReverse]), {
                            ...
                            <View style={{}}>
                              <Text style={{
                                ...
                                fontSize: sp(15),
                                lineHeight: 30,
                              }}>{AppTimezone.moment(loanRow.loanFinish).format('DD/MM/YY')}</Text>
                            </View>
                          </View>
                          <View
                            style={[cs(isRtl, commonStyles.row, [commonStyles.rowReverse]), {
                        ...
                        <View style={{
                          ...
                          <View style={{
                            ...
                            <PieChart
                              ...
                              data={[{
                                prc: 1,
                                active: ((loanRow.loanPayments - loanRow.paymentsNumberLeft) / (loanRow.loanPayments / 12)) >= 1,
                              }, {
                                prc: 1,
                                active: ((loanRow.loanPayments - loanRow.paymentsNumberLeft) / (loanRow.loanPayments / 12)) >= 2,
                              }, {
                                prc: 1,
                                active: ((loanRow.loanPayments - loanRow.paymentsNumberLeft) / (loanRow.loanPayments / 12)) >= 3,
                              }, {
                                prc: 1,
                                active: ((loanRow.loanPayments - loanRow.paymentsNumberLeft) / (loanRow.loanPayments / 12)) >= 4,
                              }, {
                                prc: 1,
                                active: ((loanRow.loanPayments - loanRow.paymentsNumberLeft) / (loanRow.loanPayments / 12)) >= 5,
                              }, {
                                prc: 1,
                                active: ((loanRow.loanPayments - loanRow.paymentsNumberLeft) / (loanRow.loanPayments / 12)) >= 6,
                              }, {
                                prc: 1,
                                active: ((loanRow.loanPayments - loanRow.paymentsNumberLeft) / (loanRow.loanPayments / 12)) >= 7,
                              }, {
                                prc: 1,
                                active: ((loanRow.loanPayments - loanRow.paymentsNumberLeft) / (loanRow.loanPayments / 12)) >= 8,
                              }, {
                                prc: 1,
                                active: ((loanRow.loanPayments - loanRow.paymentsNumberLeft) / (loanRow.loanPayments / 12)) >= 9,
                              }, {
                                prc: 1,
                                active: ((loanRow.loanPayments - loanRow.paymentsNumberLeft) / (loanRow.loanPayments / 12)) >= 10,
                              }, {
                                prc: 1,
                                active: ((loanRow.loanPayments - loanRow.paymentsNumberLeft) / (loanRow.loanPayments / 12)) >= 11,
                              }, {
                                prc: 1,
                                active: ((loanRow.loanPayments - loanRow.paymentsNumberLeft) / (loanRow.loanPayments / 12)) >= 12,
                              }].map((key, index) => {
                                return {
                                  value: key['prc'],
                                  key: `pie-${index}`,
                                  prc: key['prc'],
                                  svg: { fill: !key.active ? '#cee8e9' : '#29c2c7' },
                            ...
                            <View style={{
                              ...
                              <Text
                                ...
                                {`${loanRow.loanPayments - loanRow.paymentsNumberLeft}/${loanRow.loanPayments}`}
                              </Text>
                            </View>
                          </View>
                        </View>
                      ...
                      <View style={{
                        ...
                        <View style={[cs(isRtl, commonStyles.row, [commonStyles.rowReverse]), {
                          ...
                          <View style={{
                            ...
                            <Text style={{
                              ...
                              fontSize: sp(17),
                              lineHeight: 42,
                            }}>{getFormattedValueArray(loanRow.loanOriginalTotal)[0]}</Text>
                          </View>
                        </View>
                        <View style={[cs(isRtl, commonStyles.row, [commonStyles.rowReverse]), {
                          height: 42,
                          ...
                          <View style={{
                            ...
                            <Text style={{
                              ...
                              fontSize: sp(17),
                              lineHeight: 42,
                            }}>{getFormattedValueArray(loanRow.loanTotalLeft)[0]}</Text>
                          </View>
                        </View>
                        <View style={[cs(isRtl, commonStyles.row, [commonStyles.rowReverse]), {
                          height: 42,
                        ...
                        <View style={[cs(isRtl, commonStyles.row, [commonStyles.rowReverse]), {
                          ...
                          <View style={{
                            ...
                            <Text style={{
                              ...
                              fontSize: sp(17),
                              lineHeight: 42,
                            }}>{getFormattedValueArray(loanRow.nextPaymentTotal)[0]}</Text>
                          </View>
                        </View>
                        <View style={[cs(isRtl, commonStyles.row, [commonStyles.rowReverse]), {
                          height: 42,
                          ...
                          <View style={{
                            ...
                            <Text style={{
                              ...
                              fontSize: sp(17),
                              lineHeight: 42,
                            }}>{AppTimezone.moment(loanRow.loanNextPaymentDate).format('DD/MM/YY')}</Text>
                          </View>
                        </View>
                        ...
                        <View style={[cs(isRtl, commonStyles.row, [commonStyles.rowReverse]), {
                          ...
                          <Text>{' '}</Text>
                          <TouchableOpacity onPress={this.setStates({ deleteLoanModal: true })}>
                            <Text style={{
                              color: '#2aa1d9',
                              fontSize: sp(15),
                              lineHeight: 42,
              ...
              <SafeAreaView style={{
                ...
                <View style={{
                  ...
                  <View style={{
                    ...
                    <View style={cs(
                      ...
                      <View style={{
                        ...
                        <TouchableOpacity
                          onPress={this.setStates({ loanRow: {}, loanModalIsOpen: false })}>
                          <Text style={{
                            fontSize: sp(16),
                            color: '#ffffff',
                            fontFamily: fonts.semiBold,
                  ...
                  <View style={{
                    ...
                    <ScrollView>
                      {loans && loans.length > 0 && loans.map((item, index) => {
                        return (
                          <TouchableOpacity
                            key={index.toString()}
                            style={[cs(!isRtl, commonStyles.row, [commonStyles.rowReverse]), {
                            ...
                            <View style={{ flex: 16, alignItems: 'flex-end' }}>
                              <Text style={{
                                color: '#063c63',
                                fontSize: sp(15),
                                lineHeight: 50,
                  ...
                  <SafeAreaView style={{
                    ...
                    <View style={{
                      ...
                      <View style={{
                        ...
                        <View style={cs(
                          ...
                          <View style={{
                            ...
                            <TouchableOpacity
                              onPress={this.setStates({ deleteDepositModal: false })}>
                              <Text style={{
                                fontSize: sp(16),
                                color: '#ffffff',
                                fontFamily: fonts.semiBold,
                      ...
                      <View style={{
                        ...
                        <View>
                          <TouchableOpacity onPress={this.deleteRowDeposit}>
                            <Text style={[styles.link, {
                              color: '#2aa1d9',
                              fontSize: sp(16),
                              fontFamily: fonts.regular,
                  ...
                  <View style={{
                    ...
                    <View style={{
                      ...
                      <View style={cs(
                        ...
                        <View style={{
                          ...
                          <TouchableOpacity
                            onPress={this.setStates({ showDepositRowModal: false })}>
                            <View>
                              <Icon name='chevron-right' size={24} color={colors.white} />
                            </View>
                          </TouchableOpacity>
                    ...
                    <View style={{
                        ...
                        <View style={{
                          ...
                          {depositRow.companyAccountId && (
                            <Text style={{
                              ...
                              fontSize: sp(17),
                              lineHeight: 42,
                            }}>{t(`bankName:${accounts.find(a => a.companyAccountId === depositRow.companyAccountId).bankId}`)} {depositRow.bankAccountId}</Text>)}
                        </View>
                      </View>
                      <View style={[cs(isRtl, commonStyles.row, [commonStyles.rowReverse]), {
                        height: 42,
                        ...
                        <View style={{
                          ...
                          <Text style={{
                            ...
                            color: '#063c63',
                            fontSize: sp(17),
                          }}>{getFormattedValueArray(depositRow.depositAsTotal)[0]}</Text>
                        </View>
                      </View>
                      <View style={[cs(isRtl, commonStyles.row, [commonStyles.rowReverse]), {
                        height: 42,
                        ...
                        <View style={{
                          ...
                          <Text style={{
                            ...
                            color: '#063c63',
                            fontSize: sp(17),
                          }}>{AppTimezone.moment(depositRow.depositDate).format('DD/MM/YY')}</Text>
                        </View>
                      </View>
                      <View style={[cs(isRtl, commonStyles.row, [commonStyles.rowReverse]), {
                        height: 42,
                        ...
                        <View style={{
                          ...
                          <Text style={{
                            ...
                            color: '#063c63',
                            fontSize: sp(17),
                          }}>{AppTimezone.moment(depositRow.dueDate).format('DD/MM/YY')}</Text>
                        </View>
                      </View>
                      ...
                      <View style={[cs(isRtl, commonStyles.row, [commonStyles.rowReverse]), {
                        ...
                        <View style={{
                          ...
                          <Text style={{
                            ...
                            color: '#063c63',
                            fontSize: sp(17),
                          }}>{AppTimezone.moment(depositRow.depositExistStation).format('DD/MM/YY')}</Text>
                        </View>
                      </View>
                      ...
                      <View style={[cs(isRtl, commonStyles.row, [commonStyles.rowReverse]), {
                        ...
                        <Text>{' '}</Text>
                        <TouchableOpacity onPress={this.setStates({ deleteDepositModal: true })}>
                          <Text style={{
                            color: '#2aa1d9',
                            fontSize: sp(15),
                            lineHeight: 42,
              ...
              <SafeAreaView style={{
                ...
                <View style={{
                  ...
                  <View style={{
                    ...
                    <ScrollView>
                      {deposit && deposit.length > 0 && deposit.map((item, index) => {
                        return (
                          <TouchableOpacity
                            key={index.toString()}
                            style={[cs(!isRtl, commonStyles.row, [commonStyles.rowReverse]), {
                            ...
                            <View style={{
                              ...
                              <Icon name='chevron-left' size={24} color={'#063c63'} />
                            </View>
          ...
          <View style={{
            ...
            height: 33.5,
            width: 33.5,
            backgroundColor: statusNotValid || this.hasHeaderAlert ? '#ef3636' : '#0addc1',
            borderRadius: 16.75,
            position: 'absolute',
            elevation: 6,
            zIndex: 60,
            ...
            <Icon
              name={statusNotValid || this.hasHeaderAlert ? 'exclamation' : 'check'}
              type={statusNotValid || this.hasHeaderAlert ? 'material-community' : 'feather'}
              size={20}
              color={'#ffffff'}
            />
          </View>
            ...
            <Animated.View style={{
              ...
              <View style={{
                ...
                <Image
                  ...
                  source={require('BiziboxUI/assets/bgPopUpgrade.png')}
                />
                ...
                <View style={{
                  ...
                  <Image
                    ...
                    source={require('BiziboxUI/assets/quPopupUpgrade.png')}
                  />
                  <Text style={{
                    color: '#022258',
                    fontSize: sp(13.5),
              ...
              <View style={{
                ...
                {showLastRowPopup && (
                  <View style={{
                    ...
                    <Image style={{ width: 73 / 2, height: 74 / 2, alignSelf: 'center' }}
          ...
          <BottomSheetMutavim navigation={navigation}
            company={this.props.companies.find(acc => acc.companyId === this.props.currentCompanyId)} />
        </View>
      </Fragment>
    )
  }
