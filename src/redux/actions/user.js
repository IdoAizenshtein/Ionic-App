    ...
    return dispatch({
      ...
      async: defaultUserScreenApi.post({ body: { screenName } })
        .then(res => {
          if (!res || isEmpty(res)) {
            return {
              displayMode: ((screenName !== 'checksMobile') ? SCREEN_MODES.aggregate : SCREEN_MODES.inChecks),
              screenName,
            }
          }
          return res
...
export function setPopAddCheckShow (show) {
  return dispatch => {
    return dispatch({ type: SET_SHOW_POP_ADD_CHECK, payload: show })
  }
}

export function setPopAddCreateTranShow (show) {
  return dispatch => {
    return dispatch({ type: SET_SHOW_POP_ADD_TRANS, payload: show })
  }
}

export function setPopAddCashShow (show) {
  return dispatch => {
    return dispatch({ type: SET_SHOW_POP_ADD_CASH, payload: show })
  }
}

export function setRememberMe (boolMark) {
  return dispatch => {
    return dispatch({ type: SET_REMEMBER_ME, payload: boolMark })
  }
}

export function setTaryePopupTimes (times) {
  return dispatch => {
    return dispatch({ type: SET_TIMES_OF_TARYE, payload: times })
  }
}

export function setMutavimScreenBottomSheetTimes (times) {
  return dispatch => {
    return dispatch({ type: SET_TIMES_OF_MUTAVIM, payload: times })
  }
}