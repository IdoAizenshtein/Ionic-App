...
export function selectAccounts (selectedAccountIds = []) {
  return dispatch => {
    return dispatch({ type: SELECT_ACCOUNT, payload: selectedAccountIds })
  }
}
...
export function getAccountBalanceChartData ({ companyAccountIds, dateFrom, dateTill }) {
  return dispatch => {
    return dispatch({
      ...
      async: accountCflChartDataApi.post({ body: { companyAccountIds, dateFrom, dateTill } }),
    })
  }
}
...
export function getAccountBankTrans ({ companyAccountIds, companyId, dateFrom, dateTill }) {
  return dispatch => {
    const body = { companyAccountIds, companyId, dateFrom, dateTill }

    return dispatch({
      type: GET_BANK_TRANS.START,
      async: accountCflDataApi.post({ body })
        .then(res => {
          if (res && res.bankTransList) return res.bankTransList
          return []
        }),
    })
  }
...
export function updateBankTrans (newBankTrans) {
  return (dispatch, getState) => {
    if (!newBankTrans || !newBankTrans.bankTransId) return
    const { accountBankTrans } = getState()

    const oldIndex = accountBankTrans.findIndex(t => t.bankTransId === newBankTrans.bankTransId)
    if (oldIndex < 0) return
    const newData = [...accountBankTrans]
    newData[oldIndex] = { ...newData[oldIndex], ...newBankTrans }

    return dispatch({ type: UPDATE_BANK_TRANS_IN_STORE, payload: newData })
  }
}