...
export const getCashFlowDetailsDataBySections = createSelector(
  ...
  (cashFlowDetailsData) => {
    if (cashFlowDetailsData && cashFlowDetailsData.cashFlowDetails) {
      cashFlowDetailsData.cashFlowDetails = cashFlowDetailsData.cashFlowDetails.reduce((memo, trans, currentIndex) => {
        ...
        if (oldIndex > -1) {
          ...
          //   }, 0)
          // }
          memo.push({ first, title, data: [trans] })
        }
...
export const cashFlowAggregatedCalculateNigrarot = createSelector(
  ...
  (cashFlowAggregatedData) => {
    const accountUuidAll = cashFlowAggregatedData ? cashFlowAggregatedData.find(d => d.accountUuid === null) : null
    if (!cashFlowAggregatedData || !accountUuidAll || (accountUuidAll && !accountUuidAll.accountTransactions)) return null

    const newData = cloneDeep(accountUuidAll)

    newData.accountTransactions[0].hova = newData.accountTransactions[0].hova - newData.hovaNigrarot
    newData.accountTransactions[0].zhut = newData.accountTransactions[0].zhut - newData.zhutNigrarot

    if (newData.accountTransactions[0].transDate) {
      const daysBetween = AppTimezone.moment().diff(AppTimezone.moment(newData.accountTransactions[0].transDate), 'days')
