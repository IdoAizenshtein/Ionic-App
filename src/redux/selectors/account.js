...
export const getAccountSortByDate = createSelector(
  ...
  (accounts) => {
    return [...accounts]
      .sort((a, b) => {
        if (a.primaryAccount === b.primaryAccount) return a.dateCreated - b.dateCreated
...
export const getAccountTodayTransAggregated = createSelector(
  ...
  (accountTodayTrans) => {
    ...
    return accountTodayTrans.reduce((memo, trans) => {
      ...
      return memo
    }, { zhutTotal: 0, hovaTotal: 0, transDate: AppTimezone.moment().valueOf() })
  },
)
...
export const getAccountGroups = createSelector(
  ...
  (accounts) => {
    return cloneDeep(accounts)
      ...
      .reduce((memo, a) => {
        const currency = a.currency.toLowerCase()
...
export const getAccountsWithCreditCards = createSelector(
  ...
  (accounts, creditCards) => {
    return accounts.filter(a => creditCards.some(c => c.companyAccountId === a.companyAccountId))
  },
)
...
export const getAccountsWithSlika = createSelector(
  ...
  (accounts, slika) => {
    return accounts.filter(a => slika.solekDetails && slika.solekDetails.some(c => c.companyAccountId === a.companyAccountId))
  },
)
...
export const getAccountBankTransBySections = createSelector(
  ...
  (bankTrans, todayTrans) => {
    ...
    return [{ title: 'היום (לא סופי)', data: [...todayTrans] }, ...newBankTrans]
  },
)