          ...
          >
            {accounts.map((a, i) => {
              ...
              return (
                ...
                >
                  <Text style={styles.alertDetailsText} numberOfLines={1}>{a.accountNickname}</Text>
                  {isAccountNotUpdated
                    ? (
                      <Text style={[styles.alertAdditionalText, { color: colors.red2 }]}>
                        {t(
                          'bankAccount:lastUpdatedXDaysAgo',
                          { days: moment().diff(moment(a.balanceLastUpdatedDate), 'days') },
                        )}
                      </Text>
                    )
                    : <Text style={styles.alertValueText}
                      numberOfLines={1}>{getCurrencyChar(a.currency)} {`${value[0]}.${value[1]}`}</Text>}
