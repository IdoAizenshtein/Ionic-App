...
export default class AccountsModal extends PureComponent {
  constructor (props) {
    ...
    handleSelectAccountOfGroup = (currency, id) => {
      ...
      const selected = getAccountIdForSelection(currency, id, selectedAccountIds, selectedGroup)
          ...
          >
            {!notGrouped && currencies.map((currency, i) => (
              <Fragment key={currency}>
                ...
                {i + 1 < currencies.length &&
                  <Divider style={cs(isRtl, styles.accountGroupsDivider, styles.accountGroupsDividerRtl)} />}
              </Fragment>
            ))}
            ...
            {notGrouped && accounts.map(a => (
              <AccountItem
                ...
                isRtl={isRtl}
                isDisabled={false}
                isChecked={selectedAccountIds[0].companyAccountId === a.companyAccountId}
                account={a}
