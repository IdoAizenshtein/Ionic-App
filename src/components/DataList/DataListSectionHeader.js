...
export default class DataListSectionHeader extends PureComponent {
  render () {
    ...
    const sum = (total === undefined)
      ? (itra
        ? ((data[0] && data[0].itra !== undefined) ? getFormattedValueArray(data[0].itra) : null)
        : (data[0] && data[0].uniItratSgira !== undefined) ? getFormattedValueArray(data[0].uniItratSgira) : null)
      : getFormattedValueArray(total)

    let cashFlowTextPast = false
    if (sum && (!first || (title !== AppTimezone.moment().format('DD/MM/YY') && cashFlow))) {
      cashFlowTextPast = AppTimezone.moment(title, 'DD/MM/YY').isBefore(AppTimezone.moment())
    }
    let isBetween = false
    if (cashFlow && account) {
