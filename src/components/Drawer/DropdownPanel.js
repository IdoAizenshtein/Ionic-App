...
export default class DropdownPanel extends PureComponent {
  ...
  setMaxHeight = (e) => this.setState({ maxHeight: e.nativeEvent.layout.height })
  ...
  componentWillReceiveProps ({ isOpen, items }) {
    if (!items) return
    if (this.props.isOpen && !isOpen) return this.menuClose()
    if (!this.props.isOpen && isOpen) return this.menuOpen()
  }

  render () {
    const { height } = this.state
    const { title, items, isRtl, icon, isFirst, isLast, isOpen, navigation, notActive, name } = this.props
    const divider = <Divider style={[cs(isRtl, styles.divider, styles.dividerRtl), {
