...
import { Image, Keyboard, Modal, SafeAreaView, Text, TextInput, TouchableOpacity, View, WebView } from 'react-native'
...
import AddTokenModal from '../../screens/SettingsScreen/components/BaseTokenTab/components/AddTokenModal/AddTokenModal'
...
export default class AlertsTrial extends PureComponent {
    ...
    if (statusTokenFinished === true) {
      return (
        <View style={[styles.alertWrapper, {
          ...
          <Text style={{
            ...
            {updateToken && isValid ? 'הטעינה בוצעה בהצלחה, ' : 'משיכת נתונים מהבנק הושלמה!'}
          </Text>
          ...
          <TouchableOpacity
            ...
            }}>{updateToken && isValid ? 'רענון נתונים' : 'החלף נתוני חברה לדוגמה לנתוני אמת'}</Text>
          </TouchableOpacity>
        </View>
      )
    } else if ([
      ...
      return (
        <View style={{
          ...
          <View style={{ width: 250, height: 6, backgroundColor: 'powderblue', marginTop: 5, marginBottom: 2 }}>
  ...
  setData = () => {
    clearInterval(this.intervalId)
    const { token, companies, currentCompanyId, onlyShowPopUp, getToken, isSettings, user, dontShowActivated } = this.props
    this.getAlertTokens(currentCompanyId)
    if (token && companies && currentCompanyId) {
      const company = this.currentCompany

      const showAlert = company.daysLeftTillExpire !== null && company.daysLeftTillExpire <= 14 && company.daysLeftTillExpire > 0
      const showPopUp = isSettings ? (company.daysLeftTillExpire !== null && company.daysLeftTillExpire <= 14 && !(company.daysLeftTillExpire === 0 && company.billingAccountId === '00000000-0000-0000-0000-000000000000')) : (company.daysLeftTillExpire !== null && company.daysLeftTillExpire <= 14)

      let isFromAccountFalseTokens = []
      if (getToken && getToken.length && getToken.filter((item) => !item.isFromAccount).length) {
        isFromAccountFalseTokens = getToken.filter((item) => !item.isFromAccount).map((it) => it.token)
        if (isFromAccountFalseTokens.length) {
          this.getStatus(currentCompanyId, isFromAccountFalseTokens)
        }
      }

      const showAlertState = !onlyShowPopUp && ALERTS_TRIAL.showAlert && showAlert
      let getTokenShow = (!showPopUp && getToken && ALERTS_TRIAL.showAlert) ? ((!getToken.length || !getToken.filter((item) => !item.isFromAccount).length) ? 'example' : 'temp') : null
      ...
      this.setState({
        ...
        showPopUp: ((company.daysLeftTillExpire > 0 && company.billingAccountId === '00000000-0000-0000-0000-000000000000') || company.daysLeftTillExpire === 0) && ALERTS_TRIAL.showPopUp && showPopUp && !(!onlyShowPopUp && showAlert && company.billingAccountId !== '00000000-0000-0000-0000-000000000000'),
        activated: !dontShowActivated && ALERTS_TRIAL.showAlertActivated && !showAlertState && !getTokenShow && user && user.activated === false,
      })
    }
  }
  ...
  getStatus = (companyId, tokens) => {
    if (companyId) {
      return getStatusTokenTypeApi.post({ body: { companyId, tokens: tokens } })
        .then(([newTokenStatus]) => {
          ...
          if ([BANK_TOKEN_STATUS.VALID, BANK_TOKEN_STATUS.PASSWORD_ABOUT_TO_EXPIRED].includes(statusCode)) {
            accountCflApi.post({ body: { uuid: companyId } })
              .then(data => {
                if ((data.exampleCompany === false) && (this.state.statusToken === BANK_TOKEN_STATUS.INVALID_PASSWORD || [BANK_TOKEN_STATUS.VALID, BANK_TOKEN_STATUS.PASSWORD_ABOUT_TO_EXPIRED].includes(this.state.statusToken))
                ) {
                  this.stopPullingTokenStatus(this.state.statusToken, false)
                } else {
    ...
    if (companyId) {
      return getStatusTokenTypeApi.post({ body: { companyId, tokens: tokens } })
        .then(([newTokenStatus]) => {
          const statusCode = BankTokenService.getTokenStatusCode(newTokenStatus.tokenStatus)
          this.setState({ newTokenStatus, statusToken: statusCode })
          if (statusCode !== BANK_TOKEN_STATUS.VALID && statusCode !== BANK_TOKEN_STATUS.VALIDPOALIMBAASAKIM) {
          ...
          .then(([newTokenStatus]) => {
            ...
            setTimeout(() => {
              if (statusCode === BANK_TOKEN_STATUS.INVALID_PASSWORD || [BANK_TOKEN_STATUS.VALID, BANK_TOKEN_STATUS.PASSWORD_ABOUT_TO_EXPIRED].includes(statusCode)) {
                this.stopPullingTokenStatus(this.state.statusToken, true)
              }
            }, 20)
          })
      }
            ...
            accountCflApi.post({ body: { uuid: currentCompanyId } })
              .then(data => {
                if ((data.exampleCompany === false) &&
                  (this.state.statusToken === BANK_TOKEN_STATUS.INVALID_PASSWORD || [BANK_TOKEN_STATUS.VALID, BANK_TOKEN_STATUS.PASSWORD_ABOUT_TO_EXPIRED].includes(this.state.statusToken))
                ) {
                  this.stopPullingTokenStatus(this.state.statusToken, true)
                }
              })
  ...
  stopPullingTokenStatus = (statusTokenFinished, isValid) => {
    let valid = false
    if (isValid) {
      valid = statusTokenFinished === BANK_TOKEN_STATUS.VALID || statusTokenFinished === BANK_TOKEN_STATUS.VALIDPOALIMBAASAKIM
    }
    this.setState({
      statusTokenFinished: statusTokenFinished === BANK_TOKEN_STATUS.VALID || statusTokenFinished === BANK_TOKEN_STATUS.VALIDPOALIMBAASAKIM,
      isValid: valid,
    })
  ...
  switchCompany = () => {
    ...
    this.intervalSwitch = setInterval(() => {
      ...
      this.setState({ switchCompany: true, timeoutBackRealData })
  ...
  switchCompanyNow = () => {
    ...
    clearInterval(this.intervalSwitch)
    this.setState({ switchCompany: false, timeoutBackRealData: 5 })
  ...
  setTokenBank = () => {
    this.setState({ addAccountModalIsOpen: true, idCompany: true })
  }
  ...
  refresh = () => {
    ...
    if (updateToken) {
      ...
      this.props.navigation.setParams({ updateToken: false })
      setTimeout(() => {
        refresh()
      }, 100)
    }
  ...
  handleUpdateMail = () => {
    ...
    if (stepMail === 2) {
      if (inProgress || !(
        mailValid && passwordValid && !emailExists && password.length > 0 && mail.length > 0
      )) {
        return
      }
    }
    ...
    if (stepMail === 1) {
      ...
      this.setState({ inProgress: true, errPass: false })
      return updateUserMailApi.post({
        ...
        .then((data) => {
          this.setState({ inProgress: false, stepMail: 3 })
        })
        .catch(() => {
          this.setState({ inProgress: false, errPass: true })
        })
    }
  }
  activatedModalClose = () => {
  ...
  render () {
    const { token, t, currentCompanyId, accounts, user, isRtl, navigation } = this.props
    const {
      isValid,
    } = this.state
      ...
      <Fragment>
        ...
        {showPopUp && (
          <View style={{
            ...
            <View style={{
              ...
              {currentCompany.daysLeftTillExpire > 0 && billingAccountId === '00000000-0000-0000-0000-000000000000' && (
                <TouchableOpacity
                  style={{
                    marginTop: 16,
                    marginBottom: 27,
              ...
              {currentCompany.daysLeftTillExpire === 0 && (
                <View style={{
                  ...
                  <Text style={{
                    ...
                    {(billingAccountId === '00000000-0000-0000-0000-000000000000') ? 'לצערנו עדיין לא השלמת את תהליך הרכישה של bizibox' : 'המערכת נחסמה לשימוש. אנא הסדירו את התשלום'}
                  </Text>
                </View>
              )}
                ...
                <View style={{
                  ...
                  <TouchableOpacity
                    ...
                    <Text style={{
                      ...
                      {(billingAccountId === '00000000-0000-0000-0000-000000000000') ? 'למעבר לתהליך הרכישה' : 'הסדרת תשלום'}
                    </Text>
                  </TouchableOpacity>
                </View>
              )}
        ...
        {getTokenShow && (
          ...
          <View style={{
            ...
            {((getTokenShow === 'temp' && statusToken !== null) && ((statusToken === 0 && statusTokenFinished) || (statusToken !== 0 && !statusTokenFinished))) &&
            (<View
              style={{
                height: 46.5,
                flexDirection: 'row-reverse',
              ...
              <View style={{
                ...
                <TouchableOpacity onPress={this.handleCloseAlertToken}>
                  <Icon name='close' type='material-community' size={25} color={'#022258'} />
                </TouchableOpacity>
              </View>
            </View>
            )}
            ...
            {(getTokenShow === 'updateToken' && statusToken !== null) && (
              <View
                style={{
                  ...
                  flexDirection: 'row-reverse',
                  alignSelf: 'center',
                  justifyContent: statusToken === BANK_TOKEN_STATUS.TECHNICAL_PROBLEM || statusToken === BANK_TOKEN_STATUS.INVALID_PASSWORD_AND_ACCESS ? 'flex-start' : 'center',
                  alignItems: 'center',
                  alignContent: 'center',
                  width: '100%',
                  backgroundColor: '#73b573',
        ...
        (ALERTS_TRIAL.alertTokens && alertTokens && alertTokens.length > 0) && (
          <View style={{
            position: 'absolute',
            top: 0,
            right: 0,
            ...
            <View style={{
              ...
              {alertTokens.map((token, _id) => (
                <TouchableOpacity
                  style={{
                    flexDirection: 'row-reverse',
                  }}
                  key={token + _id}
                  onPress={this.goToSettings(token.source)}>
                  <Text style={{
                    color: colors.white,
                    fontFamily: fonts.bold,
                    fontSize: sp(18),
                  ...
                  {(alertTokens.length > 1 && (_id + 1 !== alertTokens.length)) && (
                    <Text style={{
                      color: colors.white,
                      fontFamily: fonts.semiBold,
                      fontSize: sp(18),
        ...
        {(activated && !(ALERTS_TRIAL.alertTokens && alertTokens && alertTokens.length > 0)) && (
          <View style={{
            position: 'absolute',
            top: 0,
            right: 0,
              ...
              <View style={{
                ...
                <View style={{
                  ...
                  <Text
                    ...
                    style={{
                      color: '#000000',
                      fontFamily: IS_IOS ? fonts.semiBold : fonts.regular,
                      fontSize: sp(12.5),
                      textAlign: 'right',
                    }}>
                    <Text style={{
              ...
              <View style={[{
                ...
                <View style={{ alignItems: 'center', flex: 70, alignSelf: 'center' }}>
                  <Text
                    ...
                    {stepMail === 1 ? 'מייל האימות נשלח בהצלחה' : 'החלפת מייל'}
                  </Text>
                </View>
                {stepMail !== 3 && (
                  <View>
                    <TouchableOpacity
                      ...
                      onPress={stepMail === 1 ? this.activatedModalClose : this.handleUpdateMail}>
                      <Text style={{
                    ...
                    <View style={{
                      ...
                      <TextInput
                        ...
                        style={[
                          ...
                          {
                            width: 280,
                            borderBottomColor: (!passwordValid || errPass) ? colors.red2 : colors.blue3,
                            borderBottomWidth: 2,
                            fontFamily: fonts.semiBold,
                            fontWeight: 'normal',
                            color: colors.blue29,
