...
import { combineStyles as cs, getErrText, sp } from 'src/utils/func'
...
const AnimatedSectionList = Animated.createAnimatedComponent(IS_IOS ? KeyboardAwareSectionList : SectionList)
const AnimatedFlatList = Animated.createAnimatedComponent(IS_IOS ? KeyboardAwareFlatList : FlatList)
...
class ScrollCompWithHeader extends PureComponent {
    ...
    componentWillReceiveProps (props) {
      if (
        props.currentSlideIndex !== this.props.currentSlideIndex
      ) {
        setTimeout(() => {
        ...
        <View>
          <View>
            <Swiper
              ...
              {
                this.props.selectedCardsSlider.map((item, i) => {
            ...
            <View style={commonStyles.horizontalCenterContainer}>
              ...
              {cardsLengthExist && cardsLengthExist.length > 0 && (
                <Button
                  ...
                  onPress={cardsLengthExist && cardsLengthExist.length > 0 ? onOpenCardsModal : addCard}
                  title={cardsLengthExist && cardsLengthExist.length > 0 ? t('common:changeFilter') : t('slika:noAccountsButtonText')}
                />
              )}
...
export default class BaseCardsScreen extends PureComponent {
  ...
  get headerMinHeight () {
    return this.hasHeaderAlert ? HEADER_MIN_HEIGHT + HEADER_ALERT_BORDER_HEIGHT : HEADER_MIN_HEIGHT
  }
  ...
  get selectedNotUpdatedCards () {
    ...
    if (currentCardSlideId === null) {
      ...
      if (this.props.type === 'Slika') {
        return this.selectedCards.filter(c => c.solekId === currentCardSlideId && c.isUpdate === false)
      } else {
        return this.selectedCards.filter(c => c.creditCardId === currentCardSlideId && c.isUpdate === false)
      }
    }
  }
  ...
  get selectedLowCreditLimitCards () {
    const { currentCardSlideId } = this.state
    if (currentCardSlideId === null) {
      return this.selectedCards.filter(c => (c.availableCredit / c.creditLimit * 100) < 30)
    } else {
      return this.selectedCards.filter(c => c.creditCardId === currentCardSlideId && ((c.availableCredit / c.creditLimit * 100) < 30))
    }
  }
  ...
  get selectedNoCreditLimitCards () {
    const { currentCardSlideId } = this.state
    if (currentCardSlideId === null) {
      return this.selectedCards.filter(c => c.availableCredit <= 0 && c.creditLimit !== null)
    } else {
      return this.selectedCards.filter(c => c.creditCardId === currentCardSlideId && c.availableCredit <= 0 && c.creditLimit !== null)
    }
  }
  ...
  get headerAlertState () {
    ...
    return getAlertState(
      (currentCardSlideId === null) ? this.selectedCards : (this.props.type === 'Slika' ? this.selectedCards.filter(c => c.solekId === currentCardSlideId) : this.selectedCards.filter(c => c.creditCardId === currentCardSlideId)),
  ...
  get cardsAggregatedDataBySections () {
    ...
    if (!cardsAggregatedData || !cardsAggregatedData.length) return []
  ...
  get isFilterIncorrect () {
    return Object.keys(this.incorrectFilter).some(key => this.incorrectFilter[key] === true)
  }

  get selectedCardsSlider () {
    if (this.selectedCards.length > 1) return [null, ...this.selectedCards]
    ...
    getAggregateData = (cardIds = null, setLoader = true) => {
      ...
      if (setLoader) this.setState({ inProgress: true, isLayoutComplete: false })
      if (cards.length) {
        const body = aggregatedBody(cardIds)
        return onGetAggregateData({ body })
          .then(data => {
            data = getFormattedAggregatedData(data, cardIds.length === 1)
            if (cardIds.length === 1) return this.setData(data, cardIds[0])
            return this.setAggregatedData(data)
          })
          .catch((err) => this.setState({ inProgress: false, error: getErrText(err) }))
      } else {
    ...
    setData = (data, cardId) => {
      ...
      if (oldIndex >= 0) {
        newCardsData[oldIndex] = { data: getLast12MonthData(data, true), cardId }
      } else {
        newCardsData.push({ data: getLast12MonthData(data, true), cardId })
      }
    ...
    handleCloseCardsModal = () => {
      this.setState({
        selectedCardIds: JSON.parse(JSON.stringify(this.state.selectedCardIdsSave)),
        cardsModalIsOpen: false,
      })
    };
    ...
    handleOpenCreditLimitModal = (cardId) => {
      this.setState({ creditLimitUpdateIsOpen: true, currentCreditCardId: cardId })
    };

    handleCloseCreditLimitModal = () => {
      this.setState({ creditLimitUpdateIsOpen: false, newCreditLimit: '' })
    };
    ...
    handleSubmitNewCreditLimit = () => {
      ...
      if (!newCreditLimit.length || isNaN(newCreditLimit)) return
      const currentCreditCard = cards.find(card => card.creditCardId === currentCreditCardId)
      const updatedCreditCard = { ...currentCreditCard, creditLimit: Number(newCreditLimit) }
    ...
    handleOpenCardsModal = () => this.setState({
    ...
    handleToggleSelectCard = (id) => {
      ...
      if (hasSelected) return this.setState({ selectedCardIds: selectedCardIds.filter(s => s !== id) })
    ...
    handleToggleSelectCardGroup = (cardIds = []) => {
      ...
      if (hasSelected) return this.setState({ selectedCardIds: selectedCardIds.filter(id => !cardIds.includes(id)) })
    ...
    handleToggleSelectAllCards = () => {
      ...
      if (cards.length === selectedCardIds.length) return this.setState({ selectedCardIds: [] })
    ...
    renderCarouselItem = ({ item }) => {
      const { getLast12MonthData, getCarouselItemData, SliderItem, aggregatedFutureBalance, aggregatedFutureCharges, aggregatedFutureCredits, type } = this.props
      ...
      if (!item) {
        return (
          <SliderItem
            ...
            account={null}
            card={null}
            currentMonth={currentOpenItem ? ((currentOpenItem._id) ? currentOpenItem.month + currentOpenItem._id : currentOpenItem.month) : null}
      ...
      if (data && data.data) {
        if (type === 'Card') {
          dataAll = data.data.filter((item) => {
            return (!item.cardDetails.length) || (item.cardDetails.length && item.cardDetails[0].monthlyTotal.iskatHul === 'ILS')
          })
        } else {
          dataAll = data.data
        }
    ...
    renderAggregatedScrollHead = ({ section }) => {
      return <CreditCardAggregatedDataHead section={section} type={this.props.type} />
    };

    renderListHeader = () => {
      return <View style={{ backgroundColor: colors.white, width: '100%', height: SLIDER_ITEM_HEIGHT }} />
    };
    ...
    handleSnapToItem = (index) => {
      ...
      const cardId = selectedCardIds[selectedCardIds.length > 1 ? index - 1 : index]
      this.setState({
      ...
      if (cardsData.find(d => d.cardId === cardId)) return
    ...
    handleSetCreditLimitModalPosition = (e) => this.setState({ creditLimitModalYPosition: e.nativeEvent.layout.y });
    ...
    render () {
      ...
      const isFiltersIncorrect = true
      // Object.keys(this.incorrectFilter).some(key => this.incorrectFilter[key] === true)

      if (this.isLoader) return <Loader />
      ...
      const notHasData = (
        (!cards || (cards && cards.length === 0)) ||
            ...
            (!accountsWithCards || (Array.isArray(accountsWithCards) && (accountsWithCards && accountsWithCards.length === 0)) || (!Array.isArray(accountsWithCards) && accountsWithCards.futureBallance === null && accountsWithCards.futureCharges === null && accountsWithCards.futureCredits === null)) ||
            (!cardsLengthExist || (cardsLengthExist && cardsLengthExist.length === 0)) ||
            (currentCardSlideId &&
                isLayoutComplete && (!currentAggregatedData || (currentAggregatedData && currentAggregatedData.data && currentAggregatedData.data.length === 0))) ||
            (!currentCardSlideId &&
                isLayoutComplete &&
                (!cardsAggregatedData || (cardsAggregatedData && cardsAggregatedData.length === 0))
            )
      )

      return (
        <View style={commonStyles.mainContainer}>
          <AlertsTrial refresh={this._onRefresh} updateToken={navigation.getParam('updateToken')}
            navigation={navigation} />
          ...
          <Fragment>
            {(
              (cards && (cards && cards.length > 0)) &&
                        (selectedCards && (selectedCards && selectedCards.length > 0)) &&
                        (accountsWithCards && (Array.isArray(accountsWithCards) && (accountsWithCards.length > 0))) &&
                        (cardsLengthExist && (cardsLengthExist && cardsLengthExist.length > 0)) &&
                        (currentCardSlideId &&
                            currentAggregatedData &&
                            isLayoutComplete &&
                            (currentAggregatedData && currentAggregatedData.data && currentAggregatedData.data.length > 0))
            ) && (
              <AnimatedFlatList
                refreshControl={
                  <RefreshControl
                ...
                data={[...currentAggregatedData.data].reverse()}
            ...
            {(alertDetailsIsOpen && Alert) ? (
              <Alert
                t={t}
                isRtl={isRtl}
                top={alertYPosition - ((!currentCardSlideId) ? this.currentScrollPosition : currentScrollPosition)}
                selectedNotUpdatedCards={selectedNotUpdatedCards}
                selectedLowCreditLimitCards={selectedLowCreditLimitCards}
                selectedNoCreditLimitCards={selectedNoCreditLimitCards}
                alertState={alertState}
            ...
            {isCreditCardScreen && creditLimitUpdateIsOpen && (
              <CreditCardUpdateLimit
                ...
                currentScrollPosition={((!currentCardSlideId) ? this.currentScrollPosition : currentScrollPosition)}
              />
            )}
                    ...
                    <ScrollView
                      ...
                      <View style={{
                        ...
                        <Text style={{
                          ...
                          <Text style={{
                            fontFamily: fonts.semiBold,
                          }}>{updateTypePopup.mainDesc ? updateTypePopup.mainDesc : updateTypePopup.mainDescription}</Text></Text>
                        <Text style={{
                          fontSize: sp(22),
                          color: colors.blue32,
                          fontFamily: fonts.regular,
