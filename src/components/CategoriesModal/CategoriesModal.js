...
import { getAccountCflTransTypeApi, updateAccountCflTransTypeApi } from 'src/api/index'
...
export default class CategoriesModal extends PureComponent {
  ...
  removeCategory = (transTypeId) => {
    ...
    this.setState({ categories: this.state.categories.filter(c => c.transTypeId !== transTypeId) })
  ...
  handleCreateCategory = () => {
    ...
    this.setState({ isReady: false, createInProgress: true })
  ...
  handleUpdateNewCategoryName = (newCategoryName) => this.setState({ newCategoryName })
  ...
  handleChangeCategory = (newCategory) => {
    ...
    return updateAccountCflTransTypeApi.post({
      ...
      if (this.props.companyId) {
        getAccountCflTransTypeApi.post({ body: { uuid: this.props.companyId } })
          .then(data => {
            this.setState({ categories: data.filter(c => c.shonaScreen), isReady: true })
          })
      }
    })
  }
  ...
  componentDidMount () {
    if (this.props.companyId) {
      getAccountCflTransTypeApi.post({ body: { uuid: this.props.companyId } })
        .then(data => this.setState({ categories: data.filter(c => c.shonaScreen), isReady: true }))
    }
  }
              ...
              return (
                <Fragment key={c.transTypeId}>
                  <CategoryCard
                    ...
                    isEditable={!c.createDefaultSupplier && c.companyId !== '00000000-0000-0000-0000-000000000000'}
