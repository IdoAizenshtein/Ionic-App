...
export function getListOfDatesInterval (start, end, interval = 'days', format = 'YYYY-MM-DD') {
...
export function isCardUpdated (balanceLastUpdatedDate) {
  return AppTimezone.moment(balanceLastUpdatedDate).format('DD/MM/YYYY') === AppTimezone.moment().format('DD/MM/YYYY')
}
...
export function getDaysBetweenTwoDates (now, end) {
  ...
  return Math.abs(Math.floor(AppTimezone.moment.duration(AppTimezone.moment(end).diff(AppTimezone.moment(now))).asDays()))
}